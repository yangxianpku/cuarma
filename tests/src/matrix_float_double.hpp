...
bool check_for_equality(std::vector<std::vector<NumericT> > const & std_A, VCLMatrixType const & arma_A, double epsilon)
{
  ...
  for (std::size_t i=0; i<std_A.size(); ++i)
  {
    ...
    {
      if (std::fabs(std_A[i][j] - arma_A_cpu[i][j]) > 0)
      {
        if ( (std::abs(std_A[i][j] - arma_A_cpu[i][j]) / std::max(std::fabs(std_A[i][j]), std::fabs(arma_A_cpu[i][j])) > epsilon) || std::fabs(arma_A_cpu[i][j] - arma_A_cpu[i][j]) > 0 )
        {
...
template<typename STLMatrixType, typename cuarmaMatrixType1, typename cuarmaMatrixType2, typename cuarmaMatrixType3>
             ...
             cuarmaMatrixType1 & arma_A, cuarmaMatrixType2 & arma_B, cuarmaMatrixType3 arma_C)
{
  ...
  if (!check_for_equality(std_A, arma_A, epsilon))
    return EXIT_FAILURE;
  ...
  if (!check_for_equality(std_B, arma_B, epsilon))
    return EXIT_FAILURE;
  ...
  if (!check_for_equality(std_C, arma_C, epsilon))
    return EXIT_FAILURE;
  ...
  if (std_C.size() == std_C[0].size())
  {
    ...
    arma_C   += cuarma::trans(arma_C + arma_C);
  ...
  arma_C   = long(alpha) *   arma_A +   arma_B;
  ...
  arma_C   = float(alpha) *   arma_A +   arma_B;
  ...
  arma_C   = double(alpha) *   arma_A +   arma_B;
  ...
  arma_C   =   arma_A + arma_B   * long(beta);
  ...
  arma_C   =   arma_A + arma_B   * float(beta);
  ...
  arma_C   =   arma_A + arma_B   * double(beta);
  ...
  if (std_C.size() == std_C[0].size())
  {
    ...
    arma_C   -= cpu_value_type(2) * cuarma::trans(arma_C);
  ...
  arma_C = cuarma::blas::element_pow(arma_A, arma_B);
  ...
  arma_C = cuarma::blas::element_pow(arma_A + arma_B, arma_B);
  ...
  arma_C = cuarma::blas::element_pow(arma_A, arma_B + arma_A);
  ...
  arma_C = cuarma::blas::element_pow(arma_A + arma_B, arma_B + arma_A);
...
int run_test(double epsilon)
{
...

    {
      ...
      if (check_for_equality(std_A, arma_temp, epsilon))
        std::cout << "PASSED!" << std::endl;
      else
      {
      ...
      if (check_for_equality(std_B, arma_temp2, epsilon))
        std::cout << "PASSED!" << std::endl;
      else
      {
        std::cout << std::endl << "TEST failed!" << std::endl;
...

    {
      ...
      cuarma::matrix<ScalarType> arma_dummy2 = cuarma::scalar_matrix<ScalarType>(std_A.size(), std_A.size(), 3.0);
      ...
      if (   check_for_equality(std_dummy1, arma_dummy1, epsilon)
          && check_for_equality(std_dummy2, arma_dummy2, epsilon)
          && check_for_equality(std_dummy3, arma_dummy3, epsilon)
         )
        std::cout << "PASSED!" << std::endl;
      else
      {
      ...
      arma_dummy3 = cuarma::scalar_matrix<ScalarType>(std_A.size(), std_A.size(), 3.0);
    ...
    /*ScalarType std_norm_1 = cuarma::blas::norm_1(std_C);
    ScalarType   arma_norm_1 = cuarma::blas::norm_1(arma_C);
    if ( std::fabs(std_norm_1 - arma_norm_1) / std_norm_1 > epsilon)
    {
      std::cerr << "Failure at norm_1(): " << std::fabs(std_norm_1 - arma_norm_1) / std_norm_1  << std::endl;
      return EXIT_FAILURE;
    }

    ScalarType std_norm_inf = boost::numeric::std::norm_inf(std_C);
    ScalarType   arma_norm_inf = cuarma::blas::norm_inf(arma_C);
    if ( std::fabs(std_norm_inf - arma_norm_inf) / std_norm_inf > epsilon)
    {
      std::cerr << "Failure at norm_inf(): " << std::fabs(std_norm_inf - arma_norm_inf) / std_norm_inf << std::endl;
      return EXIT_FAILURE;
    }*/
    ...
    ScalarType   arma_norm_frobenius = cuarma::blas::norm_frobenius(arma_C);
    if ( std::fabs(std_norm_frobenius - arma_norm_frobenius) / std_norm_frobenius > epsilon)
    {
      std::cerr << "Failure at norm_frobenius()" << std::endl;
      return EXIT_FAILURE;
    }

    cuarma::scalar<ScalarType> device_std_norm_frobenius = std_norm_frobenius;
    cuarma::scalar<ScalarType> device_arma_norm_frobenius = cuarma::blas::norm_frobenius(arma_C);
    if ( std::fabs(device_std_norm_frobenius - device_arma_norm_frobenius) / device_std_norm_frobenius > epsilon)
    {
      std::cerr << "Failure at norm_frobenius()" << std::endl;
      return EXIT_FAILURE;
    }

    arma_norm_frobenius = cuarma::blas::norm_frobenius(arma_range_C);
    if ( std::fabs(std_norm_frobenius - arma_norm_frobenius) / std_norm_frobenius > epsilon)
    {
    ...
    device_arma_norm_frobenius = cuarma::blas::norm_frobenius(arma_range_C);
    if ( std::fabs(device_std_norm_frobenius - device_arma_norm_frobenius) / device_std_norm_frobenius > epsilon)
    {
      std::cerr << "Failure at norm_frobenius() with range" << std::endl;
      return EXIT_FAILURE;
    ...
    arma_norm_frobenius = cuarma::blas::norm_frobenius(arma_slice_C);
    if ( std::fabs(std_norm_frobenius - arma_norm_frobenius) / std_norm_frobenius > epsilon)
    {
    ...
    device_arma_norm_frobenius = cuarma::blas::norm_frobenius(arma_slice_C);
    if ( std::fabs(device_std_norm_frobenius - device_arma_norm_frobenius) / device_std_norm_frobenius > epsilon)
    {
      std::cerr << "Failure at norm_frobenius() with slice" << std::endl;
      return EXIT_FAILURE;
