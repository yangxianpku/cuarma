...
*    @coding UTF-8
*    @brief  Tests routines for dense matrices, row-major, signed precision.
*    @brief  ≥Ì√‹æÿ’Û£¨––¥Ê¥¢£¨”–∑˚∫≈’˚–Õ
*/
...
int main (int, const char **)
{
  ...
  if (run_test<cuarma::row_major, int>() != EXIT_SUCCESS)
    return EXIT_FAILURE;

  std::cout << "# Testing setup:" << std::endl;
  std::cout << "  numeric: long" << std::endl;
  std::cout << " --- row-major ---" << std::endl;
  if (run_test<cuarma::row_major, long>() != EXIT_SUCCESS)
    return EXIT_FAILURE;
