...
{
  typedef cuarma::matrix<T, F> result_type;
  static result_type make(cuarma::matrix<T, F> const &, boost::numeric::ublas::matrix<T> & base)
  {
    cuarma::matrix<T, F> result(base.size1(), base.size2());
    cuarma::copy(base, result);
    return result;
  }
};
...
struct matrix_maker< boost::numeric::ublas::matrix_range<MatrixT>, F>
{
  ...
  typedef cuarma::matrix_range< cuarma::matrix<T, F> > result_type;

  static result_type make(cuarma::matrix<T, F> & M, boost::numeric::ublas::matrix_range<MatrixT> & base)
  {
    cuarma::range r0(base.start1(), base.start1() + base.size1());
    cuarma::range r1(base.start2(), base.start2() + base.size2());
...
{
  typedef typename MatrixT::value_type T;
  typedef cuarma::matrix_slice< cuarma::matrix<T, F> > result_type;

  static result_type make(cuarma::matrix<T, F> & M, boost::numeric::ublas::matrix_slice<MatrixT> & base)
  {
    cuarma::slice s0(base.start1(), std::size_t(base.stride1()), base.size1());
    cuarma::slice s1(base.start2(), std::size_t(base.stride2()), base.size2());
...
int test_all_layouts(std::size_t CM, std::size_t CN, RefCType & cC,
                     ...
                     T epsilon)
{
  cuarma::matrix<T, cuarma::row_major> ArowTmp(AM, AK);
  cuarma::matrix<T, cuarma::row_major> ATrowTmp(AK, AM);
  cuarma::matrix<T, cuarma::row_major> BrowTmp(BK, BN);
  cuarma::matrix<T, cuarma::row_major> BTrowTmp(BN, BK);
  cuarma::matrix<T, cuarma::row_major> CrowTmp(CM, CN);

  cuarma::matrix<T, cuarma::column_major> AcolTmp(AM, AK);
  cuarma::matrix<T, cuarma::column_major> ATcolTmp(AK, AM);
  cuarma::matrix<T, cuarma::column_major> BcolTmp(BK, BN);
  cuarma::matrix<T, cuarma::column_major> BTcolTmp(BN, BK);
  cuarma::matrix<T, cuarma::column_major> CcolTmp(CM, CN);


  typename matrix_maker<RefCType, cuarma::row_major>::result_type Crow = matrix_maker<RefCType, cuarma::row_major>::make(CrowTmp, cC);
  typename matrix_maker<RefAType, cuarma::row_major>::result_type Arow = matrix_maker<RefAType, cuarma::row_major>::make(ArowTmp, cA);
  typename matrix_maker<RefAType, cuarma::row_major>::result_type ATrow = matrix_maker<RefAType, cuarma::row_major>::make(ATrowTmp, cAT);
  typename matrix_maker<RefBType, cuarma::row_major>::result_type Brow = matrix_maker<RefBType, cuarma::row_major>::make(BrowTmp, cB);
  typename matrix_maker<RefBType, cuarma::row_major>::result_type BTrow = matrix_maker<RefBType, cuarma::row_major>::make(BTrowTmp, cBT);

  typename matrix_maker<RefCType, cuarma::column_major>::result_type Ccol = matrix_maker<RefCType, cuarma::column_major>::make(CcolTmp, cC);
  typename matrix_maker<RefAType, cuarma::column_major>::result_type Acol = matrix_maker<RefAType, cuarma::column_major>::make(AcolTmp, cA);
  typename matrix_maker<RefAType, cuarma::column_major>::result_type ATcol = matrix_maker<RefAType, cuarma::column_major>::make(ATcolTmp, cAT);
  typename matrix_maker<RefBType, cuarma::column_major>::result_type Bcol = matrix_maker<RefBType, cuarma::column_major>::make(BcolTmp, cB);
  typename matrix_maker<RefBType, cuarma::column_major>::result_type BTcol = matrix_maker<RefBType, cuarma::column_major>::make(BTcolTmp, cBT);
...
void init_rand(MatrixType & A)
{
  ...
  cuarma::tools::uniform_random_numbers<T> randomNumber;
