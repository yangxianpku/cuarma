...
/**  @file   vector_int.cu
 *   @coding UTF-8
 *   @brief  Tests vector operations (BLAS level 1) for signed integer arithmetic.
 *   @brief  测试：向量-整型标量
 */
         ...
         cuarmaVectorType1 & arma_v1, cuarmaVectorType2 & arma_v2)
{
  ...
  cpu_result2 = 0;
  for (std::size_t i=0; i<std_v1.size(); ++i)   //note: norm_1 broken for unsigned ints on MacOS
    cpu_result2 += std::abs(std_v1[i] + std_v2[i]);
  ...
  cpu_result = 0;
  for (std::size_t i=0; i<std_v1.size(); ++i)
    if (std::abs(std_v1[i]) > cpu_result)
      cpu_result = std::abs(std_v1[i]);
  ...
  cpu_result = 0;
  for (std::size_t i=0; i<std_v1.size(); ++i)
    if (std::abs(std_v1[i] + std_v2[i]) > cpu_result)
    {
      cpu_result = std::abs(std_v1[i] + std_v2[i]);
      cpu_index = i;
    }
  cpu_result = std_v1[cpu_index];
  ...
  std::vector<NumericT> x = std_v1;
  std::vector<NumericT> y = std_v2;
  for (std::size_t i=0; i<std_v1.size(); ++i)
  {
    NumericT tmp;
    tmp  =   NumericT(1) * x[i] + NumericT(2) * y[i];
    y[i] = - NumericT(2) * x[i] + NumericT(1) * y[i];
    x[i] = tmp;
  }

  cuarma::blas::plane_rotation(arma_v1, arma_v2, NumericT(1), NumericT(2));

  if (check(x, arma_v1) != EXIT_SUCCESS)
    return EXIT_FAILURE;
  if (check(y, arma_v2) != EXIT_SUCCESS)
    return EXIT_FAILURE;
  ...
  for (std::size_t i=0; i<std_v1.size(); ++i)
    std_v1[i] = std_v1[i] / alpha - std_v2[i] / alpha;
  ...
  for (std::size_t i=0; i<std_v1.size(); ++i)
    std_v1[i] = beta * (std_v1[i] - alpha * std_v2[i]);
  ...
  for (std::size_t i=0; i<std_v1.size(); ++i)
  {
    std_v1[i] = NumericT(1.0) + NumericT(i);
    std_v2[i] = NumericT(5.0) + NumericT(i);
  }
...
#define GENERATE_UNARY_OP_TEST(FUNCNAME) \
  ...
  \
  for (std::size_t i=0; i<std_v1.size(); ++i) \
    std_v1[i] = std::FUNCNAME(std_v2[i]); \
 ...
 \
  for (std::size_t i=0; i<std_v1.size(); ++i) \
    std_v1[i] = std::FUNCNAME(std_v1[i] + std_v2[i]); \
 ...
 \
  for (std::size_t i=0; i<std_v1.size(); ++i) \
    std_v1[i] += std::FUNCNAME(std_v1[i]); \
 ...
 \
  ...
  for (std::size_t i=0; i<std_v1.size(); ++i)
    std_v1[i] = std_v2[i] / alpha + beta * std_v1[i] - alpha * std_v2[i] + beta * std_v1[i] - alpha * std_v1[i];
