...
{
  typedef typename cuarma::result_of::reference_if_nonscalar<LHS>::type     lhs_reference_type;
  typedef typename cuarma::result_of::reference_if_nonscalar<RHS>::type     rhs_reference_type;
...
public:
  ...
  arma_size_t size1() const { return cuarma::tools::MATRIX_SIZE_DEDUCER<LHS, RHS, OP>::size1(lhs_, rhs_); }
  arma_size_t size2() const { return cuarma::tools::MATRIX_SIZE_DEDUCER<LHS, RHS, OP>::size2(lhs_, rhs_); }
...
public:
  ...
  matrix_iterator(MatrixT & mat,
                  arma_size_t start_row,
                  arma_size_t start_col) : mat_(mat), row_(start_row), col_(start_col) {}

  value_type operator*(void) { return mat_(row_, col_); }
  self_type & operator++(void) { cuarma::tools::MATRIX_ITERATOR_INCREMENTER<ROWCOL, MatrixT>::apply(mat_, row_, col_); return *this; }
  ...
  arma_size_t index1() { return row_; }
  arma_size_t index2() { return col_; }
...
* @param ctx      Optional context in which the matrix is created (one out of multiple CUDA contexts, host)
*/
template<class NumericT, typename SizeT, typename DistanceT>
matrix_base<NumericT, SizeT, DistanceT>::matrix_base(size_type rows, size_type columns, bool is_row_major, cuarma::context ctx)
  : size1_(rows), size2_(columns), start1_(0), start2_(0), stride1_(1), stride2_(1),
    internal_size1_(cuarma::tools::align_to_multiple<size_type>(rows, dense_padding_size)),
    internal_size2_(cuarma::tools::align_to_multiple<size_type>(columns, dense_padding_size)),
    row_major_fixed_(true), row_major_(is_row_major)
{
  if (rows > 0 && columns > 0)
  {
    cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), ctx);
    clear();
  }
}
...
matrix_base<NumericT, SizeT, DistanceT>::matrix_base(matrix_expression<const LHS, const RHS, OP> const & proxy) :
  size1_(cuarma::traits::size1(proxy)), size2_(cuarma::traits::size2(proxy)), start1_(0), start2_(0), stride1_(1), stride2_(1),
  internal_size1_(cuarma::tools::align_to_multiple<size_type>(size1_, dense_padding_size)),
  internal_size2_(cuarma::tools::align_to_multiple<size_type>(size2_, dense_padding_size)),
  row_major_fixed_(true), row_major_(cuarma::traits::row_major(proxy))
{
  elements_.switch_active_handle_id(cuarma::traits::active_handle_id(proxy));
  if (internal_size() > 0)
  {
    cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), cuarma::traits::context(proxy));
    clear();
...
// CUDA or host memory:
template<class NumericT, typename SizeT, typename DistanceT>
matrix_base<NumericT, SizeT, DistanceT>::matrix_base(NumericT * ptr_to_mem, cuarma::memory_types mem_type,
  ...
  : size1_(mat_size1), size2_(mat_size2),
    ...
    row_major_fixed_(true), row_major_(is_row_major)
{
  if (mem_type == cuarma::CUDA_MEMORY)
  {
#ifdef CUARMA_WITH_CUDA
    elements_.switch_active_handle_id(cuarma::CUDA_MEMORY);
  ...
  else if (mem_type == cuarma::MAIN_MEMORY)
  {
    elements_.switch_active_handle_id(cuarma::MAIN_MEMORY);
...
{
  elements_.switch_active_handle_id(cuarma::traits::active_handle_id(other));
  if (internal_size() > 0)
  {
    cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), cuarma::traits::context(other));
    clear();
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator=(const self_type & other)  //enables implicit conversions
{
  ...
  cuarma::blas::am(*this,other, cpu_value_type(1.0), 1, false, false);
  return *this;
}
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator=(const matrix_base<OtherNumericT, SizeT, DistanceT> & other)
{
  ...
  cuarma::blas::convert(*this, other);
  return *this;
}
...
{
  assert(  (cuarma::traits::size1(proxy) == size1() || size1() == 0)
           && (cuarma::traits::size2(proxy) == size2() || size2() == 0)
           && bool("Incompatible matrix sizes!"));
  if (internal_size() == 0 && cuarma::traits::size1(proxy) > 0 && cuarma::traits::size2(proxy) > 0)
  {
    size1_ = cuarma::traits::size1(proxy);
    size2_ = cuarma::traits::size2(proxy);
    internal_size1_ = cuarma::tools::align_to_multiple<size_type>(size1_, dense_padding_size);
    internal_size2_ = cuarma::tools::align_to_multiple<size_type>(size2_, dense_padding_size);
    if (!row_major_fixed_)
      row_major_ = cuarma::traits::row_major(proxy);
    cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), cuarma::traits::context(proxy));
  ...
  if (internal_size() > 0)
    blas::detail::op_executor<self_type, op_assign, matrix_expression<const LHS, const RHS, OP> >::apply(*this, proxy);
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator=(const matrix_expression<const self_type, const self_type, op_trans> & proxy)
{
  ...
  {
    cuarma::matrix_base<NumericT> temp(proxy.lhs().size2(), proxy.lhs().size1(),proxy.lhs().row_major());
    cuarma::blas::trans(proxy, temp);
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator+=(const matrix_expression<const LHS, const RHS, OP> & proxy)
{
  assert(  (cuarma::traits::size1(proxy) == size1())
           && (cuarma::traits::size2(proxy) == size2())
           && bool("Incompatible matrix sizes!"));
  assert( (size1() > 0) && bool("Vector not yet initialized!") );
  assert( (size2() > 0) && bool("Vector not yet initialized!") );

  blas::detail::op_executor<self_type, op_inplace_add, matrix_expression<const LHS, const RHS, OP> >::apply(*this, proxy);
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator-=(const matrix_expression<const LHS, const RHS, OP> & proxy)
{
  ...
  blas::detail::op_executor<self_type, op_inplace_sub, matrix_expression<const LHS, const RHS, OP> >::apply(*this, proxy);
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator = (identity_matrix<NumericT> const & m)
{
  ...
  if (internal_size() == 0)
  {
    ...
    if (internal_size() > 0)
    {
      cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), m.context());
      clear();
    }
  }
  else
    cuarma::blas::matrix_assign(*this, NumericT(0));

  if (internal_size() > 0)
    cuarma::blas::matrix_diagonal_assign(*this, m(0,0));
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator = (scalar_matrix<NumericT> const & m)
{
  ...
  if (internal_size() > 0)
  {
    cuarma::blas::matrix_assign(*this, m(0,0));
  }
...
{
  cuarma::blas::ambm(*this, *this, NumericT(1.0), 1, false, false, other, NumericT(1.0), 1, false, false);
  return *this;
}
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator -= (const matrix_base<NumericT, SizeT, DistanceT> & other)
{
  cuarma::blas::ambm(*this, *this, NumericT(1.0), 1, false, false, other, NumericT(1.0), 1, false, true);
  return *this;
}
...
{
  cuarma::blas::am(*this, *this, NumericT(val), 1, false, false);
  return *this;
}
...
matrix_base<NumericT, SizeT, DistanceT> & matrix_base<NumericT, SizeT, DistanceT>::operator /= (char val)
{
  cuarma::blas::am(*this, *this, NumericT(val), 1, true, false);
  return *this;
}
...
template<class NumericT, typename SizeT, typename DistanceT>
void matrix_base<NumericT, SizeT, DistanceT>::clear() { cuarma::blas::matrix_assign(*this, NumericT(0), true); }
...
void matrix_base<NumericT, SizeT, DistanceT>::resize(size_type rows, size_type columns, bool preserve)
{
  ...
  if (preserve && internal_size() > 0)
  {
    ...
    cuarma::backend::memory_read(elements_, 0, sizeof(NumericT)*internal_size(), &(old_entries[0]));

    //set up entries of new matrix:
    std::vector< NumericT > new_entries(  cuarma::tools::align_to_multiple<arma_size_t>(rows,    dense_padding_size)
                                          * cuarma::tools::align_to_multiple<arma_size_t>(columns, dense_padding_size));
    for (size_type i=0; i<rows; ++i)
    {
      ...
      for (size_type j=0; j<columns; ++j)
      {
        if (j >= size2_)
          continue;
        if (row_major_)
          new_entries[row_major::mem_index(i, j, cuarma::tools::align_to_multiple<arma_size_t>(rows, dense_padding_size), cuarma::tools::align_to_multiple<arma_size_t>(columns, dense_padding_size))]
              = old_entries[row_major::mem_index(i, j, internal_size1(), internal_size2())];
        else
          new_entries[column_major::mem_index(i, j, cuarma::tools::align_to_multiple<arma_size_t>(rows, dense_padding_size), cuarma::tools::align_to_multiple<arma_size_t>(columns, dense_padding_size))]
    ...
    internal_size1_ = cuarma::tools::align_to_multiple<size_type>(size1_, dense_padding_size);
    internal_size2_ = cuarma::tools::align_to_multiple<size_type>(size2_, dense_padding_size);
    cuarma::backend::memory_create(elements_, sizeof(NumericT)*new_entries.size(), cuarma::traits::context(elements_), &(new_entries[0]));
  }
  else //discard old entries:
  {
    ...
    cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), cuarma::traits::context(elements_));
    clear();
  }
}
...
public:
  ...
  explicit matrix() : base_type(static_cast<bool>(cuarma::is_row_major<F>::value)) {}
  ...
  /** @brief Creates the matrix with the given dimensions
    ...
    * @param ctx      Optional context in which the matrix is created (one out of multiple CUDA contexts, host)
    */
  explicit matrix(size_type rows, size_type columns, cuarma::context ctx = cuarma::context()) : base_type(rows, columns, cuarma::is_row_major<F>::value, ctx) {}

  /** @brief Wraps a CUDA or host buffer provided by the user.
    *
    * @param ptr_to_mem   The pointer to existing memory
    * @param mem_type     Type of the memory (either cuarma::CUDA_MEMORY if available, or cuarma::HOST_MEMORY)
    * @param rows         Number of rows of the matrix
    * @param cols         Number of columns of the matrix
    */
  explicit matrix(NumericT * ptr_to_mem, cuarma::memory_types mem_type, size_type rows, size_type cols)
    : base_type(ptr_to_mem, mem_type,
                rows, 0, 1, rows,
                cols, 0, 1, cols,
                cuarma::is_row_major<F>::value) {}
  ...
  /** @brief Wraps a CUDA or host buffer provided by the user including padding of rows and columns.
    ...
    */
  explicit matrix(NumericT * ptr_to_mem, cuarma::memory_types mem_type,
    ...
    : base_type(ptr_to_mem, mem_type,
                ...
                true, cuarma::is_row_major<F>::value) {}
  ...
  matrix(identity_matrix<NumericT> const & m) : base_type(m.size1(), m.size2(), cuarma::is_row_major<F>::value, m.context())
  {
  ...
  matrix(zero_matrix<NumericT> const & m) : base_type(m.size1(), m.size2(), cuarma::is_row_major<F>::value, m.context())
  {
    if (base_type::internal_size() > 0)
      base_type::operator=(m);
  }
  ...
  matrix(scalar_matrix<NumericT> const & m) : base_type(m.size1(), m.size2(), cuarma::is_row_major<F>::value, m.context())
  {
    if (base_type::internal_size() > 0)
      base_type::operator=(m);
  }

  matrix(const base_type & other) : base_type(other.size1(), other.size2(), cuarma::is_row_major<F>::value, cuarma::traits::context(other))
  {
  ...
  //copy constructor:
  matrix(const self_type & other) : base_type(other.size1(), other.size2(), cuarma::is_row_major<F>::value, cuarma::traits::context(other))
  {
    base_type::operator=(other);
  }
  ...
  base_type & operator=(cuarma::matrix<OtherNumericT, F2> const & B)                          { return base_type::operator=(static_cast<cuarma::matrix_base<OtherNumericT> const &>(B)); }

  template<typename OtherNumericT, typename F2>
  base_type & operator=(cuarma::matrix_range<cuarma::matrix<OtherNumericT, F2> > const & B) { return base_type::operator=(static_cast<cuarma::matrix_base<OtherNumericT> const &>(B)); }

  template<typename OtherNumericT, typename F2>
  base_type & operator=(cuarma::matrix_slice<cuarma::matrix<OtherNumericT, F2> > const & B) { return base_type::operator=(static_cast<cuarma::matrix_base<OtherNumericT> const &>(B)); }
...
* @param gpu_matrix   A dense cuarma matrix
*/
...
std::ostream & operator<<(std::ostream & s, const matrix_base<NumericT> & gpu_matrix)
{
  ...
  cuarma::backend::memory_read(gpu_matrix.handle(), 0, sizeof(NumericT) * gpu_matrix.internal_size(), &(tmp[0]));
...
{
  typedef typename cuarma::tools::CPU_SCALAR_TYPE_DEDUCER< typename tools::CONST_REMOVER<LHS>::ResultType >::ResultType     ScalarType;
...
//
/** @brief Copies a dense matrix from the host (CPU) to the device (GPU or multi-core CPU)
*
          ...
          matrix<NumericT, F, AlignmentV> & gpu_matrix )
{
  ...
  cuarma::backend::memory_write(gpu_matrix.handle(), 0, sizeof(NumericT) * data.size(), &(data[0]));
  //gpu_matrix.elements_ = cuarma::ocl::current_context().create_memory(CL_MEM_READ_WRITE, data);
...
//cpu to gpu, STL type:
//
/** @brief Copies a dense STL-type matrix from the host (CPU) to the device (GPU or multi-core CPU)
*
...
//
/** @brief Copies a dense matrix from the host (CPU) to the device (GPU or multi-core CPU) without temporary. Matrix-Layout on CPU must be equal to the matrix-layout on the GPU.
*
               ...
               matrix<NumericT, F, AlignmentV> & gpu_matrix)
{
  if (gpu_matrix.internal_size() == 0)
    cuarma::backend::memory_create(gpu_matrix.handle(), sizeof(NumericT) * static_cast<arma_size_t>(cpu_matrix_end - cpu_matrix_begin), cuarma::traits::context(gpu_matrix), cpu_matrix_begin);
  else
  {
    assert( (gpu_matrix.internal_size() >= static_cast<arma_size_t>(cpu_matrix_end - cpu_matrix_begin)) && bool("fast_copy(): Matrix not large enough to fit data!"));
    cuarma::backend::memory_write(gpu_matrix.handle(), 0, sizeof(NumericT) * static_cast<arma_size_t>(cpu_matrix_end - cpu_matrix_begin), cpu_matrix_begin);
  }
}
...
//
/** @brief Copies a dense matrix from the device (GPU or multi-core CPU) to the host (CPU).
*
* @param gpu_matrix   A dense cuarma matrix
          ...
          CPUMatrixT & cpu_matrix )
{
  ...
  if ( (gpu_matrix.size1() > 0) && (gpu_matrix.size2() > 0) )
  {
    assert( cuarma::traits::size1(cpu_matrix) == gpu_matrix.size1() && bool("Matrix dimensions mismatch: rows"));

    std::vector<NumericT> temp_buffer(gpu_matrix.internal_size());
    cuarma::backend::memory_read(gpu_matrix.handle(), 0, sizeof(NumericT)*gpu_matrix.internal_size(), &(temp_buffer[0]));

    //now copy entries to cpu_matrix:
    for (size_type i = 0; i < gpu_matrix.size1(); ++i)
    {
      assert( cuarma::traits::size2(cpu_matrix) == gpu_matrix.size2() && bool("Matrix dimensions mismatch: columns"));
      for (size_type j = 0; j < gpu_matrix.size2(); ++j)
...
{
  cuarma::backend::memory_read(gpu_matrix.handle(), 0, sizeof(NumericT)*gpu_matrix.internal_size(), cpu_matrix_begin);
}
...
matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_expression<const LHS2, const RHS2, OP2>, op_add>
...
{
  assert( (cuarma::traits::size1(proxy1) == cuarma::traits::size1(proxy2)) && (cuarma::traits::size2(proxy1) == cuarma::traits::size2(proxy2)) && bool("Incompatible matrix sizes!"));
  return matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_expression<const LHS2, const RHS2, OP2>, op_add>(proxy1, proxy2);
}

template<typename LHS1, typename RHS1, typename OP1, typename NumericT>
matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_base<NumericT>, op_add>
operator + (matrix_expression<const LHS1, const RHS1, OP1> const & proxy1, matrix_base<NumericT> const & proxy2)
{
  assert(    (cuarma::traits::size1(proxy1) == cuarma::traits::size1(proxy2))
             && (cuarma::traits::size2(proxy1) == cuarma::traits::size2(proxy2))
             && bool("Incompatible matrix sizes!"));
  return matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_base<NumericT>, op_add> (proxy1, proxy2);
}

template<typename NumericT, typename LHS2, typename RHS2, typename OP2>
matrix_expression< const matrix_base<NumericT>, const matrix_expression<const LHS2, const RHS2, OP2>, op_add>
operator + (matrix_base<NumericT> const & proxy1, matrix_expression<const LHS2, const RHS2, OP2> const & proxy2)
{
  assert(    (cuarma::traits::size1(proxy1) == cuarma::traits::size1(proxy2))
             && (cuarma::traits::size2(proxy1) == cuarma::traits::size2(proxy2))
             && bool("Incompatible matrix sizes!"));
  return  matrix_expression< const matrix_base<NumericT>, const matrix_expression<const LHS2, const RHS2, OP2>, op_add>(proxy1, proxy2);
}
...
{
  return matrix_expression< const matrix_base<NumericT>, const matrix_base<NumericT>, op_add > (m1, m2);
}


// operator -
template<typename LHS1, typename RHS1, typename OP1, typename LHS2, typename RHS2, typename OP2>
matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_expression<const LHS2, const RHS2, OP2>, op_sub>
operator - (matrix_expression<const LHS1, const RHS1, OP1> const & proxy1, matrix_expression<const LHS2, const RHS2, OP2> const & proxy2)
{
  assert(    (cuarma::traits::size1(proxy1) == cuarma::traits::size1(proxy2))
             && (cuarma::traits::size2(proxy1) == cuarma::traits::size2(proxy2))
             && bool("Incompatible matrix sizes!"));
  return matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>,const matrix_expression<const LHS2, const RHS2, OP2>, op_sub>(proxy1, proxy2);
}

template<typename LHS1, typename RHS1, typename OP1, typename NumericT>
matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_base<NumericT>, op_sub>
operator - (matrix_expression<const LHS1, const RHS1, OP1> const & proxy1, matrix_base<NumericT> const & proxy2)
{
  assert(    (cuarma::traits::size1(proxy1) == cuarma::traits::size1(proxy2))
             && (cuarma::traits::size2(proxy1) == cuarma::traits::size2(proxy2))
             && bool("Incompatible matrix sizes!"));
  return matrix_expression< const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_base<NumericT>, op_sub>(proxy1, proxy2);
}

template<typename NumericT, typename LHS2, typename RHS2, typename OP2>
matrix_expression< const matrix_base<NumericT>, const matrix_expression<const LHS2, const RHS2, OP2>, op_sub>
            ...
            matrix_expression<const LHS2, const RHS2, OP2> const & proxy2)
{
  assert(    (cuarma::traits::size1(proxy1) == cuarma::traits::size1(proxy2))
             && (cuarma::traits::size2(proxy1) == cuarma::traits::size2(proxy2))
             && bool("Incompatible matrix sizes!"));
  return  matrix_expression< const matrix_base<NumericT>, const matrix_expression<const LHS2, const RHS2, OP2>, op_sub>(proxy1, proxy2);
}
...
// operator *
/** @brief Operator overload for the expression alpha * m1, where alpha is a host scalar (float or double) and m1 is a cuarma matrix.
* @param value   The host scalar (float or double)
* @param m1      A cuarma matrix
*/
template<typename S1, typename NumericT>
typename cuarma::enable_if<    cuarma::is_any_scalar<S1>::value,
...
/** @brief Operator overload for the multiplication of a matrix expression with a cuarma scalar from the left, e.g. alpha * (beta * m1). Here, beta * m1 is wrapped into a matrix_expression and then multiplied with alpha from the left.
*
* @param val     Right hand side scalar
* @param proxy   Left hand side matrix expression
*/
...
template<typename NumericT, typename S1>
typename cuarma::enable_if< cuarma::is_scalar<S1>::value, matrix_base<NumericT> & >::type
operator *= (matrix_base<NumericT> & m1, S1 const & gpu_val)
{
  bool is_sign_flip = cuarma::is_flip_sign_scalar<S1>::value;
  cuarma::blas::am(m1, m1, gpu_val, 1, false, is_sign_flip ? true : false);
  return m1;
}
...
{
  cuarma::blas::am(m1, m1, NumericT(gpu_val), 1, false, false);
  return m1;
}
...
operator /= (matrix_base<NumericT> & m1, S1 const & gpu_val)
{
  cuarma::blas::am(m1, m1, gpu_val, 1, true, false);
  return m1;
}
...
{
  cuarma::blas::am(m1, m1, NumericT(gpu_val), 1, true, false);
  return m1;
}
...
cuarma::matrix_expression< const cuarma::matrix_expression< const vector_base<NumericT>, const vector_base<NumericT>, op_prod>,
const S1,
op_mult>
>::type
operator*(const cuarma::matrix_expression< const vector_base<NumericT>, const vector_base<NumericT>, op_prod> & proxy,
          const S1 & val)
{
  return cuarma::matrix_expression< const cuarma::matrix_expression< const vector_base<NumericT>, const vector_base<NumericT>, op_prod>,
      const S1,
      op_mult>(proxy, val);
}

template<typename NumericT, typename S1>
typename cuarma::enable_if< cuarma::is_cpu_scalar<S1>::value,
cuarma::matrix_expression< const cuarma::matrix_expression< const vector_base<NumericT>, const vector_base<NumericT>, op_prod>,
const NumericT,
op_mult>
>::type
...
operator*(const S1 & val,
          const cuarma::matrix_expression< const vector_base<NumericT>, const vector_base<NumericT>, op_prod> & proxy)
{
  return cuarma::matrix_expression< const cuarma::matrix_expression< const vector_base<NumericT>, const vector_base<NumericT>, op_prod>,
      const S1,
      op_mult>(proxy, val);
...
namespace detail
{
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_base<T> >
  {
    ...
    {
      cuarma::blas::am(lhs, rhs, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_trans> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_trans> const & rhs)
    {
      ...
      cuarma::blas::am(lhs, temp, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const matrix_expression<const LhsT, const RhsT, OpT>, const matrix_expression<const LhsT, const RhsT, OpT>, op_trans> >
  {
                                                              ...
                                                              op_trans> const & rhs)
    {
      ...
      matrix_base<T> temp2(cuarma::trans(temp1));
      cuarma::blas::am(lhs, temp2, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_base<T> >
  {
    static void apply(matrix_base<T> & lhs, matrix_base<T> const & rhs)
    {
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, rhs, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_trans> >
  {
    ...
    {
      matrix_base<T> temp(rhs);
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, temp, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_expression<const matrix_expression<const LhsT, const RhsT, OpT>, const matrix_expression<const LhsT, const RhsT, OpT>, op_trans> >
  {
                                                              ...
                                                              op_trans> const & rhs)
    {
      matrix_base<T> temp1(rhs.rhs());
      matrix_base<T> temp2(cuarma::trans(temp1));
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, temp2, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_sub, matrix_base<T> >
  {
    static void apply(matrix_base<T> & lhs, matrix_base<T> const & rhs)
    {
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, rhs, T(1), 1, false, true);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_sub, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_trans> >
  {
    ...
    {
      matrix_base<T> temp(rhs);
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, temp, T(1), 1, false, true);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_sub, matrix_expression<const matrix_expression<const LhsT, const RhsT, OpT>, const matrix_expression<const LhsT, const RhsT, OpT>, op_trans> >
  {
                                                              ...
                                                              op_trans> const & rhs)
    {
      matrix_base<T> temp1(rhs.rhs());
      matrix_base<T> temp2(cuarma::trans(temp1));
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, temp2, T(1), 1, false, true);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const matrix_base<T>, const ScalarType, op_mult> >
  {
    ...
    {
      cuarma::blas::am(lhs, proxy.lhs(), proxy.rhs(), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_expression<const matrix_base<T>, const ScalarType, op_mult> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const ScalarType, op_mult> const & proxy)
    {
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, false, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_sub, matrix_expression<const matrix_base<T>, const ScalarType, op_mult> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const ScalarType, op_mult> const & proxy)
    {
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, false, true);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const matrix_base<T>, const ScalarType, op_div> >
  {
    ...
    {
      cuarma::blas::am(lhs, proxy.lhs(), proxy.rhs(), 1, true, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_expression<const matrix_base<T>, const ScalarType, op_div> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const ScalarType, op_div> const & proxy)
    {
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, true, false);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_sub, matrix_expression<const matrix_base<T>, const ScalarType, op_div> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const ScalarType, op_div> const & proxy)
    {
      cuarma::blas::ambm(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, true, true);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const LHS, const RHS, op_prod> >
  {
    ...
    static void apply(matrix_base<T> & lhs, matrix_expression<const SparseMatrixType,
                      const cuarma::matrix_base<T>,
                      cuarma::op_prod> const & proxy)
    {
      // check for x = A * x
      if (op_aliasing(lhs, proxy.rhs()))
      {
        ...
        cuarma::blas::prod_impl(proxy.lhs(), proxy.rhs(), lhs);
    }

    // dense = sparse * trans(dense)
    template< typename SparseMatrixType >
    static void apply(matrix_base<T> & lhs, matrix_expression<const SparseMatrixType,
                      const cuarma::matrix_expression< const cuarma::matrix_base<T>,
                      const cuarma::matrix_base<T>,
                      cuarma::op_trans >,
                      cuarma::op_prod> const & proxy)
    {
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const LHS, const int, op_vector_diag> >
  {
    ...
    {
      cuarma::blas::matrix_diag_from_vector(proxy.lhs(), proxy.rhs(), lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const LHS, const int, op_matrix_diag> >
  {
    ...
    {
      cuarma::blas::matrix_diag_to_vector(proxy.lhs(), proxy.rhs(), lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const LHS, const unsigned int, op_row> >
  {
    ...
    {
      cuarma::blas::matrix_row(proxy.lhs(), proxy.rhs(), lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const LHS, const unsigned int, op_column> >
  {
    ...
    {
      cuarma::blas::matrix_column(proxy.lhs(), proxy.rhs(), lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const matrix_base<T>, const matrix_base<T>, op_row_sum> >
  {
    ...
    {
      cuarma::blas::row_sum_impl(proxy.lhs(), lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const matrix_expression<LHS, RHS, OP>, const matrix_expression<LHS, RHS, OP>, op_row_sum> >
  {
    static void apply(vector_base<T> & lhs, vector_expression<const matrix_expression<LHS, RHS, OP>, const matrix_expression<LHS, RHS, OP>, op_row_sum> const & proxy)
    {
      ...
      cuarma::blas::row_sum_impl(tmp, lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const matrix_base<T>, const matrix_base<T>, op_col_sum> >
  {
    ...
    {
      cuarma::blas::column_sum_impl(proxy.lhs(), lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const matrix_expression<LHS, RHS, OP>, const matrix_expression<LHS, RHS, OP>, op_col_sum> >
  {
    ...
    {
      matrix_base<T> tmp(proxy.lhs());
      cuarma::blas::column_sum_impl(tmp, lhs);
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const LHS, const RHS, op_element_binary<OP> > >
  {
    ...
    {
      cuarma::blas::element_op(lhs, proxy);
    }
    ...
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const matrix_expression<const LHS2, const RHS2, OP2>, op_element_binary<OP> > const & proxy)
    {
      ...
      cuarma::blas::element_op(lhs, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_binary<OP> >(proxy.lhs(), temp));
    }
    ...
    {
      matrix_base<T> temp(proxy.lhs());
      cuarma::blas::element_op(lhs, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_binary<OP> >(temp, proxy.rhs()));
    }
                      ...
                      op_element_binary<OP> > const & proxy)
    {
      ...
      cuarma::blas::element_op(lhs, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_binary<OP> >(temp1, temp2));
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_expression<const LHS, const RHS, op_element_binary<OP> > >
  {
    ...
    {
      matrix_base<T> temp(proxy.rhs());
      matrix_base<T> temp2(temp.size1(), temp.size2(), lhs.row_major(), cuarma::traits::context(lhs));
      cuarma::blas::element_op(temp2, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_binary<OP> >(proxy.lhs(), temp));
      lhs += temp2;
    }
    ...
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_expression<const LHS1, const RHS1, OP1>, const matrix_base<T>, op_element_binary<OP> > const & proxy)
    {
      matrix_base<T> temp(proxy.lhs());
      matrix_base<T> temp2(temp.size1(), temp.size2(), lhs.row_major(), cuarma::traits::context(lhs));
      cuarma::blas::element_op(temp2, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_binary<OP> >(temp, proxy.rhs()));
      lhs += temp2;
    }
                      ...
                      op_element_binary<OP> > const & proxy)
    {
      matrix_base<T> temp1(proxy.lhs());
      matrix_base<T> temp2(proxy.rhs());
      matrix_base<T> temp3(temp1.size1(), temp1.size2(), lhs.row_major(), cuarma::traits::context(lhs));
      cuarma::blas::element_op(temp3, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_binary<OP> >(temp1, temp2));
      lhs += temp3;
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const LHS, const RHS, op_element_unary<OP> > >
  {
    ...
    {
      matrix_base<T> temp(proxy.rhs());
      cuarma::blas::element_op(lhs, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_unary<OP> >(temp, temp));
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_expression<const LHS, const RHS, op_element_unary<OP> > >
  {
    ...
    {
      matrix_base<T> temp(proxy.rhs());
      cuarma::blas::element_op(temp, cuarma::matrix_expression<const matrix_base<T>, const matrix_base<T>, op_element_unary<OP> >(temp, temp)); // inplace operation is safe here
      lhs += temp;
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_mat_mat_prod> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_mat_mat_prod> const & rhs)
    {
      if (op_aliasing(lhs, rhs.lhs()) || op_aliasing(lhs, rhs.rhs()))
      {
        ...
        cuarma::blas::prod_impl(rhs.lhs(), rhs.rhs(), lhs, T(1.0), T(0));
    }
  };
                    ...
                    >
  {
                                        ...
                                        op_mat_mat_prod> const & rhs)
    {
      ...
      cuarma::blas::prod_impl(rhs.lhs(), temp, lhs, T(1.0), T(0));
    }
  };
                    ...
                    >
  {
                                        ...
                                        op_mat_mat_prod> const & rhs)
    {
      ...
      cuarma::blas::prod_impl(temp, rhs.rhs(), lhs, T(1.0), T(0));
    }
  };
                    ...
                    >
  {
                                        ...
                                        op_mat_mat_prod> const & rhs)
    {
      ...
      cuarma::blas::prod_impl(temp1, temp2, lhs, T(1.0), T(0));
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_add, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_mat_mat_prod> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const matrix_base<T>, const matrix_base<T>, op_mat_mat_prod> const & rhs)
    {
      if (op_aliasing(lhs, rhs.lhs()) || op_aliasing(lhs, rhs.rhs()))
      {
        ...
        cuarma::blas::prod_impl(rhs.lhs(), rhs.rhs(), lhs, T(1.0), T(1.0));
    }
  };
                    ...
                    >
  {
    ...
    {
      matrix_base<T> temp(rhs.rhs());
      cuarma::blas::prod_impl(rhs.lhs(), temp, lhs, T(1.0), T(1.0));
    }
  };
                    ...
                    >
  {
    ...
    {
      matrix_base<T> temp(rhs.lhs());
      cuarma::blas::prod_impl(temp, rhs.rhs(), lhs, T(1.0), T(1.0));
    }
  };
                    ...
                    >
  {
                                        ...
                                        op_mat_mat_prod> const & rhs)
    {
      matrix_base<T> temp1(rhs.lhs());
      matrix_base<T> temp2(rhs.rhs());
      cuarma::blas::prod_impl(temp1, temp2, lhs, T(1.0), T(1.0));
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const matrix_base<T>, const vector_base<T>, op_prod> >
  {
    static void apply(vector_base<T> & lhs, vector_expression<const matrix_base<T>, const vector_base<T>, op_prod> const & rhs)
    {
      ...
      if (op_aliasing(lhs, rhs.rhs()))
      {
        ...
        cuarma::blas::prod_impl(rhs.lhs(), rhs.rhs(), lhs);
    }
  };
                    ...
                    >
  {
    ...
    {
      matrix_base<T> temp(rhs.lhs());
      cuarma::blas::prod_impl(temp, rhs.rhs(), lhs);
    }
  };
                    ...
                    >
  {
                                        ...
                                        op_prod> const & rhs)
    {
      ...
      cuarma::blas::prod_impl(rhs.lhs(), x, lhs);
    }
  };
                    ...
                    >
  {
                                        ...
                                        op_prod> const & rhs)
    {
      matrix_base<T> A(rhs.lhs());
      vector_base<T> y(lhs);
      cuarma::blas::prod_impl(A, rhs.rhs(), y);
      lhs += y;
    }
  };
                    ...
                    >
  {
                                        ...
                                        op_prod> const & rhs)
    {
      vector_base<T> x(rhs.rhs());
      vector_base<T> y(lhs);
      cuarma::blas::prod_impl(rhs.lhs(), x, y);
      lhs += y;
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_assign, matrix_expression<const vector_base<T>, const vector_base<T>, op_prod> >
  {
    ...
    {
      lhs.clear();
      cuarma::blas::scaled_rank_1_update(lhs, T(1.0), 1, false, false, rhs.lhs(), rhs.rhs());
    }
  };
      ...
      op_mult> >
  {
    ...
    {
      lhs.clear();
      cuarma::blas::scaled_rank_1_update(lhs, rhs.rhs(), 1, false, false, rhs.lhs().lhs(), rhs.lhs().rhs());
    }
  };
  ...
  struct op_executor<matrix_base<T>, op_inplace_sub, matrix_expression<const vector_base<T>, const vector_base<T>, op_prod> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression<const vector_base<T>, const vector_base<T>, op_prod> const & rhs)
    {
      cuarma::blas::scaled_rank_1_update(lhs, T(1.0), 1, false, true, rhs.lhs(), rhs.rhs());
    }
  };
      ...
      op_mult> >
  {
    static void apply(matrix_base<T> & lhs, matrix_expression< const matrix_expression<const vector_base<T>, const vector_base<T>, op_prod>,
                      ...
                      op_mult> const & rhs)
    {
      cuarma::blas::scaled_rank_1_update(lhs, rhs.rhs(), 1, false, true, rhs.lhs().lhs(), rhs.lhs().rhs());
    }
  };
