...
/** @file cuarma/detail/matrix_def.hpp
 *  @encoding:UTF-8 文档编码
...
protected:
  ...
  implicit_matrix_base(size_type size1, size_type size2, NumericT value, bool diag, cuarma::context ctx) : size1_(size1), size2_(size2), value_(value), diag_(diag), off_diag_(0), ctx_(ctx){ }
public:
...
// Initializer types
//
/** @brief Represents a vector consisting of 1 at a given index and zeros otherwise. To be used as an initializer for cuarma::vector, vector_range, or vector_slize only. */
template<typename NumericT>
...
public:
  ...
  identity_matrix(size_type s, cuarma::context ctx = cuarma::context()) : implicit_matrix_base<NumericT>(s, s, 1, true, ctx){}
};


/** @brief Represents a vector consisting of zeros only. To be used as an initializer for cuarma::vector, vector_range, or vector_slize only. */
template<typename NumericT>
...
public:
  ...
  zero_matrix(size_type s1, size_type s2, cuarma::context ctx = cuarma::context()) : implicit_matrix_base<NumericT>(s1, s2, 0, false, ctx){}
};
...
public:
  ...
  scalar_matrix(size_type s1, size_type s2, const_reference val, cuarma::context ctx = cuarma::context()) : implicit_matrix_base<NumericT>(s1, s2, val, false, ctx) {}
};
...
public:
  ...
  /** @brief Creates the matrix with the given dimensions
    ...
    */
  explicit matrix_base(size_type rows, size_type columns, bool is_row_major, cuarma::context ctx = cuarma::context());

  /** @brief Constructor for creating a matrix_range or matrix_stride from some other matrix/matrix_range/matrix_stride */
  explicit matrix_base(cuarma::backend::mem_handle & h,
  ...
  // CUDA or host memory:
  explicit matrix_base(NumericT * ptr_to_mem, cuarma::memory_types mem_type,
                       size_type mat_size1, size_type mat_start1, size_type mat_stride1, size_type mat_internal_size1,
                       size_type mat_size2, size_type mat_start2, size_type mat_stride2, size_type mat_internal_size2,
                       bool is_row_major);
  ...
  /** @brief Returns the internal number of rows.  */
  size_type internal_size1() const { return internal_size1_; }

  /** @brief Returns the internal number of columns. */
  ...
  /** @brief Returns the handle, const-version */
  ...
  void switch_memory_context(cuarma::context new_ctx) { cuarma::backend::switch_memory_context<NumericT>(elements_, new_ctx); }

protected:
  void set_handle(cuarma::backend::mem_handle const & h);
