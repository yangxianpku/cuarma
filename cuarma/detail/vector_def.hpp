...
/** @file  cuarma/detail/vector_def.hpp
 *  @encoding:UTF-8 文档编码
...
{
protected:
  implicit_vector_base(arma_size_t s, arma_size_t i, NumericT v, cuarma::context ctx) : size_(s), index_(std::make_pair(true,i)), value_(v), ctx_(ctx){ }
  implicit_vector_base(arma_size_t s, NumericT v, cuarma::context ctx) : size_(s), index_(std::make_pair(false,0)), value_(v), ctx_(ctx){ }

public:
  typedef NumericT const & const_reference;
  typedef NumericT cpu_value_type;

  cuarma::context context() const { return ctx_; }
  arma_size_t size() const { return size_; }
  cpu_value_type  value() const { return value_; }
  arma_size_t index() const { return index_.second; }
  bool has_index() const { return index_.first; }

  cpu_value_type operator()(arma_size_t i) const
  {
    if (index_.first)
...
protected:
  arma_size_t size_;
  std::pair<bool, arma_size_t> index_;
  NumericT value_;
  cuarma::context ctx_;
};
...
{
  unit_vector(arma_size_t s, arma_size_t ind, cuarma::context ctx = cuarma::context()) : implicit_vector_base<NumericT>(s, ind, 1, ctx)
  {
...
/** @brief Represents a vector consisting of scalars 's' only, i.e. v[i] = s for all i. To be used as an initializer for cuarma::vector, vector_range, or vector_slize only. */
template<typename NumericT>
struct scalar_vector : public implicit_vector_base<NumericT>
{
  scalar_vector(arma_size_t s, NumericT val, cuarma::context ctx = cuarma::context()) : implicit_vector_base<NumericT>(s, val, ctx) {}
};

template<typename NumericT>
struct zero_vector : public scalar_vector<NumericT>
{
  zero_vector(arma_size_t s, cuarma::context ctx = cuarma::context()) : scalar_vector<NumericT>(s, 0, ctx){}
};
...
public:
  ...
  cuarma::memory_types memory_domain() const { return elements_.get_active_handle_id();  }
  ...
  /** @brief An explicit constructor for wrapping an existing vector into a vector_range or vector_slice.
    ...
    */
  explicit vector_base(cuarma::backend::mem_handle & h, size_type vec_size, size_type vec_start, size_type vec_stride);

  /** @brief Creates a vector and allocates the necessary memory */
  explicit vector_base(size_type vec_size, cuarma::context ctx = cuarma::context());

  // CUDA or host memory:
  explicit vector_base(NumericT * ptr_to_mem, cuarma::memory_types mem_type, size_type vec_size, arma_size_t start = 0, size_type stride = 1);
  ...
  /** @brief Operator overload for v1 = A * v2, where v1, v2 are vectors and A is a dense matrix.
    * @param proxy An expression template proxy class
    */
  self_type & operator=(const cuarma::vector_expression< const matrix_base<NumericT>, const vector_base<NumericT>, cuarma::op_prod> & proxy);
...
protected:
  ...
  void set_handle(cuarma::backend::mem_handle const & h) {  elements_ = h; }

  /** @brief Swaps the handles of two vectors by swapping the handles only, no data copy */
  ...
  /** @brief Resizes the allocated memory for the vector. Convenience function for setting an context in case reallocation is needed
    *
    *  @param new_size  The new size of the vector
...
private:
  ...
  void resize_impl(size_type new_size, cuarma::context ctx, bool preserve = true);
