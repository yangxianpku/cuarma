...
/** @file cuarma/backend/memory.hpp
 *  @encoding:UTF-8 文档编码
...
#include "cuarma/backend/util.hpp"
...
namespace backend
{
  ...
  /** @brief Synchronizes the execution. finish() will only return after all compute kernels (CUDA) have completed. */
  inline void finish()
  {
#ifdef CUARMA_WITH_CUDA
  ...
  *
  * This is the generic version for CPU RAM and CUDA. Creates the memory in the currently active memory domain.
  *
  ...
  *
  */
  inline void memory_create(mem_handle & handle, arma_size_t size_in_bytes, cuarma::context const & ctx, const void * host_ptr = NULL)
  {
    if (size_in_bytes > 0)
    {
  ...
  /*
  inline void memory_create(mem_handle & handle, arma_size_t size_in_bytes, const void * host_ptr = NULL)
  {
    cuarma::context  ctx(default_memory_type());
  ...
  *
  * This is the generic version for CPU RAM and CUDA. Copies the memory in the currently active memory domain.
  *
  *
  ...
  */
  inline void memory_copy(mem_handle const & src_buffer,
                          ...
                          arma_size_t src_offset,
                          arma_size_t dst_offset,
                          arma_size_t bytes_to_copy)
  {
  ...
  *
  * This is the generic version for CPU RAM and CUDA. Writes the memory in the currently active memory domain.
  *
  ...
  *
  * This is the generic version for CPU RAM and CUDA. Reads the memory from the currently active memory domain.
  *
  ...
  */
  ...
  {
    template<typename T>
    arma_size_t element_size(memory_types /* mem_type */)
    {
      return sizeof(T);
    }
    ...
    template<>
    inline arma_size_t element_size<unsigned long>(memory_types )
    {
      return sizeof(unsigned long);
    }

    template<>
    inline arma_size_t element_size<long>(memory_types )
    {
      return sizeof(long);
    }
    ...
    template<>
    inline arma_size_t element_size<unsigned int>(memory_types )
    {
      return sizeof(unsigned int);
    }

    template<>
    inline arma_size_t element_size<int>(memory_types )
    {
      return sizeof(int);
    }
  ...
  void switch_memory_context(mem_handle & handle, cuarma::context new_ctx)
  {
    if (handle.get_active_handle_id() == new_ctx.memory_type())
      return;

    if (handle.get_active_handle_id() == cuarma::MEMORY_NOT_INITIALIZED || handle.raw_size() == 0)
    {
      handle.switch_active_handle_id(new_ctx.memory_type());
      return;
    }

    arma_size_t size_dst = detail::element_size<DataType>(handle.get_active_handle_id());
    arma_size_t size_src = detail::element_size<DataType>(new_ctx.memory_type());
  ...
  void typesafe_memory_copy(mem_handle const & handle_src, mem_handle & handle_dst)
  {
    ...
    arma_size_t element_size_src = detail::element_size<DataType>(handle_src.get_active_handle_id());
    arma_size_t element_size_dst = detail::element_size<DataType>(handle_dst.get_active_handle_id());
    ...
    if (element_size_src != element_size_dst)
    {
      ...
      case MAIN_MEMORY:
        ...
        for (arma_size_t i=0; i<buffer_dst.size(); ++i)
      ...
      if (handle_dst.raw_size() == buffer_dst.raw_size())
        cuarma::backend::memory_write(handle_dst, 0, buffer_dst.raw_size(), buffer_dst.get());
      else
        cuarma::backend::memory_create(handle_dst, buffer_dst.raw_size(), cuarma::traits::context(handle_dst), buffer_dst.get());
    ...
    else
    {
      ...
      case MAIN_MEMORY:
        ...
        case CUDA_MEMORY:
          if (handle_dst.raw_size() == handle_src.raw_size())
            cuarma::backend::memory_write(handle_dst, 0, handle_src.raw_size(), handle_src.ram_handle().get());
          else
            cuarma::backend::memory_create(handle_dst, handle_src.raw_size(), cuarma::traits::context(handle_dst), handle_src.ram_handle().get());
          break;
      ...
      case CUDA_MEMORY:
        ...
        case MAIN_MEMORY:
          if (handle_dst.raw_size() != handle_src.raw_size())
            cuarma::backend::memory_create(handle_dst, handle_src.raw_size(), cuarma::traits::context(handle_dst));
          cuarma::backend::memory_read(handle_src, 0, handle_src.raw_size(), handle_dst.ram_handle().get());
          break;

        case CUDA_MEMORY:
          if (handle_dst.raw_size() != handle_src.raw_size())
            cuarma::backend::memory_create(handle_dst, handle_src.raw_size(), cuarma::traits::context(handle_dst));
          cuarma::backend::memory_copy(handle_src, handle_dst, 0, 0, handle_src.raw_size());
          break;
...
template<typename T>
void switch_memory_context(T & obj, cuarma::context new_ctx)
{
