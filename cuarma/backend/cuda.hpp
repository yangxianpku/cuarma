...
#define CUARMA_CUDA_ERROR_CHECK(err)  detail::cuda_error_check (err, __FILE__, __LINE__)
...
{
  void * dev_ptr = NULL;
  CUARMA_CUDA_ERROR_CHECK( cudaMalloc(&dev_ptr, size_in_bytes) );
...
inline void memory_copy(handle_type const & src_buffer, handle_type & dst_buffer, arma_size_t src_offset, arma_size_t dst_offset, arma_size_t bytes_to_copy)
{
  ...
  cudaMemcpy(reinterpret_cast<void *>(dst_buffer.get() + dst_offset),reinterpret_cast<void *>(src_buffer.get() + src_offset),bytes_to_copy, cudaMemcpyDeviceToDevice);
}
 ...
 */
inline void memory_write(handle_type & dst_buffer, arma_size_t dst_offset, arma_size_t bytes_to_copy,const void * ptr, bool async = false)
{
  assert( (dst_buffer.get() != NULL) && bool("Memory not initialized!"));

  if (async)
    cudaMemcpyAsync(reinterpret_cast<char *>(dst_buffer.get()) + dst_offset,  reinterpret_cast<const char *>(ptr), bytes_to_copy,cudaMemcpyHostToDevice);
  else
    cudaMemcpy(reinterpret_cast<char *>(dst_buffer.get()) + dst_offset, reinterpret_cast<const char *>(ptr),bytes_to_copy, cudaMemcpyHostToDevice);
}
 ...
 * @param async              Whether the operation should be asynchronous
 */
inline void memory_read(handle_type const & src_buffer,arma_size_t src_offset,arma_size_t bytes_to_copy,void * ptr,bool async = false)
{
  assert( (src_buffer.get() != NULL) && bool("Memory not initialized!"));

  if (async)
    cudaMemcpyAsync(reinterpret_cast<char *>(ptr), reinterpret_cast<char *>(src_buffer.get()) + src_offset,bytes_to_copy,cudaMemcpyDeviceToHost);
  else
    cudaMemcpy(reinterpret_cast<char *>(ptr),reinterpret_cast<char *>(src_buffer.get()) + src_offset, bytes_to_copy,cudaMemcpyDeviceToHost);
}
