...
/** @file  cuarma/vector.hpp
 *  @encoding:UTF-8 文档编码
    @brief The vector type with operator-overloads and proxy classes is defined here.
           Linear algebra operations such as norms and inner products are located in blas/vector_operations.hpp
*/
...
*
* In contrast to full expression templates as introduced by Veldhuizen, cuarma does not allow nested expressions.
* The reason is that this requires automated GPU cuarma::ocl::kernel generation, which then has to be compiles just-in-time.
* For performance-critical applications, one better writes the appropriate cuarma::ocl::kernels by hand.
*
...
public:
  ...
  size_type size() const { return cuarma::traits::size(*this); }
...
public:
  ...
  typedef arma_size_t                 size_type;
  typedef arma_ptrdiff_t              difference_type;
...
public:
  ...
  self_type operator+(difference_type diff) const { return self_type(elements_, static_cast<arma_size_t>(static_cast<difference_type>(base_type::index_) + diff), base_type::start_, base_type::stride_); }
...
template<class NumericT, typename SizeT, typename DistanceT>
vector_base<NumericT, SizeT, DistanceT>::vector_base(cuarma::backend::mem_handle & h,
...
template<class NumericT, typename SizeT, typename DistanceT>
vector_base<NumericT, SizeT, DistanceT>::vector_base(size_type vec_size, cuarma::context ctx)
  : size_(vec_size), start_(0), stride_(1), internal_size_(cuarma::tools::align_to_multiple<size_type>(size_, dense_padding_size))
{
  if (size_ > 0)
  {
...
// CUDA or host memory:
template<class NumericT, typename SizeT, typename DistanceT>
vector_base<NumericT, SizeT, DistanceT>::vector_base(NumericT * ptr_to_mem, cuarma::memory_types mem_type, size_type vec_size, arma_size_t start, size_type stride)
...
vector_base<NumericT, SizeT, DistanceT>::vector_base(vector_expression<const LHS, const RHS, OP> const & proxy)
  : size_(cuarma::traits::size(proxy)), start_(0), stride_(1), internal_size_(cuarma::tools::align_to_multiple<size_type>(size_, dense_padding_size))
{
  if (size_ > 0)
  {
...
vector_base<NumericT, SizeT, DistanceT>::vector_base(const vector_base<NumericT, SizeT, DistanceT> & other) :
  ...
  internal_size_(cuarma::tools::align_to_multiple<size_type>(other.size_, dense_padding_size))
{
...
vector_base<NumericT, SizeT, DistanceT>::vector_base(const vector_base<OtherNumericT> & other) :
  ...
  internal_size_(cuarma::tools::align_to_multiple<size_type>(other.size(), dense_padding_size))
{
  elements_.switch_active_handle_id(cuarma::traits::active_handle_id(other));
  if (internal_size() > 0)
  {
...
vector_base<NumericT, SizeT, DistanceT> & vector_base<NumericT, SizeT, DistanceT>::operator=(const self_type & vec)
{
  ...
  if (vec.size() > 0)
  {
    ...
    {
      size_ = vec.size();
      internal_size_ = cuarma::tools::align_to_multiple<size_type>(size_, dense_padding_size);
      elements_.switch_active_handle_id(vec.handle().get_active_handle_id());
      cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), cuarma::traits::context(vec));
      pad();
    }
...
{
  assert( ( (cuarma::traits::size(proxy) == size()) || (size() == 0) )
          && bool("Incompatible vector sizes!"));

  // initialize the necessary buffer
  if (size() == 0)
  {
    size_ = cuarma::traits::size(proxy);
    internal_size_ = cuarma::tools::align_to_multiple<size_type>(size_, dense_padding_size);
    cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), cuarma::traits::context(proxy));
    pad();
  }

  blas::detail::op_executor<self_type, op_assign, vector_expression<const LHS, const RHS, OP> >::apply(*this, proxy);
...
vector_base<NumericT, SizeT, DistanceT> & vector_base<NumericT, SizeT, DistanceT>:: operator = (const vector_base<OtherNumericT> & v1)
{
  ...
  if (size() == 0)
  {
    ...
    {
      internal_size_ = cuarma::tools::align_to_multiple<size_type>(size_, dense_padding_size);
      cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), cuarma::traits::context(v1));
      pad();
    }
  }

  cuarma::blas::convert(*this, v1);
...
vector_base<NumericT, SizeT, DistanceT> & vector_base<NumericT, SizeT, DistanceT>::operator = (unit_vector<NumericT> const & v)
{
  ...
  if (size() == 0)
  {
    ...
    if (size_ > 0)
    {
      cuarma::backend::memory_create(elements_, sizeof(NumericT)*internal_size(), v.context());
      clear();
    }
  }
  else
    cuarma::blas::vector_assign(*this, NumericT(0));
...
vector_base<NumericT, SizeT, DistanceT> & vector_base<NumericT, SizeT, DistanceT>::operator = (scalar_vector<NumericT> const & v)
{
  ...
  if (size_ > 0)
    cuarma::blas::vector_assign(*this, v[0]);
...
//matrix<>
template<class NumericT, typename SizeT, typename DistanceT>
vector_base<NumericT, SizeT, DistanceT> & vector_base<NumericT, SizeT, DistanceT>::operator=(const cuarma::vector_expression< const matrix_base<NumericT>, const vector_base<NumericT>, cuarma::op_prod> & proxy)
{
  assert(cuarma::traits::size1(proxy.lhs()) == size() && bool("Size check failed for v1 = A * v2: size1(A) != size(v1)"));

  // check for the special case x = A * x
  if (cuarma::traits::handle(proxy.rhs()) == cuarma::traits::handle(*this))
  {
    cuarma::vector<NumericT> result(cuarma::traits::size1(proxy.lhs()));
...
vector_base<NumericT, SizeT, DistanceT> & vector_base<NumericT, SizeT, DistanceT>::operator=(const vector_expression< const matrix_expression< const matrix_base<NumericT>, const matrix_base<NumericT>, op_trans >,
                                                                                             ...
                                                                                             op_prod> & proxy)
{
  assert(cuarma::traits::size1(proxy.lhs()) == size() && bool("Size check failed in v1 = trans(A) * v2: size2(A) != size(v1)"));
...
{
  cuarma::blas::vector_swap(*this, other);
  return *this;
}
...
{
  cuarma::blas::vector_assign(*this, cpu_value_type(0.0), true);
}
...
void vector_base<NumericT, SizeT, DistanceT>::pad()
{
  if (internal_size() != size())
  {
    ...
    cuarma::backend::memory_write(elements_, sizeof(NumericT) * size(), sizeof(NumericT) * pad.size(), &(pad[0]));
  }
}

template<class NumericT, typename SizeT, typename DistanceT>
void vector_base<NumericT, SizeT, DistanceT>::switch_memory_context(cuarma::context new_ctx)
{
...
{
  resize_impl(new_size, cuarma::traits::context(*this), preserve);
}

template<class NumericT, typename SizeT, typename DistanceT>
void vector_base<NumericT, SizeT, DistanceT>::resize(size_type new_size, cuarma::context ctx, bool preserve)
{
  resize_impl(new_size, ctx, preserve);
}

template<class NumericT, typename SizeT, typename DistanceT>
void vector_base<NumericT, SizeT, DistanceT>::resize_impl(size_type new_size, cuarma::context ctx, bool preserve)
{
  assert(new_size > 0 && bool("Positive size required when resizing vector!"));

  if (new_size != size_)
  {
    arma_size_t new_internal_size = cuarma::tools::align_to_multiple<arma_size_t>(new_size, dense_padding_size);
    ...
    {
      cuarma::backend::memory_create(elements_, sizeof(NumericT)*new_internal_size, ctx, NULL);
    }
...
public:
  ...
  */
  ...
  explicit vector(size_type vec_size, cuarma::context ctx) : base_type(vec_size, ctx) {}

  explicit vector(NumericT * ptr_to_mem, cuarma::memory_types mem_type, size_type vec_size, size_type start = 0, size_type stride = 1)
  ...
  vector(const base_type & v) : base_type(v.size(), cuarma::traits::context(v))
  {
    if (v.size() > 0)
      base_type::operator=(v);
  }

  vector(const self_type & v) : base_type(v.size(), cuarma::traits::context(v))
  {
    if (v.size() > 0)
      base_type::operator=(v);
  }
  ...
  {
    if (v.size() > 0)
      cuarma::blas::vector_assign(*this, NumericT(0.0));
  }
  ...
  */
  ...
  void resize(size_type new_size, cuarma::context ctx, bool preserve = true)
  {
...
/** @brief Tuple class holding pointers to multiple vectors. Mainly used as a temporary object returned from cuarma::tie(). */
...
public:
  ...
  {
    for (arma_size_t i=0; i<vecs.size(); ++i)
  ...
  arma_size_t size()       const { return non_const_vectors_.size(); }
  arma_size_t const_size() const { return     const_vectors_.size(); }

  VectorType       &       at(arma_size_t i) const { return *(non_const_vectors_.at(i)); }
  VectorType const & const_at(arma_size_t i) const { return     *(const_vectors_.at(i)); }
               ...
               CPU_ITERATOR cpu_begin )
{
  if (gpu_begin != gpu_end)
  {
    ...
    {
      cuarma::backend::memory_read(gpu_begin.handle(),
                                     sizeof(NumericT)*gpu_begin.offset(),
                                     sizeof(NumericT)*gpu_begin.stride() * static_cast<arma_size_t>(gpu_end - gpu_begin),
                                     &(*cpu_begin));
    }
    else
    {
      arma_size_t gpu_size = static_cast<arma_size_t>(gpu_end - gpu_begin);
      std::vector<NumericT> temp_buffer(gpu_begin.stride() * gpu_size);
      cuarma::backend::memory_read(gpu_begin.handle(), sizeof(NumericT)*gpu_begin.offset(), sizeof(NumericT)*temp_buffer.size(), &(temp_buffer[0]));

      for (arma_size_t i=0; i<gpu_size; ++i)
      {
...
/** @brief Transfer from a gpu vector to a cpu vector. Convenience wrapper for cuarma::blas::fast_copy(gpu_vec.begin(), gpu_vec.end(), cpu_vec.begin());
*
...
{
  cuarma::fast_copy(gpu_vec.begin(), gpu_vec.end(), cpu_vec.begin());
}
...
* In order to wait for the transfer to complete, use cuarma::backend::finish().
...
{
  cuarma::async_copy(gpu_vec.begin(), gpu_vec.end(), cpu_vec.begin());
}
          ...
          CPU_ITERATOR cpu_begin )
{
  ...
  {
    std::vector<NumericT> temp_buffer(static_cast<arma_size_t>(gpu_end - gpu_begin));
...
{
  cuarma::copy(const_vector_iterator<NumericT, AlignmentV>(gpu_begin),
                 const_vector_iterator<NumericT, AlignmentV>(gpu_end),
                 cpu_begin);
}

/** @brief Transfer from a gpu vector to a cpu vector. Convenience wrapper for cuarma::blas::copy(gpu_vec.begin(), gpu_vec.end(), cpu_vec.begin());
*
* @param gpu_vec    A gpu vector
...
{
  cuarma::copy(gpu_vec.begin(), gpu_vec.end(), cpu_vec.begin());
}
               ...
               vector_iterator<NumericT, AlignmentV> gpu_begin)
{
  if (cpu_end - cpu_begin > 0)
  {
    if (gpu_begin.stride() == 1)
    {
      cuarma::backend::memory_write(gpu_begin.handle(),
                                      sizeof(NumericT)*gpu_begin.offset(),
                                      sizeof(NumericT)*gpu_begin.stride() * static_cast<arma_size_t>(cpu_end - cpu_begin), &(*cpu_begin));
    }
    else //writing to slice:
    {
      arma_size_t cpu_size = static_cast<arma_size_t>(cpu_end - cpu_begin);
      std::vector<NumericT> temp_buffer(gpu_begin.stride() * cpu_size);

      cuarma::backend::memory_read(gpu_begin.handle(), sizeof(NumericT)*gpu_begin.offset(), sizeof(NumericT)*temp_buffer.size(), &(temp_buffer[0]));

      for (arma_size_t i=0; i<cpu_size; ++i)
        temp_buffer[i * gpu_begin.stride()] = (&(*cpu_begin))[i];

      cuarma::backend::memory_write(gpu_begin.handle(), sizeof(NumericT)*gpu_begin.offset(), sizeof(NumericT)*temp_buffer.size(), &(temp_buffer[0]));
    }
  }
}


/** @brief Transfer from a cpu vector to a gpu vector. Convenience wrapper for cuarma::blas::fast_copy(cpu_vec.begin(), cpu_vec.end(), gpu_vec.begin());
*
...
{
  cuarma::fast_copy(cpu_vec.begin(), cpu_vec.end(), gpu_vec.begin());
}
...
{
  cuarma::async_copy(cpu_vec.begin(), cpu_vec.end(), gpu_vec.begin());
}
          ...
          vector_iterator<NumericT, AlignmentV> gpu_begin)
{
  ...
  if (cpu_begin != cpu_end)
  {
    ...
    std::vector<NumericT> temp_buffer(static_cast<arma_size_t>(cpu_end - cpu_begin));
    std::copy(cpu_begin, cpu_end, temp_buffer.begin());
    cuarma::fast_copy(temp_buffer.begin(), temp_buffer.end(), gpu_begin);
  }
}

// for things like copy(std_vec.begin(), std_vec.end(), arma_vec.begin() + 1);

/** @brief Transfer from a host vector object to a cuarma vector proxy. Requires the vector proxy to have the necessary size. Convenience wrapper for cuarma::blas::copy(cpu_vec.begin(), cpu_vec.end(), gpu_vec.begin());
*
* @param cpu_vec    A cpu vector. Type requirements: Iterator can be obtained via member function .begin() and .end()
* @param gpu_vec    The gpu vector.
*/
...
{
  cuarma::copy(cpu_vec.begin(), cpu_vec.end(), gpu_vec.begin());
}

/** @brief Transfer from a host vector object to a cuarma vector. Resizes the cuarma vector if it has zero size. Convenience wrapper for cuarma::blas::copy(cpu_vec.begin(), cpu_vec.end(), gpu_vec.begin());
*
* @param cpu_vec    A host vector. Type requirements: Iterator can be obtained via member function .begin() and .end()
* @param gpu_vec    The gpu (cuarma) vector.
*/
...
{
  if (gpu_vec.size() == 0)
    gpu_vec.resize(static_cast<arma_size_t>(cpu_vec.end() - cpu_vec.begin()));
  cuarma::copy(cpu_vec.begin(), cpu_vec.end(), gpu_vec.begin());
}
          ...
          vector_iterator<NumericT, AlignmentV_DEST> gpu_dest_begin)
{
  ...
  assert(gpu_src_begin.stride() == 1 && bool("cuarma ERROR: copy() for GPU->GPU not implemented for slices! Use operator= instead for the moment."));

  if (gpu_src_begin.stride() == 1 && gpu_dest_begin.stride() == 1)
  {
    if (gpu_src_begin != gpu_src_end)
      cuarma::backend::memory_copy(gpu_src_begin.handle(), gpu_dest_begin.handle(),
...
void copy(vector_iterator<NumericT, AlignmentV_SRC> const & gpu_src_begin,
          ...
          vector_iterator<NumericT, AlignmentV_DEST> gpu_dest_begin)
{
  cuarma::copy(static_cast<const_vector_iterator<NumericT, AlignmentV_SRC> >(gpu_src_begin),
                 static_cast<const_vector_iterator<NumericT, AlignmentV_SRC> >(gpu_src_end),
                 gpu_dest_begin);
}

/** @brief Transfer from a cuarma vector to another cuarma vector. Convenience wrapper for cuarma::blas::copy(gpu_src_vec.begin(), gpu_src_vec.end(), gpu_dest_vec.begin());
*
...
{
  cuarma::copy(gpu_src_vec.begin(), gpu_src_vec.end(), gpu_dest_vec.begin());
}
...
std::ostream & operator<<(std::ostream & os, vector_base<T> const & val)
{
  ...
  cuarma::copy(val.begin(), val.end(), tmp.begin());
  os << "[" << val.size() << "](";
...
{
  typedef typename cuarma::result_of::cpu_value_type<typename LHS::value_type>::type ScalarType;
  cuarma::vector<ScalarType> result = proxy;
  os << result;
  return os;
}
...
{
  cuarma::blas::vector_swap(vec1, vec2);
}
...
{
  bool flip_sign = cuarma::is_flip_sign_scalar<S1>::value;
  if (v1.size() > 0)
    cuarma::blas::av(v1,
...
vector_expression< const vector_expression< LHS1, RHS1, OP1>, const vector_expression< LHS2, RHS2, OP2>, cuarma::op_add>
            ...
            vector_expression<LHS2, RHS2, OP2> const & proxy2)
{
  ...
  return   vector_expression< const vector_expression<LHS1, RHS1, OP1>, const vector_expression<LHS2, RHS2, OP2>, cuarma::op_add>(proxy1, proxy2);
}
...
vector_expression< const vector_expression<LHS, RHS, OP>, const vector_base<T>, cuarma::op_add>
operator + (vector_expression<LHS, RHS, OP> const & proxy, vector_base<T> const & vec)
{
  ...
  return vector_expression< const vector_expression<LHS, RHS, OP>, const vector_base<T>, cuarma::op_add>(proxy, vec);
}
...
vector_expression< const vector_base<T>, const vector_expression<LHS, RHS, OP>, cuarma::op_add>
operator + (vector_base<T> const & vec, vector_expression<LHS, RHS, OP> const & proxy)
{
  assert(proxy.size() == vec.size() && bool("Incompatible vector sizes!"));
  return vector_expression< const vector_base<T>, const vector_expression<LHS, RHS, OP>, cuarma::op_add>(vec, proxy);
}
...
*/
template<typename LHS1, typename RHS1, typename OP1, typename LHS2, typename RHS2, typename OP2>
vector_expression< const vector_expression< LHS1, RHS1, OP1>, const vector_expression< LHS2, RHS2, OP2>, cuarma::op_sub>
operator - (vector_expression<LHS1, RHS1, OP1> const & proxy1, vector_expression<LHS2, RHS2, OP2> const & proxy2)
{
  assert(proxy1.size() == proxy2.size() && bool("Incompatible vector sizes!"));
  return   vector_expression< const vector_expression<LHS1, RHS1, OP1>, const vector_expression<LHS2, RHS2, OP2>, cuarma::op_sub>(proxy1, proxy2);
}
...
*/
template<typename LHS, typename RHS, typename OP, typename T>
vector_expression< const vector_expression<LHS, RHS, OP>, const vector_base<T>, cuarma::op_sub>
operator - (vector_expression<LHS, RHS, OP> const & proxy, vector_base<T> const & vec)
{
  assert(proxy.size() == vec.size() && bool("Incompatible vector sizes!"));
  return vector_expression< const vector_expression<LHS, RHS, OP>, const vector_base<T>, cuarma::op_sub>(proxy, vec);
}
...
*/
template<typename T, typename LHS, typename RHS, typename OP>
vector_expression< const vector_base<T>, const vector_expression<LHS, RHS, OP>, cuarma::op_sub>
operator - (vector_base<T> const & vec, vector_expression<LHS, RHS, OP> const & proxy)
{
  assert(proxy.size() == vec.size() && bool("Incompatible vector sizes!"));
  return vector_expression< const vector_base<T>, const vector_expression<LHS, RHS, OP>, cuarma::op_sub>(vec, proxy);
}
...
/** @brief Operator overload for the expression alpha * v1, where alpha is a host scalar (float or double) and v1 is a cuarma vector.
*
* @param value   The host scalar (float or double)
* @param vec     A cuarma vector
*/
...
/** @brief Operator overload for the expression alpha * v1, where alpha is a scalar expression and v1 is a cuarma vector.
*
...
typename cuarma::enable_if< cuarma::is_any_scalar<S1>::value,
cuarma::vector_expression<const vector_expression<LHS, RHS, OP>, const S1, op_mult>  >::type
operator * (vector_expression< LHS, RHS, OP> const & proxy,
            S1 const & val)
{
  return cuarma::vector_expression<const vector_expression<LHS, RHS, OP>, const S1, op_mult>(proxy, val);
}

/** @brief Operator overload for the multiplication of a vector expression with a cuarma scalar from the left, e.g. alpha * (beta * vec1). Here, beta * vec1 is wrapped into a vector_expression and then multiplied with alpha from the left.
*
* @param val     Right hand side scalar
* @param proxy   Left hand side vector expression
*/
...
template<typename S1, typename LHS, typename RHS, typename OP>
typename cuarma::enable_if< cuarma::is_any_scalar<S1>::value,
cuarma::vector_expression<const vector_expression<LHS, RHS, OP>, const S1, op_div>  >::type
operator / (vector_expression< LHS, RHS, OP> const & proxy,
            S1 const & val)
{
  return cuarma::vector_expression<const vector_expression<LHS, RHS, OP>, const S1, op_div>(proxy, val);
}
...
namespace detail
{
  ...
  struct op_executor<vector_base<T>, op_assign, vector_base<T> >
  {
    ...
    {
      cuarma::blas::av(lhs, rhs, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const vector_base<T>, const vector_tuple<T>, op_inner_prod> >
  {
    ...
    {
      cuarma::blas::inner_prod_impl(rhs.lhs(), rhs.rhs(), lhs);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_inplace_add, vector_base<T> >
  {
    static void apply(vector_base<T> & lhs, vector_base<T> const & rhs)
    {
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, rhs, T(1), 1, false, false);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_inplace_sub, vector_base<T> >
  {
    static void apply(vector_base<T> & lhs, vector_base<T> const & rhs)
    {
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, rhs, T(1), 1, false, true);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_inplace_add, vector_expression<const vector_base<T>, const ScalarType, op_mult> >
  {
    ...
    {
      T alpha = proxy.rhs();
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, proxy.lhs(), alpha, 1, false, false);
    }

    static void apply(vector_base<T> & lhs, vector_expression<const vector_base<T>, const scalar<T>, op_mult> const & proxy)
    {
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, false, false);
    }
  ...
  struct op_executor<vector_base<T>, op_inplace_sub, vector_expression<const vector_base<T>, const ScalarType, op_mult> >
  {
    ...
    {
      T alpha = proxy.rhs();
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, proxy.lhs(), alpha, 1, false, true);
    }

    static void apply(vector_base<T> & lhs, vector_expression<const vector_base<T>, const scalar<T>, op_mult> const & proxy)
    {
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, false, true);
    }
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const vector_base<T>, const ScalarType, op_div> >
  {
    ...
    {
      cuarma::blas::av(lhs, proxy.lhs(), proxy.rhs(), 1, true, false);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_inplace_add, vector_expression<const vector_base<T>, const ScalarType, op_div> >
  {
    static void apply(vector_base<T> & lhs, vector_expression<const vector_base<T>, const ScalarType, op_div> const & proxy)
    {
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, true, false);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_inplace_sub, vector_expression<const vector_base<T>, const ScalarType, op_div> >
  {
    static void apply(vector_base<T> & lhs, vector_expression<const vector_base<T>, const ScalarType, op_div> const & proxy)
    {
      cuarma::blas::avbv(lhs, lhs, T(1), 1, false, false, proxy.lhs(), proxy.rhs(), 1, true, true);
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const LHS, const RHS, op_element_binary<OP> > >
  {
    ...
    static void apply(vector_base<T> & lhs, vector_expression<const vector_base<T>, const vector_expression<const LHS2, const RHS2, OP2>, op_element_binary<OP> > const & proxy)
    {
      ...
      cuarma::blas::element_op(lhs, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<OP> >(proxy.lhs(), temp));
    }
    ...
    {
      vector<T> temp(proxy.lhs());
      cuarma::blas::element_op(lhs, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<OP> >(temp, proxy.rhs()));
    }
                      ...
                      op_element_binary<OP> > const & proxy)
    {
      ...
      cuarma::blas::element_op(lhs, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<OP> >(temp1, temp2));
    }
  };
  ...
  struct op_executor<vector_base<T>, op_inplace_add, vector_expression<const LHS, const RHS, op_element_binary<OP> > >
  {
    ...
    static void apply(vector_base<T> & lhs, vector_expression<const vector_base<T>, const vector_expression<const LHS2, const RHS2, OP2>,  op_element_binary<OP> > const & proxy)
    {
      ...
      cuarma::blas::element_op(temp2, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<OP> >(proxy.lhs(), temp));
      lhs += temp2;
    }
    ...
    static void apply(vector_base<T> & lhs, vector_expression<const vector_expression<const LHS1, const RHS1, OP1>, const vector_base<T>, op_element_binary<OP> > const & proxy)
    {
      vector<T> temp(proxy.lhs());
      vector<T> temp2(temp.size());
      cuarma::blas::element_op(temp2, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<OP> >(temp, proxy.rhs()));
      lhs += temp2;
    }
                      ...
                      op_element_binary<OP> > const & proxy)
    {
      ...
      cuarma::blas::element_op(temp3, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<OP> >(temp1, temp2));
      lhs += temp3;
    }
  };
  ...
  struct op_executor<vector_base<T>, op_assign, vector_expression<const LHS, const RHS, op_element_unary<OP> > >
  {
    ...
    {
      vector<T> temp(proxy.rhs());
      cuarma::blas::element_op(lhs, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_unary<OP> >(temp, temp));
    }
  };
  ...
  struct op_executor<vector_base<T>, op_inplace_add, vector_expression<const LHS, const RHS, op_element_unary<OP> > >
  {
    ...
    {
      vector<T> temp(proxy.rhs());
      cuarma::blas::element_op(temp, cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_unary<OP> >(temp, temp)); // inplace operation is safe here
      lhs += temp;
    }
  };
