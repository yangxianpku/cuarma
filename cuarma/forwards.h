...
/** @file cuarma/forwards.h
 *  @encoding:UTF-8 文档编码
 *  @brief 声明、定义等
    @brief This file provides the forward declarations for the main types used within cuarma
*/

//compatibility defines:
#ifdef CUARMA_HAVE_UBLAS
...
#include "cuarma/version.hpp"

/** @brief Main namespace in cuarma. Holds all the basic types such as vector, matrix, etc. and defines operations upon them. */
namespace cuarma
{
  typedef std::size_t                                       arma_size_t;
  typedef std::ptrdiff_t                                    arma_ptrdiff_t;
 ...
 struct op_leq {};
  ...
  template<class SCALARTYPE, typename SizeType = arma_size_t, typename DistanceType = arma_ptrdiff_t>
  class vector_base;
  ...
  struct row_major
  {
    ...
    */
    static arma_size_t mem_index(arma_size_t i, arma_size_t j, arma_size_t /* num_rows */, arma_size_t num_cols)
    {
      return i * num_cols + j;
    }
  };
  ...
  // Matrix types:
  //
  static const arma_size_t dense_padding_size = 128;
  ...
  */
  ...
  // Proxies:
  //
  template<typename SizeType = arma_size_t, typename DistanceType = std::ptrdiff_t>
  class basic_range;
  ...
  /** @brief Helper struct for checking whether a type is a cuarma::scalar<> */
  template<typename T>
  struct is_scalar
  {
    enum { value = false };
  ...
  /** @brief Helper struct for checking whether a type represents a sign flip on a cuarma::scalar<> */
  template<typename T>
  ...
  /** @brief Helper struct for checking whether the provided type represents a scalar (either host, from cuarma, or a flip-sign proxy) */
  template<typename T>
  ...
  public:
    ...
    memory_exception(std::string message) : message_("cuarma: Internal memory error: " + message) {}
  ...
  {
  public:
    cuda_not_available_exception() : message_("cuarma was compiled without CUDA support, but CUDA functionality required for this operation.") {}
  ...
  namespace tools
  {
    ...
    //must be specialized for every cuarma matrix type
  ...
  namespace blas
  {
    ...
    template<class SCALARTYPE, unsigned int ALIGNMENT>
    void convolve_i(cuarma::vector<SCALARTYPE, ALIGNMENT>& input1,
                    cuarma::vector<SCALARTYPE, ALIGNMENT>& input2,
                    cuarma::vector<SCALARTYPE, ALIGNMENT>& output);

    template<typename T>
    cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<op_prod> >
    element_prod(vector_base<T> const & v1, vector_base<T> const & v2);

    template<typename T>
    cuarma::vector_expression<const vector_base<T>, const vector_base<T>, op_element_binary<op_div> >
    ...
    template<typename LHS, typename RHS, typename OP, typename T>
    void max_impl(cuarma::vector_expression<LHS, RHS, OP> const & vec,
                  scalar<T> & result);
    ...
    template<typename LHS, typename RHS, typename OP, typename S2>
    void max_cpu(cuarma::vector_expression<LHS, RHS, OP> const & vec,
                 S2 & result);
    ...
    template<typename LHS, typename RHS, typename OP, typename T>
    void min_impl(cuarma::vector_expression<LHS, RHS, OP> const & vec,
                  scalar<T> & result);
    ...
    template<typename LHS, typename RHS, typename OP, typename S2>
    void min_cpu(cuarma::vector_expression<LHS, RHS, OP> const & vec,
                 S2 & result);
    ...
    template<typename LHS, typename RHS, typename OP, typename T>
    void sum_impl(cuarma::vector_expression<LHS, RHS, OP> const & vec,
                  scalar<T> & result);
    ...
    template<typename LHS, typename RHS, typename OP, typename S2>
    void sum_cpu(cuarma::vector_expression<LHS, RHS, OP> const & vec,
                 S2 & result);
  ...
  /** @brief Holds implementation details for functionality in the main cuarma-namespace. Not intended for direct use by library users. */
  namespace detail
  {
  ...
  namespace blas
  {
    /** @brief Holds all CUDA compute kernels used by cuarma. */
    namespace cuda
    {
    ...
    /** @brief Namespace containing the OpenCL kernels. Deprecated, will be moved to cuarma::blas::opencl in future releases. */
    namespace kernels {}
  }
  ...
  /** @brief Namespace for various tools used within cuarma. */
  namespace tools
  {
