...
namespace detail
{
  ...
  NumericT const & matrix_access(MatrixT const & A, arma_size_t i, arma_size_t j)
  {
    return A(i, j);
  }

  template<typename NumericT>
  NumericT const & matrix_access(std::vector< std::vector<NumericT> > const & A, arma_size_t i, arma_size_t j)
  {
    return A[i][j];
  }
  ...
  template<typename NumericT, typename MatrixT>
  NumericT & matrix_access(MatrixT & A, arma_size_t i, arma_size_t j)
  {
    return A(i, j);
  }

  template<typename NumericT>
  NumericT & matrix_access(std::vector< std::vector<NumericT> > & A, arma_size_t i, arma_size_t j)
  {
    return A[i][j];
  }
...
public:
  ...
  typedef typename cuarma::result_of::cpu_value_type<value_type>::type    cpu_value_type;
  ...
  base_type & operator=(cuarma::matrix<OtherNumericT, F> const & B)                          { return base_type::operator=(static_cast<cuarma::matrix_base<OtherNumericT> const &>(B)); }

  template<typename OtherNumericT, typename F>
  base_type & operator=(cuarma::matrix_range<cuarma::matrix<OtherNumericT, F> > const & B) { return base_type::operator=(static_cast<cuarma::matrix_base<OtherNumericT> const &>(B)); }

  template<typename OtherNumericT, typename F>
  base_type & operator=(cuarma::matrix_slice<cuarma::matrix<OtherNumericT, F> > const & B) { return base_type::operator=(static_cast<cuarma::matrix_base<OtherNumericT> const &>(B)); }
};
...
{
  assert(    (cuarma::traits::size1(cpu_matrix) == gpu_matrix_range.size1())
          && (cuarma::traits::size2(cpu_matrix) == gpu_matrix_range.size2())
  ...
  if ( gpu_matrix_range.start2() != 0)
  {
    ...
    for (arma_size_t i=0; i < gpu_matrix_range.size1(); ++i)
    {
      for (arma_size_t j=0; j < gpu_matrix_range.size2(); ++j)
        entries[j] = detail::matrix_access<NumericT>(cpu_matrix, i, j);

      arma_size_t start_offset = (gpu_matrix_range.start1() + i) * gpu_matrix_range.internal_size2() + gpu_matrix_range.start2();
      arma_size_t num_entries = gpu_matrix_range.size2();
      cuarma::backend::memory_write(gpu_matrix_range.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*num_entries, &(entries[0]));
  ...
  else
  {
    ...
    arma_size_t start_offset = gpu_matrix_range.start1() * gpu_matrix_range.internal_size2();
    arma_size_t num_entries = gpu_matrix_range.size1() * gpu_matrix_range.internal_size2();
    cuarma::backend::memory_write(gpu_matrix_range.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*num_entries, &(entries[0]));
          ...
          matrix_range<matrix<NumericT, column_major, 1> > & gpu_matrix_range )
{
  ...
  if ( gpu_matrix_range.start1() != 0 ||  gpu_matrix_range.size1() != gpu_matrix_range.size1())
  {
    ...
    for (arma_size_t j=0; j < gpu_matrix_range.size2(); ++j)
    {
      ...
      arma_size_t start_offset = (gpu_matrix_range.start2() + j) * gpu_matrix_range.internal_size1() + gpu_matrix_range.start1();
      arma_size_t num_entries = gpu_matrix_range.size1();
      cuarma::backend::memory_write(gpu_matrix_range.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*num_entries, &(entries[0]));
      //std::cout << "Strided copy worked!" << std::endl;
    }
  }
  else
  {
    ...
    arma_size_t start_offset = gpu_matrix_range.start2() * gpu_matrix_range.internal_size1();
    arma_size_t num_entries = gpu_matrix_range.internal_size1() * gpu_matrix_range.size2();
    cuarma::backend::memory_write(gpu_matrix_range.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*num_entries, &(entries[0]));
    //std::cout << "Block copy worked!" << std::endl;
  }
          ...
          CPUMatrixT & cpu_matrix)
{
  ...
  if ( gpu_matrix_range.start2() != 0)
  {
    ...
    for (arma_size_t i=0; i < gpu_matrix_range.size1(); ++i)
    {
      arma_size_t start_offset = (gpu_matrix_range.start1() + i) * gpu_matrix_range.internal_size2() + gpu_matrix_range.start2();
      arma_size_t num_entries = gpu_matrix_range.size2();
      cuarma::backend::memory_read(gpu_matrix_range.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*num_entries, &(entries[0]));
      //std::cout << "Strided copy worked!" << std::endl;
  ...
  else
  {
    ...
    arma_size_t start_offset = gpu_matrix_range.start1() * gpu_matrix_range.internal_size2();
    cuarma::backend::memory_read(gpu_matrix_range.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*entries.size(), &(entries[0]));
    //std::cout << "Block copy worked!" << std::endl;
...
//
template<typename MatrixType>
matrix_range<MatrixType> project(MatrixType const & A, cuarma::range const & r1, cuarma::range const & r2)
{
...
template<typename MatrixType>
matrix_range<MatrixType> project(matrix_range<MatrixType> const & A, cuarma::range const & r1, cuarma::range const & r2)
{
  assert(r1.size() <= A.size1() && r2.size() <= A.size2() && bool("Size of range invalid!"));
...
{
  assert(    (cuarma::traits::size1(cpu_matrix) == gpu_matrix_slice.size1())
          && (cuarma::traits::size2(cpu_matrix) == gpu_matrix_slice.size2())
          && bool("Matrix size mismatch!"));

  if ( (gpu_matrix_slice.size1() > 0) && (gpu_matrix_slice.size1() > 0) )
  {
    arma_size_t num_entries = gpu_matrix_slice.size2() * gpu_matrix_slice.stride2(); //no. of entries per stride

    std::vector<NumericT> entries(num_entries);

    //copy each stride separately:
    for (arma_size_t i=0; i < gpu_matrix_slice.size1(); ++i)
    {
      arma_size_t start_offset = (gpu_matrix_slice.start1() + i * gpu_matrix_slice.stride1()) * gpu_matrix_slice.internal_size2() + gpu_matrix_slice.start2();
      cuarma::backend::memory_read(gpu_matrix_slice.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*num_entries, &(entries[0]));

      for (arma_size_t j=0; j < gpu_matrix_slice.size2(); ++j)
        entries[j * gpu_matrix_slice.stride2()] = detail::matrix_access<NumericT>(cpu_matrix, i, j);

      cuarma::backend::memory_write(gpu_matrix_slice.handle(), sizeof(NumericT)*start_offset, sizeof(NumericT)*num_entries, &(entries[0]));
    }
  }
}
          ...
          matrix_slice<matrix<NumericT, column_major, 1> > & gpu_matrix_slice )
{
  ...
  if ( (gpu_matrix_slice.size1() > 0) && (gpu_matrix_slice.size1() > 0) )
  {
    arma_size_t num_entries = gpu_matrix_slice.size1() * gpu_matrix_slice.stride1(); //no. of entries per stride
    ...
    for (arma_size_t j=0; j < gpu_matrix_slice.size2(); ++j)
    {
      arma_size_t start_offset = gpu_matrix_slice.start1() + (gpu_matrix_slice.start2() + j * gpu_matrix_slice.stride2()) * gpu_matrix_slice.internal_size1();
...
//
template<typename MatrixType>
matrix_slice<MatrixType> project(MatrixType const & A, cuarma::slice const & r1, cuarma::slice const & r2)
{
...
template<typename MatrixType>
matrix_slice<MatrixType> project(matrix_range<MatrixType> const & A, cuarma::slice const & r1, cuarma::slice const & r2)
{
  assert(r1.size() <= A.size1() && r2.size() <= A.size2() && bool("Size of slice invalid!"));
...
template<typename MatrixType>
matrix_slice<MatrixType> project(matrix_slice<MatrixType> const & A, cuarma::slice const & r1, cuarma::slice const & r2)
{
  assert(r1.size() <= A.size1() && r2.size() <= A.size2() && bool("Size of slice invalid!"));
