...
/** @file cuarma/scheduler/execute_matrix_prod.hpp
 *  @encoding:UTF-8 文档编码
...
namespace detail
{
                                 ...
                                 double beta)
  {
    if (A.type_family == MATRIX_TYPE_FAMILY && B.type_family == MATRIX_TYPE_FAMILY)        // C = A * B
    {
      ...
      if (A.subtype == DENSE_MATRIX_TYPE && B.subtype == DENSE_MATRIX_TYPE && result.subtype == DENSE_MATRIX_TYPE)
      {
        ...
        {
        case FLOAT_TYPE:
          cuarma::blas::prod_impl(*A.matrix_float, *B.matrix_float, *result.matrix_float, static_cast<float>(alpha), static_cast<float>(beta)); break;
        case DOUBLE_TYPE:
          cuarma::blas::prod_impl(*A.matrix_double, *B.matrix_double, *result.matrix_double, alpha, beta); break;
        default:
    ...
    else if (A.type_family == MATRIX_TYPE_FAMILY && B.type_family == COMPOSITE_OPERATION_FAMILY)        // C = A * B^T
    {
      ...
      if (A.subtype == DENSE_MATRIX_TYPE && leaf.lhs.subtype == DENSE_MATRIX_TYPE && result.subtype == DENSE_MATRIX_TYPE)
      {
        ...
        {
        case FLOAT_TYPE:
          cuarma::blas::prod_impl(*A.matrix_float,
                                      cuarma::matrix_expression< const matrix_base<float>,
        ...
        case DOUBLE_TYPE:
          cuarma::blas::prod_impl(*A.matrix_double,
                                      cuarma::matrix_expression< const matrix_base<double>,
    ...
    else if (A.type_family == COMPOSITE_OPERATION_FAMILY && B.type_family == MATRIX_TYPE_FAMILY)        // C = A^T * B
    {
      ...
      if (leaf.lhs.subtype == DENSE_MATRIX_TYPE && B.subtype == DENSE_MATRIX_TYPE && result.subtype == DENSE_MATRIX_TYPE)
      {
        ...
        {
        case FLOAT_TYPE:
          cuarma::blas::prod_impl(cuarma::matrix_expression< const matrix_base<float>,
                                      const matrix_base<float>,
                                      op_trans>(*leaf.lhs.matrix_float, *leaf.lhs.matrix_float),
                                      *B.matrix_float,
                                      *result.matrix_float, static_cast<float>(alpha), static_cast<float>(beta)); break;
        case DOUBLE_TYPE:
          cuarma::blas::prod_impl(cuarma::matrix_expression< const matrix_base<double>,
                                      const matrix_base<double>,
                                      op_trans>(*leaf.lhs.matrix_double, *leaf.lhs.matrix_double),
                                      *B.matrix_double,
                                      *result.matrix_double, alpha, beta); break;
                                 ...
                                 lhs_rhs_element const & x)
  {
    ...
    else if (A.subtype == DENSE_MATRIX_TYPE)
    {
      ...
      {
      case FLOAT_TYPE:
        cuarma::blas::prod_impl(*A.matrix_float, *x.vector_float, *result.vector_float);
        break;
      case DOUBLE_TYPE:
        cuarma::blas::prod_impl(*A.matrix_double, *x.vector_double, *result.vector_double);
        break;
      default:
        throw statement_not_supported_exception("Invalid numeric type in matrix-{matrix,vector} multiplication");
      }
    ...
    else if (A.subtype == COMPRESSED_MATRIX_TYPE)
    {
      ...
      {
      case FLOAT_TYPE:
        cuarma::blas::prod_impl(*A.compressed_matrix_float, *x.vector_float, float(1), *result.vector_float, float(0));
        break;
      case DOUBLE_TYPE:
        cuarma::blas::prod_impl(*A.compressed_matrix_double, *x.vector_double, double(1),  *result.vector_double, double(0));
        break;
      default:
        throw statement_not_supported_exception("Invalid numeric type in matrix-{matrix,vector} multiplication");
      }
    ...
    else if (A.subtype == COORDINATE_MATRIX_TYPE)
    {
      ...
      {
      case FLOAT_TYPE:
        cuarma::blas::prod_impl(*A.coordinate_matrix_float, *x.vector_float, float(1), *result.vector_float, float(0));
        break;
      case DOUBLE_TYPE:
        cuarma::blas::prod_impl(*A.coordinate_matrix_double, *x.vector_double, double(1), *result.vector_double, double(0));
        break;
      default:
        throw statement_not_supported_exception("Invalid numeric type in matrix-{matrix,vector} multiplication");
      }
    ...
    else if (A.subtype == ELL_MATRIX_TYPE)
    {
      ...
      {
      case FLOAT_TYPE:
        cuarma::blas::prod_impl(*A.ell_matrix_float, *x.vector_float, float(1), *result.vector_float, float(0));
        break;
      case DOUBLE_TYPE:
        cuarma::blas::prod_impl(*A.ell_matrix_double, *x.vector_double, double(1), *result.vector_double, double(0));
        break;
      default:
        throw statement_not_supported_exception("Invalid numeric type in matrix-{matrix,vector} multiplication");
      }
    ...
    else if (A.subtype == HYB_MATRIX_TYPE)
    {
      ...
      {
      case FLOAT_TYPE:
        cuarma::blas::prod_impl(*A.hyb_matrix_float, *x.vector_float, float(1), *result.vector_float, float(0));
        break;
      case DOUBLE_TYPE:
        cuarma::blas::prod_impl(*A.hyb_matrix_double, *x.vector_double, double(1), *result.vector_double, double(0));
        break;
      default:
        throw statement_not_supported_exception("Invalid numeric type in matrix-{matrix,vector} multiplication");
      }
