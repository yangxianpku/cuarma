...
/** @file cuarma/misc/cuthill_mckee.hpp
 *  @encoding:UTF-8 文档编码
...
namespace detail
{
                           ...
                           std::vector<IndexT> const & permutation)
  {
    ...
    for (arma_size_t i = 0; i < permutation.size(); i++)
    {
      ...
      {
        arma_size_t col_index = static_cast<arma_size_t>(it->first);
  ...
  bool comb_inc(std::vector<IndexT> & comb, arma_size_t n)
  {
    arma_size_t m;
    arma_size_t k;

    m = static_cast<arma_size_t>(comb.size());
                         ...
                         std::vector< std::vector<IndexT> > & layer_list)
  {
    ...
    //
    for (arma_size_t i=0; i<layer_list.size(); ++i)
    {
    ...
    {
      arma_size_t layer_index = layer_list.size();  //parent nodes are at layer 0
                         ...
                         int s)
  {
    ...
    nlist.push_back(s);
    inr[static_cast<arma_size_t>(s)] = true;
    l.push_back(nlist);
    ...
    for (;;)
    {
      ...
      for (std::vector<int>::iterator it  = l.back().begin();
           ...
           it++)
      {
        for (typename MatrixType::value_type::const_iterator it2  = matrix[static_cast<arma_size_t>(*it)].begin();
             it2 != matrix[static_cast<arma_size_t>(*it)].end();
             it2++)
        {
          if (it2->first == *it) continue;
          if (inr[static_cast<arma_size_t>(it2->first)]) continue;

          nlist.push_back(it2->first);
          inr[static_cast<arma_size_t>(it2->first)] = true;
        }
      }
                                             ...
                                             std::vector<IndexT> & node_list)
  {
    ...
    {
      arma_size_t node_id = static_cast<arma_size_t>(node_queue.front());
      ...
      if (!node_visited_already[node_id])
      {
        ...
        for (typename MatrixT::value_type::const_iterator it  = matrix[node_id].begin();
             ...
             it++)
        {
          arma_size_t neighbor_node_id = static_cast<arma_size_t>(it->first);
  ...
  /** @brief Runs the Cuthill-McKee algorithm on a strongly connected component of a graph
    ...
    */
  template<typename IndexT, typename ValueT>
  arma_size_t cuthill_mckee_on_strongly_connected_component(std::vector< std::map<IndexT, ValueT> > const & matrix,
                                                           std::deque<IndexT> & node_assignment_queue,
                                                           std::vector<bool>  & dof_assigned_to_node,
                                                           std::vector<IndexT> & permutation,
                                                           arma_size_t current_dof)
  {
    ...
    while (!node_assignment_queue.empty())
    {
      ...
      arma_size_t node_id = static_cast<arma_size_t>(node_assignment_queue.front());
      ...
      if (!dof_assigned_to_node[node_id])
      {
        ...
        for (arma_size_t i=0; i<num_neighbors; ++i)
...
std::vector<IndexT> reorder(std::vector< std::map<IndexT, ValueT> > const & matrix, cuthill_mckee_tag)
{
  ...
  arma_size_t current_dof = 0;  //the dof to be assigned
  ...
  while (current_dof < matrix.size()) //outer loop for each strongly connected component (there may be more than one)
  {
    ...
    //
    arma_size_t current_min_degree = matrix.size();
    arma_size_t node_with_minimum_degree = 0;
    bool found_unassigned_node = false;
    for (arma_size_t i=0; i<matrix.size(); ++i)
    {
      if (!dof_assigned_to_node[i])
      {
...
public:
  /** @brief CTOR which may take the additional parameters for the advanced algorithm.
      ...
      *
     */
  advanced_cuthill_mckee_tag(double a = 0.0, arma_size_t gmax = 1) : starting_node_param_(a), max_root_nodes_(gmax) {}
  ...
  arma_size_t max_root_nodes() const { return max_root_nodes_; }
  void max_root_nodes(arma_size_t gmax) { max_root_nodes_ = gmax; }

private:
  double starting_node_param_;
  arma_size_t max_root_nodes_;
};
                            ...
                            advanced_cuthill_mckee_tag const & tag)
{
  ...
  arma_size_t gmax = tag.max_root_nodes();
  ...
  while (current_dof < matrix.size()) // for all strongly connected components
  {
    ...
    for (typename std::vector<IndexT>::iterator it  = nodes_in_strongly_connected_component.begin();
         ...
         it++)
    {
      deg = matrix[static_cast<arma_size_t>(*it)].size();
      if (deg_min == 0 || deg < deg_min)
        deg_min = deg;
      if (deg_max == 0 || deg > deg_max)
        deg_max = deg;
    }
    deg_a = deg_min + static_cast<arma_size_t>(a * double(deg_max - deg_min));
    ...
    for (typename std::vector<IndexT>::iterator it  = nodes_in_strongly_connected_component.begin();
         ...
         it++)
    {
      if (matrix[static_cast<arma_size_t>(*it)].size() <= deg_a)
    ...
    arma_size_t current_dof_backup = current_dof;
    ...
    for (;;)
    {
      ...
      for (typename std::vector<IndexT>::iterator it = comb.begin(); it != comb.end(); it++)
        node_queue.push_back(parent_nodes[static_cast<arma_size_t>(*it)]);
