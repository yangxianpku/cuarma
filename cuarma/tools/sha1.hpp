...
namespace detail
{
  ...
  public:
    ...
    inline static uint32_t LeftRotate(uint32_t value, arma_size_t count) {
    ...
    sha1& processBytes(const void* const data, arma_size_t len) {
      const uint8_t* block = static_cast<const uint8_t*>(data);
      processBlock(block, block + len);
      return *this;
    }
    const uint32_t* getDigest(digest32_t digest) {
      arma_size_t bitCount = this->m_byteCount * 8;
      processByte(0x80);
  ...
  private:
    digest32_t m_digest;
    uint8_t m_block[64];
    arma_size_t m_blockByteIndex;
    arma_size_t m_byteCount;
  };
