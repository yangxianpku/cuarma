...
public:
    ...
    : mat_(mat), i_(static_cast<size_type>(i)), j_(static_cast<size_type>(j))
  {
    ...
    else  //i_ is valid
    {
      ...
      {
        if (i_ < mat_.size() && mat_[static_cast<arma_size_t>(i)].size() > 0 )
        {
          //TODO: Start at entry j, not at the beginning
          if (static_cast<int>(mat_[static_cast<arma_size_t>(i)].rbegin()->first) < j)
            iter2 = mat_[static_cast<arma_size_t>(i)].end();
          else
            iter2 = mat_[static_cast<arma_size_t>(i)].begin();
        }
        else if (i_ < mat_.size() && mat_[static_cast<arma_size_t>(i)].size() == 0)
          iter2 = mat_[static_cast<arma_size_t>(i)].end();
...
public:
  ...
  NumericT & operator()(arma_size_t i, arma_size_t j) { return mat_[i][static_cast<size_type>(j)]; }

  void resize(arma_size_t i, arma_size_t j, bool preserve = true)
  {
    if (i>0)
      mat_.resize(i);
    if (!preserve)
