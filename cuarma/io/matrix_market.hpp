...
long read_matrix_market_file_impl(MatrixT & mat,
                                  ...
                                  long index_base)
{
  typedef typename cuarma::result_of::cpu_value_type<typename cuarma::result_of::value_type<MatrixT>::type>::type    ScalarT;
  ...
  if (!reader){
    std::cerr << "cuarma: Matrix Market Reader: Cannot open file " << file << std::endl;
    return EXIT_FAILURE;
  }
  ...
  while (reader.good())
  {
    ...
    if (buffer[0] == '%')
    {
      if (buffer[1] == '%')
      {
        ...
        {
          std::cerr << "Error in file " << file << ": The MatrixMarket reader provided with cuarma supports only real valued floating point arithmetic or pattern type matrices." << std::endl;
          return 0;
        }
        ...
        {
          std::cerr << "Error in file " << file << ": The MatrixMarket reader provided with cuarma supports only general or symmetric matrices." << std::endl;
          return 0;
        }
    ...
    else
    {
      ...
      if (is_header)
      {
        ...
        if (rows > 0 && cols > 0)
          cuarma::traits::resize(mat, rows, cols);
      ...
      else
      {
        ...
        if (dense_format)
        {
          ScalarT value;
          line >> value;
          cuarma::traits::fill(mat, static_cast<arma_size_t>(cur_row), static_cast<arma_size_t>(cur_col), value);

          if (++cur_row == static_cast<long>(cuarma::traits::size1(mat)))
          {
            //next column
            ++cur_col;
            cur_row = 0;
        ...
        else //sparse format
        {
          ...
          if (row >= static_cast<long>(cuarma::traits::size1(mat)) || row < 0)
          {
            std::cerr << "Error in file " << file << " at line " << linenum << ": Row index out of bounds: " << row << " (matrix dim: " << cuarma::traits::size1(mat) << " x " << cuarma::traits::size2(mat) << ")" << std::endl;
            return 0;
          }

          if (col >= static_cast<long>(cuarma::traits::size2(mat)) || col < 0)
          {
            std::cerr << "Error in file " << file << " at line " << linenum << ": Column index out of bounds: " << col << " (matrix dim: " << cuarma::traits::size1(mat) << " x " << cuarma::traits::size2(mat) << ")" << std::endl;
            return 0;
          }

          cuarma::traits::fill(mat, static_cast<arma_size_t>(row), static_cast<arma_size_t>(col), value); //basically equivalent to mat(row, col) = value;
          if (symmetric)
            cuarma::traits::fill(mat, static_cast<arma_size_t>(col), static_cast<arma_size_t>(row), value); //basically equivalent to mat(col, row) = value;
...
long read_matrix_market_file(std::vector< std::map<unsigned int, ScalarT> > & mat,
                             ...
                             long index_base = 1)
{
  cuarma::tools::sparse_matrix_adapter<ScalarT> adapted_matrix(mat);
...
void write_matrix_market_file(std::vector< std::map<unsigned int, ScalarT> > const & mat,
                              ...
                              long index_base = 1)
{
  cuarma::tools::const_sparse_matrix_adapter<ScalarT> adapted_matrix(mat);
