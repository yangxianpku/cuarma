...
public:
  ...
  base_type & operator=(cuarma::vector<OtherNumericT> const & v)                          { return base_type::operator=(static_cast<cuarma::vector_base<OtherNumericT> const &>(v)); }

  template<typename OtherNumericT>
  base_type & operator=(cuarma::vector_range<cuarma::vector<OtherNumericT> > const & v) { return base_type::operator=(static_cast<cuarma::vector_base<OtherNumericT> const &>(v)); }

  template<typename OtherNumericT>
  base_type & operator=(cuarma::vector_slice<cuarma::vector<OtherNumericT> > const & v) { return base_type::operator=(static_cast<cuarma::vector_base<OtherNumericT> const &>(v)); }
};
...
void copy(const VectorType & cpu_vector, vector_range<vector<NumericT> > & gpu_vector_range )
{
  ...
  if (cpu_vector.end() - cpu_vector.begin() > 0)
  {
    ...
    std::vector<NumericT> temp_buffer(static_cast<arma_size_t>(cpu_vector.end() - cpu_vector.begin()));
    std::copy(cpu_vector.begin(), cpu_vector.end(), temp_buffer.begin());
    cuarma::backend::memory_write(gpu_vector_range.handle(), sizeof(NumericT)*gpu_vector_range.start(), sizeof(NumericT)*temp_buffer.size(), &(temp_buffer[0]));
  }
}
...
void copy(vector_range<vector<NumericT> > const & gpu_vector_range, VectorType & cpu_vector)
{
  ...
  {
    std::vector<NumericT> temp_buffer(static_cast<arma_size_t>(cpu_vector.end() - cpu_vector.begin()));
    cuarma::backend::memory_read(gpu_vector_range.handle(), sizeof(NumericT)*gpu_vector_range.start(), sizeof(NumericT)*temp_buffer.size(), &(temp_buffer[0]));
...
/** @brief Transfer from a GPU vector range to a CPU vector. Convenience wrapper for cuarma::blas::fast_copy(gpu_vec.begin(), gpu_vec.end(), cpu_vec.begin());
*
...
//
template<typename VectorType>
vector_range<VectorType> project(VectorType const & vec, cuarma::range const & r1)
{
  return vector_range<VectorType>(vec, r1);
}

template<typename VectorType>
vector_range<VectorType> project(cuarma::vector_range<VectorType> const & vec, cuarma::range const & r1)
{
...
public:
  ...
  vector_slice(VectorType const & v, slice const & entry_slice)
    : base_type(const_cast<handle_type &>(v.handle()), entry_slice.size(), v.start() + v.stride() * entry_slice.start(), entry_slice.stride() * v.stride()) {}
...
void copy(const VectorType & cpu_vector, vector_slice<vector<NumericT> > & gpu_vector_slice )
{
  if (cpu_vector.size() > 0)
  {
    ...
    cuarma::backend::memory_read(gpu_vector_slice.handle(), sizeof(NumericT)*gpu_vector_slice.start(), sizeof(NumericT)*temp_buffer.size(), &(temp_buffer[0]));

    for (arma_size_t i=0; i<cpu_vector.size(); ++i)
      temp_buffer[i * gpu_vector_slice.stride()] = cpu_vector[i];

    cuarma::backend::memory_write(gpu_vector_slice.handle(), sizeof(NumericT)*gpu_vector_slice.start(), sizeof(NumericT)*temp_buffer.size(), &(temp_buffer[0]));
  }
}
...
//
template<typename VectorType>
vector_slice<VectorType> project(VectorType const & vec, cuarma::slice const & s1)
{
...
template<typename VectorType>
vector_slice<VectorType> project(cuarma::vector_slice<VectorType> const & vec, cuarma::slice const & s1)
{
...
template<typename VectorType>
vector_slice<VectorType> project(cuarma::vector_slice<VectorType> const & vec, cuarma::range const & r1)
{
  assert(r1.size() <= vec.size() && bool("Size of slice larger than vector proxy!"));
  return vector_slice<VectorType>(vec, cuarma::slice(r1.start(), 1, r1.size()));
}

template<typename VectorType>
vector_slice<VectorType> project(cuarma::vector_range<VectorType> const & vec, cuarma::slice const & s1)
{
  assert(s1.size() <= vec.size() && bool("Size of slice larger than vector proxy!"));
  return vector_slice<VectorType>(vec, s1);
}
