...
public:
  ...
  hyb_matrix(cuarma::context ctx) : csr_threshold_(NumericT(0.8)), rows_(0), cols_(0)
  {
  ...
  void clear()
  {
    ...
    cuarma::backend::typesafe_host_array<unsigned int> host_coords_buffer(ell_coords_, internal_size1());
    std::vector<NumericT> host_elements(internal_size1());

    cuarma::backend::memory_create(ell_coords_,   host_coords_buffer.element_size() * internal_size1(), cuarma::traits::context(ell_coords_),   host_coords_buffer.get());
    cuarma::backend::memory_create(ell_elements_, sizeof(NumericT) * internal_size1(),                  cuarma::traits::context(ell_elements_), &(host_elements[0]));

    // CSR part:
    csrnnz_ = 0;

    cuarma::backend::typesafe_host_array<unsigned int> host_row_buffer(csr_rows_, rows_ + 1);
    cuarma::backend::typesafe_host_array<unsigned int> host_col_buffer(csr_cols_, 1);
    host_elements.resize(1);

    cuarma::backend::memory_create(csr_rows_,     host_row_buffer.element_size() * (rows_ + 1), cuarma::traits::context(csr_rows_),     host_row_buffer.get());
    cuarma::backend::memory_create(csr_cols_,     host_col_buffer.element_size() * 1,           cuarma::traits::context(csr_cols_),     host_col_buffer.get());
    cuarma::backend::memory_create(csr_elements_, sizeof(NumericT) * 1,                         cuarma::traits::context(csr_elements_), &(host_elements[0]));
  }
  ...
  arma_size_t internal_ellnnz() const {return cuarma::tools::align_to_multiple<arma_size_t>(ellnnz_, AlignmentV); }
  arma_size_t ell_nnz() const { return ellnnz_; }
  arma_size_t csr_nnz() const { return csrnnz_; }
...
void copy(const CPUMatrixT& cpu_matrix, hyb_matrix<NumericT, AlignmentV>& gpu_matrix )
{
  ...
  if (cpu_matrix.size1() > 0 && cpu_matrix.size2() > 0)
  {
    ...
    std::vector<arma_size_t> hist_entries(cpu_matrix.size2() + 1, 0);
    ...
    arma_size_t sum = 0;
    for (arma_size_t ind = 0; ind <= max_entries_per_row; ind++)
    {
    ...
    arma_size_t nnz = gpu_matrix.internal_size1() * gpu_matrix.internal_ellnnz();

    cuarma::backend::typesafe_host_array<unsigned int>  ell_coords(gpu_matrix.ell_coords_, nnz);
    cuarma::backend::typesafe_host_array<unsigned int>  csr_rows(gpu_matrix.csr_rows_, cpu_matrix.size1() + 1);
    std::vector<unsigned int> csr_cols;

    std::vector<NumericT> ell_elements(nnz);
    std::vector<NumericT> csr_elements;

    arma_size_t csr_index = 0;
    ...
    cuarma::backend::typesafe_host_array<unsigned int> csr_cols_for_gpu(gpu_matrix.csr_cols_, csr_cols.size());
    for (arma_size_t i=0; i<csr_cols.size(); ++i)
      csr_cols_for_gpu.set(i, csr_cols[i]);

    cuarma::backend::memory_create(gpu_matrix.ell_coords_,   ell_coords.raw_size(),                    traits::context(gpu_matrix.ell_coords_), ell_coords.get());
    cuarma::backend::memory_create(gpu_matrix.ell_elements_, sizeof(NumericT) * ell_elements.size(), traits::context(gpu_matrix.ell_elements_), &(ell_elements[0]));

    cuarma::backend::memory_create(gpu_matrix.csr_rows_,     csr_rows.raw_size(),                      traits::context(gpu_matrix.csr_rows_), csr_rows.get());
    cuarma::backend::memory_create(gpu_matrix.csr_cols_,     csr_cols_for_gpu.raw_size(),              traits::context(gpu_matrix.csr_cols_), csr_cols_for_gpu.get());
    cuarma::backend::memory_create(gpu_matrix.csr_elements_, sizeof(NumericT) * csr_elements.size(), traits::context(gpu_matrix.csr_elements_), &(csr_elements[0]));
  }
}
...
/** @brief Copies a sparse matrix from the host to the compute device. The host type is the std::vector< std::map < > > format .
  ...
  * @param gpu_matrix   The sparse hyb_matrix from cuarma
  */
...
void copy(const hyb_matrix<NumericT, AlignmentV>& gpu_matrix, CPUMatrixT& cpu_matrix)
{
  ...
  if (gpu_matrix.size1() > 0 && gpu_matrix.size2() > 0)
  {
    ...
    cuarma::backend::typesafe_host_array<unsigned int> ell_coords(gpu_matrix.handle2(), gpu_matrix.internal_size1() * gpu_matrix.internal_ellnnz());

    std::vector<NumericT> csr_elements(gpu_matrix.csr_nnz());
    cuarma::backend::typesafe_host_array<unsigned int> csr_rows(gpu_matrix.handle3(), gpu_matrix.size1() + 1);
    cuarma::backend::typesafe_host_array<unsigned int> csr_cols(gpu_matrix.handle4(), gpu_matrix.csr_nnz());

    cuarma::backend::memory_read(gpu_matrix.handle(), 0, sizeof(NumericT) * ell_elements.size(), &(ell_elements[0]));
    cuarma::backend::memory_read(gpu_matrix.handle2(), 0, ell_coords.raw_size(), ell_coords.get());
    cuarma::backend::memory_read(gpu_matrix.handle3(), 0, csr_rows.raw_size(),   csr_rows.get());
    cuarma::backend::memory_read(gpu_matrix.handle4(), 0, csr_cols.raw_size(),   csr_cols.get());
    cuarma::backend::memory_read(gpu_matrix.handle5(), 0, sizeof(NumericT) * csr_elements.size(), &(csr_elements[0]));


    for (arma_size_t row = 0; row < gpu_matrix.size1(); row++)
    {
      for (arma_size_t ind = 0; ind < gpu_matrix.internal_ellnnz(); ind++)
      {
        ...
        {
          std::cerr << "cuarma encountered invalid data " << offset << " " << ind << " " << row << " " << ell_coords[offset] << " " << gpu_matrix.size2() << std::endl;
          return;
        }
      ...
      for (arma_size_t ind = csr_rows[row]; ind < csr_rows[row+1]; ind++)
      {
        ...
        {
          std::cerr << "cuarma encountered invalid data " << std::endl;
          return;
        }
