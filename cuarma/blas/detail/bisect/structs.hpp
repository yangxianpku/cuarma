...
/** @file cuarma/blas/detail//bisect/structs.hpp
 *  @encoding:UTF-8 文档编码
...
struct InputData
{
  ...
  cuarma::vector<NumericT> g_a;
  //!device side representation of superdiagonal
  cuarma::vector<NumericT> g_b;
              ...
              std_a(sz), std_b(sz), g_a(sz), g_b(sz)
  {
   ...
   cuarma::copy(std_b, g_b);
   cuarma::copy(std_a, g_a);
  }

  InputData(cuarma::vector<NumericT> diagonal, cuarma::vector<NumericT> superdiagonal, const unsigned int sz) :
              std_a(sz), std_b(sz), g_a(sz), g_b(sz)
  {
   g_a = diagonal;
   g_b = superdiagonal;

   cuarma::copy(g_a, std_a);
   cuarma::copy(g_b, std_b);
  }
};
...
struct ResultDataSmall
{
  ...
  ResultDataSmall(const unsigned int mat_size) :
    std_eigenvalues(mat_size), arma_g_left(mat_size), arma_g_right(mat_size), arma_g_left_count(mat_size), arma_g_right_count(mat_size) {}
};
...
//! eigenvalues
  ...
  cuarma::vector<unsigned int> g_blocks_mult_sum;
