...
/** @file cuarma/blas/host_based/ilu_operations.hpp
 *  @encoding:UTF-8 文档编码
    @brief Implementations of specialized routines for the Chow-Patel parallel ILU preconditioner using the host
*/
               ...
               compressed_matrix<NumericT>       & L)
{
  ...
  //
  cuarma::vector_base<unsigned int> wrapped_L_row_buffer(L.handle1(), L.size1() + 1, 0, 1);
  cuarma::blas::exclusive_scan(wrapped_L_row_buffer);
                ...
                compressed_matrix<NumericT>       & U)
{
  ...
  cuarma::vector_base<unsigned int> wrapped_U_row_buffer(U.handle1(), U.size1() + 1, 0, 1);
  cuarma::blas::exclusive_scan(wrapped_U_row_buffer);
                   ...
                   compressed_matrix<NumericT>       & B)
{
  ...
  B = compressed_matrix<NumericT>(A.size2(), A.size1(), A.nnz(), cuarma::traits::context(A));

  NumericT     * B_elements   = cuarma::blas::host_based::detail::extract_raw_pointer<NumericT>(B.handle());
  unsigned int * B_row_buffer = cuarma::blas::host_based::detail::extract_raw_pointer<unsigned int>(B.handle1());
  unsigned int * B_col_buffer = cuarma::blas::host_based::detail::extract_raw_pointer<unsigned int>(B.handle2());
