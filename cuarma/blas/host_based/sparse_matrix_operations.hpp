...
#include "cuarma/blas/host_based/spgemm_vector.hpp"
#include <vector>
...
namespace detail
{
                ...
                cuarma::blas::detail::row_info_types info_selector)
  {
    ...
    for (arma_size_t row = 0; row < mat.size1(); ++row)
    {
      NumericT value = 0;
      ...
      {
        case cuarma::blas::detail::SPARSE_ROW_NORM_INF: //inf-norm
          for (unsigned int i = row_buffer[row]; i < row_end; ++i)
            value = std::max<NumericT>(value, std::fabs(elements[i]));
          break;

        case cuarma::blas::detail::SPARSE_ROW_NORM_1: //1-norm
          for (unsigned int i = row_buffer[row]; i < row_end; ++i)
            value += std::fabs(elements[i]);
          break;

        case cuarma::blas::detail::SPARSE_ROW_NORM_2: //2-norm
          for (unsigned int i = row_buffer[row]; i < row_end; ++i)
            value += elements[i] * elements[i];
          value = std::sqrt(value);
          break;

        case cuarma::blas::detail::SPARSE_ROW_DIAGONAL: //diagonal entry
          for (unsigned int i = row_buffer[row]; i < row_end; ++i)
          {
            if (col_buffer[i] == row)
            {
               ...
               NumericT beta)
{
  ...
  for (long row = 0; row < static_cast<long>(mat.size1()); ++row)
  {
    ...
    for (arma_size_t i = row_buffer[row]; i < row_end; ++i)
      dot_prod += elements[i] * vec_buf[col_buffer[i] * vec.stride() + vec.start()];

    if (beta < 0 || beta > 0)
    {
      arma_size_t index = static_cast<arma_size_t>(row) * result.stride() + result.start();
      result_buf[index] = alpha * dot_prod + beta * result_buf[index];
    }
    else
      result_buf[static_cast<arma_size_t>(row) * result.stride() + result.start()] = alpha * dot_prod;
  }
...
void prod_impl(const cuarma::compressed_matrix<NumericT, AlignmentV> & sp_mat,
  ...
  arma_size_t d_mat_start1 = cuarma::traits::start1(d_mat);
  arma_size_t d_mat_start2 = cuarma::traits::start2(d_mat);
  arma_size_t d_mat_inc1   = cuarma::traits::stride1(d_mat);
  arma_size_t d_mat_inc2   = cuarma::traits::stride2(d_mat);
  arma_size_t d_mat_internal_size1  = cuarma::traits::internal_size1(d_mat);
  arma_size_t d_mat_internal_size2  = cuarma::traits::internal_size2(d_mat);

  arma_size_t result_start1 = cuarma::traits::start1(result);
  arma_size_t result_start2 = cuarma::traits::start2(result);
  arma_size_t result_inc1   = cuarma::traits::stride1(result);
  arma_size_t result_inc2   = cuarma::traits::stride2(result);
  arma_size_t result_internal_size1  = cuarma::traits::internal_size1(result);
  arma_size_t result_internal_size2  = cuarma::traits::internal_size2(result);
  ...
  if ( d_mat.row_major() ) {
    ...
    for (long row = 0; row < static_cast<long>(sp_mat.size1()); ++row) {
      arma_size_t row_start = sp_mat_row_buffer[row];
      arma_size_t row_end = sp_mat_row_buffer[row+1];
      for (arma_size_t col = 0; col < d_mat.size2(); ++col) {
        NumericT temp = 0;
        for (arma_size_t k = row_start; k < row_end; ++k) {
          temp += sp_mat_elements[k] * d_mat_wrapper_row(static_cast<arma_size_t>(sp_mat_col_buffer[k]), col);
        }
        if (result.row_major())
    ...
    for (long col = 0; col < static_cast<long>(d_mat.size2()); ++col) {
      for (long row = 0; row < static_cast<long>(sp_mat.size1()); ++row) {
        ...
        NumericT temp = 0;
        for (arma_size_t k = row_start; k < row_end; ++k) {
          temp += sp_mat_elements[k] * d_mat_wrapper_col(static_cast<arma_size_t>(sp_mat_col_buffer[k]), static_cast<arma_size_t>(col));
        }
        if (result.row_major())
          result_wrapper_row(row, col) = temp;
        else
...
void prod_impl(const cuarma::compressed_matrix<NumericT, AlignmentV> & sp_mat,
  ...
  arma_size_t d_mat_start1 = cuarma::traits::start1(d_mat.lhs());
  arma_size_t d_mat_start2 = cuarma::traits::start2(d_mat.lhs());
  arma_size_t d_mat_inc1   = cuarma::traits::stride1(d_mat.lhs());
  arma_size_t d_mat_inc2   = cuarma::traits::stride2(d_mat.lhs());
  arma_size_t d_mat_internal_size1  = cuarma::traits::internal_size1(d_mat.lhs());
  arma_size_t d_mat_internal_size2  = cuarma::traits::internal_size2(d_mat.lhs());
  ...
  if ( d_mat.lhs().row_major() ) {
    ...
    for (long row = 0; row < static_cast<long>(sp_mat.size1()); ++row) {
      ...
      for (arma_size_t col = 0; col < d_mat.size2(); ++col) {
        NumericT temp = 0;
        for (arma_size_t k = row_start; k < row_end; ++k) {
          temp += sp_mat_elements[k] * d_mat_wrapper_row(col, static_cast<arma_size_t>(sp_mat_col_buffer[k]));
        }
        if (result.row_major())
          result_wrapper_row(row, col) = temp;
        else
    ...
    for (long col = 0; col < static_cast<long>(d_mat.size2()); ++col) {
      for (arma_size_t row = 0; row < sp_mat.size1(); ++row) {
        arma_size_t row_start = sp_mat_row_buffer[row];
        arma_size_t row_end = sp_mat_row_buffer[row+1];
        NumericT temp = 0;
        for (arma_size_t k = row_start; k < row_end; ++k) {
          temp += sp_mat_elements[k] * d_mat_wrapper_col(col, static_cast<arma_size_t>(sp_mat_col_buffer[k]));
        }
        if (result.row_major())
          result_wrapper_row(row, col) = temp;
        else
...
namespace detail
{
  ...
  void csr_inplace_solve(IndexArrayT const & row_buffer,
                         ...
                         arma_size_t num_cols,
                         cuarma::blas::unit_lower_tag)
  {
    arma_size_t row_begin = row_buffer[1];
    for (arma_size_t row = 1; row < num_cols; ++row)
    {
      NumericT vec_entry = vec_buffer[row];
      arma_size_t row_end = row_buffer[row+1];
      for (arma_size_t i = row_begin; i < row_end; ++i)
      {
        arma_size_t col_index = col_buffer[i];
        if (col_index < row)
  ...
  {
    arma_size_t row_begin = row_buffer[0];
    for (arma_size_t row = 0; row < num_cols; ++row)
    {
      NumericT vec_entry = vec_buffer[row];
  ...
  {
    for (arma_size_t row2 = 1; row2 < num_cols; ++row2)
    {
      arma_size_t row = (num_cols - row2) - 1;
      NumericT vec_entry = vec_buffer[row];
  ...
  {
    for (arma_size_t row2 = 0; row2 < num_cols; ++row2)
    {
      arma_size_t row = (num_cols - row2) - 1;
      NumericT vec_entry = vec_buffer[row];
...
void inplace_solve(compressed_matrix<NumericT, AlignmentV> const & L,
                   vector_base<NumericT> & vec,
                   cuarma::blas::unit_lower_tag tag)
{
...
void inplace_solve(compressed_matrix<NumericT, AlignmentV> const & U,
                   vector_base<NumericT> & vec,
                   cuarma::blas::unit_upper_tag tag)
{
  NumericT           * vec_buf    = detail::extract_raw_pointer<NumericT>(vec.handle());
...
namespace detail
{
  ...
  void csr_trans_inplace_solve(IndexArrayT const & row_buffer,
                               ...
                               cuarma::blas::unit_lower_tag)
  {
    arma_size_t col_begin = row_buffer[0];
    for (arma_size_t col = 0; col < num_cols; ++col)
    {
      NumericT vec_entry = vec_buffer[col];
      arma_size_t col_end = row_buffer[col+1];
      for (arma_size_t i = col_begin; i < col_end; ++i)
      {
                               ...
                               cuarma::blas::lower_tag)
  {
    ...
    for (arma_size_t col = 0; col < num_cols; ++col)
    {
      ...
      for (arma_size_t i = col_begin; i < col_end; ++i)
      {
        arma_size_t row_index = col_buffer[i];
        if (row_index == col)
        {
          diagonal_entry = element_buffer[i];
          break;
  ...
  void csr_trans_inplace_solve(IndexArrayT const & row_buffer,
                               ...
                               cuarma::blas::unit_upper_tag)
  {
    for (arma_size_t col2 = 0; col2 < num_cols; ++col2)
    {
      arma_size_t col = (num_cols - col2) - 1;

      NumericT vec_entry = vec_buffer[col];
      arma_size_t col_begin = row_buffer[col];
      arma_size_t col_end = row_buffer[col+1];
      for (arma_size_t i = col_begin; i < col_end; ++i)
      {
        arma_size_t row_index = col_buffer[i];
                           ...
                           cuarma::blas::unit_lower_tag)
  {
    ...
    unsigned int const * row_buffer = cuarma::blas::host_based::detail::extract_raw_pointer<unsigned int>(L.lhs().handle1());
    unsigned int const * col_buffer = cuarma::blas::host_based::detail::extract_raw_pointer<unsigned int>(L.lhs().handle2());
    NumericT     const * elements   = cuarma::blas::host_based::detail::extract_raw_pointer<NumericT>(L.lhs().handle());
    NumericT           * vec_buffer = detail::extract_raw_pointer<NumericT>(vec.handle());

    arma_size_t col_begin = row_buffer[0];
    for (arma_size_t col = 0; col < L.lhs().size1(); ++col)
    {
      NumericT vec_entry = vec_buffer[col];
      arma_size_t col_end = row_buffer[col+1];
      for (arma_size_t i = col_begin; i < col_end; ++i)
                           ...
                           cuarma::blas::unit_upper_tag)
  {
    ...
    unsigned int const * row_buffer = cuarma::blas::host_based::detail::extract_raw_pointer<unsigned int>(U.lhs().handle1());
    unsigned int const * col_buffer = cuarma::blas::host_based::detail::extract_raw_pointer<unsigned int>(U.lhs().handle2());
    NumericT     const * elements   = cuarma::blas::host_based::detail::extract_raw_pointer<NumericT>(U.lhs().handle());
    NumericT           * vec_buffer = detail::extract_raw_pointer<NumericT>(vec.handle());

    for (arma_size_t col2 = 0; col2 < U.lhs().size1(); ++col2)
    {
      arma_size_t col = (U.lhs().size1() - col2) - 1;
               ...
               NumericT beta)
{
  ...
  if (beta < 0 || beta > 0)
  {
    for (arma_size_t i = 0; i< result.size(); ++i)
  ...
  {
    NumericT dot_prod = 0;
    arma_size_t row_end = row_buffer[i+1];
    for (arma_size_t j = row_buffer[i]; j < row_end; ++j)
      dot_prod += elements[j] * vec_buf[col_buffer[j] * vec.stride() + vec.start()];

    if (beta > 0 || beta < 0)
      result_buf[arma_size_t(row_indices[i]) * result.stride() + result.start()] += alpha * dot_prod;
    else
      result_buf[arma_size_t(row_indices[i]) * result.stride() + result.start()]  = alpha * dot_prod;
  }
...
namespace detail
{
                ...
                cuarma::blas::detail::row_info_types info_selector)
  {
    ...
    for (arma_size_t i = 0; i < mat.nnz(); ++i)
    {
      unsigned int current_row = coord_buffer[2*i];

      if (current_row != last_row)
      {
        if (info_selector == cuarma::blas::detail::SPARSE_ROW_NORM_2)
          value = std::sqrt(value);
...
void prod_impl(const cuarma::coordinate_matrix<NumericT, AlignmentV> & sp_mat,
  ...
  if ( d_mat.row_major() ) {
    ...
    for (long i = 0; i < static_cast<long>(sp_mat.nnz()); ++i) {
      ...
      arma_size_t r = static_cast<arma_size_t>(sp_mat_coords[2*i]);
      arma_size_t c = static_cast<arma_size_t>(sp_mat_coords[2*i+1]);
      for (arma_size_t col = 0; col < d_mat.size2(); ++col) {
    ...
    for (long col = 0; col < static_cast<long>(d_mat.size2()); ++col) {
      ...
      for (arma_size_t i = 0; i < sp_mat.nnz(); ++i) {
               ...
               NumericT beta)
{
  ...
  for (arma_size_t row = 0; row < mat.size1(); ++row)
  {
    ...
    {
      arma_size_t offset = row + item_id * mat.internal_size1();
    ...
    if (beta < 0 || beta > 0)
    {
      arma_size_t index = row * result.stride() + result.start();
                     ...
                     cuarma::matrix_base<NumericT> & result)
{
  ...
  if ( d_mat.row_major() ) {
    ...
    for (long row = 0; row < static_cast<long>(sp_mat.size1()); ++row)
    {
      ...
      {
        arma_size_t offset = static_cast<arma_size_t>(row) + static_cast<arma_size_t>(item_id) * sp_mat.internal_size1();
        NumericT sp_mat_val = static_cast<NumericT>(sp_mat_elements[offset]);
        arma_size_t sp_mat_col = static_cast<arma_size_t>(sp_mat_coords[offset]);

        if (sp_mat_val < 0 || sp_mat_val > 0) // sp_mat_val != 0 without compiler warnings
        {
          if (result.row_major())
            for (arma_size_t col = 0; col < d_mat.size2(); ++col)
              result_wrapper_row(static_cast<arma_size_t>(row), col) += sp_mat_val * d_mat_wrapper_row( sp_mat_col, col);
          else
            for (arma_size_t col = 0; col < d_mat.size2(); ++col)
              result_wrapper_col(static_cast<arma_size_t>(row), col) += sp_mat_val * d_mat_wrapper_row( sp_mat_col, col);
        }
      }
    }
  }
    ...
    for (long col = 0; col < static_cast<long>(d_mat.size2()); ++col) {
      ...
      for (unsigned int item_id = 0; item_id < sp_mat.maxnnz(); ++item_id) {
        ...
        for (arma_size_t row = 0; row < sp_mat.size1(); ++row) {
          ...
          arma_size_t offset = row + item_id * sp_mat.internal_size1();
          NumericT sp_mat_val = static_cast<NumericT>(sp_mat_elements[offset]);
          arma_size_t sp_mat_col = static_cast<arma_size_t>(sp_mat_coords[offset]);
  ...
  else
  {
    ...
    for (arma_size_t row = 0; row < sp_mat.size1(); ++row) {
      ...
      for (long item_id = 0; item_id < static_cast<long>(sp_mat.maxnnz()); ++item_id) {
        ...
        arma_size_t offset = row + static_cast<arma_size_t>(item_id) * sp_mat.internal_size1();
        NumericT sp_mat_val = static_cast<NumericT>(sp_mat_elements[offset]);
        arma_size_t sp_mat_col = static_cast<arma_size_t>(sp_mat_coords[offset]);
               ...
               NumericT beta)
{
  ...
  arma_size_t num_blocks = mat.size1() / mat.rows_per_block() + 1;


  for (long block_idx2 = 0; block_idx2 < static_cast<long>(num_blocks); ++block_idx2)
  {
    arma_size_t block_idx = static_cast<arma_size_t>(block_idx2);
    arma_size_t current_columns_per_block = columns_per_block[block_idx];
    ...
    for (IndexT column_entry_index = 0;
              ...
              ++column_entry_index)
    {
      arma_size_t stride_start = block_start[block_idx] + column_entry_index * mat.rows_per_block();
    ...
    arma_size_t first_row_in_matrix = block_idx * mat.rows_per_block();
    if (beta < 0 || beta > 0)
    {
      for (IndexT row_in_block = 0; row_in_block < mat.rows_per_block(); ++row_in_block)
      {
        ...
        {
          arma_size_t index = (first_row_in_matrix + row_in_block) * result.stride() + result.start();
               ...
               NumericT beta)
{
  ...
  for (arma_size_t row = 0; row < mat.size1(); ++row)
  {
    ...
    //
    arma_size_t col_begin = csr_row_buffer[row];
    arma_size_t col_end   = csr_row_buffer[row + 1];

    for (arma_size_t item_id = col_begin; item_id < col_end; item_id++)
    {
                     ...
                     cuarma::matrix_base<NumericT> & result)
{
  ...
  for (arma_size_t result_col = 0; result_col < result.size2(); ++result_col)
  {
    for (arma_size_t row = 0; row < mat.size1(); ++row)
    {
      NumericT sum = 0;
      ...
      for (unsigned int item_id = 0; item_id < mat.internal_ellnnz(); ++item_id)
      {
        ...
        {
          arma_size_t col = static_cast<arma_size_t>(coords[offset]);
          if (d_mat.row_major())
      ...
      if (d_mat.row_major())
        for (arma_size_t item_id = col_begin; item_id < col_end; item_id++)
          sum += d_mat_wrapper_row(static_cast<arma_size_t>(csr_col_buffer[item_id]), result_col) * csr_elements[item_id];
      else
        for (arma_size_t item_id = col_begin; item_id < col_end; item_id++)
          sum += d_mat_wrapper_col(static_cast<arma_size_t>(csr_col_buffer[item_id]), result_col) * csr_elements[item_id];
                     ...
                     cuarma::matrix_base<NumericT> & result)
{
  ...
  for (arma_size_t result_col = 0; result_col < result.size2(); ++result_col)
  {
    for (arma_size_t row = 0; row < mat.size1(); ++row)
    {
      ...
      if (d_mat.lhs().row_major())
        for (arma_size_t item_id = col_begin; item_id < col_end; item_id++)
          sum += d_mat_wrapper_row(result_col, static_cast<arma_size_t>(csr_col_buffer[item_id])) * csr_elements[item_id];
      else
        for (arma_size_t item_id = col_begin; item_id < col_end; item_id++)
          sum += d_mat_wrapper_col(result_col, static_cast<arma_size_t>(csr_col_buffer[item_id])) * csr_elements[item_id];
