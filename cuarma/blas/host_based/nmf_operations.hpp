 ...
 *  @encoding:UTF-8 文档编码
 @brief Implementations of NMF operations using a plain single-threaded execution on CPU
 */
...
public:
  nmf_config(double val_epsilon = 1e-4, double val_epsilon_stagnation = 1e-5,
      arma_size_t num_max_iters = 10000, arma_size_t num_check_iters = 100) :
  ...
  friend void nmf(cuarma::matrix_base<ScalarType> const & V,
      cuarma::matrix_base<ScalarType> & W, cuarma::matrix_base<ScalarType> & H,
      nmf_config const & conf);

private:
  double eps_;
  double stagnation_eps_;
  arma_size_t max_iters_;
  arma_size_t check_after_steps_;
  bool print_relative_error_;
public:
  mutable arma_size_t iters_;
};
...
namespace host_based
{
  ...
  void el_wise_mul_div(NumericT       * matrix1,
                       ...
                       NumericT const * matrix3, arma_size_t size)
  {
           ...
           cuarma::blas::nmf_config const & conf)
  {
    ...
    if (cuarma::blas::norm_frobenius(W) <= 0)
      W = cuarma::scalar_matrix<NumericT>(W.size1(), W.size2(), NumericT(1.0));

    if (cuarma::blas::norm_frobenius(H) <= 0)
    ...
    for (arma_size_t i = 0; i < conf.max_iterations(); i++)
    {
      ...
      cuarma::blas::host_based::el_wise_mul_div(data_H, data_hn, data_hd, H.internal_size1() * H.internal_size2());
      ...
      cuarma::blas::host_based::el_wise_mul_div(data_W, data_wn, data_wd, W.internal_size1() * W.internal_size2());
