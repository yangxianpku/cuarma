...
/** @file cuarma/blas/host_based/iterative_operations.hpp
 *  @encoding:UTF-8 文档编码
    @brief Implementations of specialized kernels for fast iterative solvers on the CPU
*/
...
namespace detail
{
                           ...
                           arma_size_t buffer_chunk_offset)
  {
    ...
    value_type         * Ap_buf      = detail::extract_raw_pointer<value_type>(Ap.handle()) + cuarma::traits::start(Ap);
    value_type   const *  p_buf      = detail::extract_raw_pointer<value_type>(p.handle()) + cuarma::traits::start(p);
    ...
    for (long row = 0; row < static_cast<long>(A.size1()); ++row)
    {
      ...
      value_type val_p_diag = p_buf[static_cast<arma_size_t>(row)]; //likely to be loaded from cache if required again in this row
      ...
      Ap_buf[static_cast<arma_size_t>(row)] = dot_prod;
      inner_prod_ApAp += dot_prod * dot_prod;
      inner_prod_pAp  += val_p_diag * dot_prod;
      inner_prod_Ap_r0star += r0star ? dot_prod * r0star[static_cast<arma_size_t>(row)] : value_type(0);
    }
                           ...
                           arma_size_t buffer_chunk_offset)
  {
    ...
    for (arma_size_t i = 0; i< Ap.size(); ++i)
      Ap_buf[i] = 0;

    // matrix-vector product with a general COO format
    for (arma_size_t i = 0; i < A.nnz(); ++i)
                           ...
                           arma_size_t buffer_chunk_offset)
  {
    ...
    value_type inner_prod_pAp = 0;
    value_type inner_prod_Ap_r0star = 0;
    for (arma_size_t row = 0; row < A.size1(); ++row)
    {
      value_type sum = 0;
      value_type val_p_diag = p_buf[static_cast<arma_size_t>(row)]; //likely to be loaded from cache if required again in this row
      ...
      {
        arma_size_t offset = row + item_id * A.internal_size1();
                           ...
                           arma_size_t buffer_chunk_offset)
  {
    ...
    arma_size_t num_blocks = A.size1() / A.rows_per_block() + 1;
    std::vector<value_type> result_values(A.rows_per_block());

    value_type inner_prod_ApAp = 0;
    value_type inner_prod_pAp = 0;
    value_type inner_prod_Ap_r0star = 0;
    for (arma_size_t block_idx = 0; block_idx < num_blocks; ++block_idx)
    {
      arma_size_t current_columns_per_block = columns_per_block[block_idx];

      for (arma_size_t i=0; i<result_values.size(); ++i)
        result_values[i] = 0;
      ...
      for (IndexT column_entry_index = 0;
                ...
                ++column_entry_index)
      {
        arma_size_t stride_start = block_start[block_idx] + column_entry_index * A.rows_per_block();
      ...
      arma_size_t first_row_in_matrix = block_idx * A.rows_per_block();
      for (IndexT row_in_block = 0; row_in_block < A.rows_per_block(); ++row_in_block)
      {
        arma_size_t row = first_row_in_matrix + row_in_block;
        if (row < Ap.size())
        {
                                ...
                                vector_base<NumericT> & inner_prod_buffer)
{
  ...
  arma_size_t size  = cuarma::traits::size(result);

  value_type inner_prod_r = 0;
  for (long i = 0; i < static_cast<long>(size); ++i)
  {
    value_type value_p = data_p[static_cast<arma_size_t>(i)];
    value_type value_r = data_r[static_cast<arma_size_t>(i)];


    data_result[static_cast<arma_size_t>(i)] += alpha * value_p;
    value_r -= alpha * data_Ap[static_cast<arma_size_t>(i)];
    value_p  = value_r + beta * value_p;
    inner_prod_r += value_r * value_r;

    data_p[static_cast<arma_size_t>(i)] = value_p;
    data_r[static_cast<arma_size_t>(i)] = value_r;
  }
                       ...
                       vector_base<NumericT> & inner_prod_buffer)
{
  ...
  cuarma::blas::host_based::detail::pipelined_prod_impl(A, p, Ap, PtrType(NULL), inner_prod_buffer, inner_prod_buffer.size() / 3, 0);
}
                                 ...
                                 arma_size_t buffer_chunk_offset)
{
  ...
  // Note: Due to the special setting in CG, there is no need to check for sizes and strides
  arma_size_t size  = cuarma::traits::size(s);

  // part 1: compute alpha:
  value_type r_in_r0 = 0;
  value_type Ap_in_r0 = 0;
  for (arma_size_t i=0; i<buffer_chunk_size; ++i)
  {
  ...
  for (long i = 0; i < static_cast<long>(size); ++i)
  {
    value_type value_s  = data_s[static_cast<arma_size_t>(i)];

    value_s = data_r[static_cast<arma_size_t>(i)] - alpha * data_Ap[static_cast<arma_size_t>(i)];
    inner_prod_s += value_s * value_s;

    data_s[static_cast<arma_size_t>(i)] = value_s;
  }
...
/** @brief Performs a joint vector update operation needed for an efficient pipelined BiCGStab algorithm.
                                       ...
                                       arma_size_t buffer_chunk_size)
 {
   ...
   for (long i = 0; i < static_cast<long>(size); ++i)
   {
     arma_size_t index = static_cast<arma_size_t>(i);
                              ...
                              arma_size_t buffer_chunk_offset)
 {
   ...
   cuarma::blas::host_based::detail::pipelined_prod_impl(A, p, Ap, data_r0star, inner_prod_buffer, buffer_chunk_size, buffer_chunk_offset);
 }
                                 ...
                                 arma_size_t buffer_chunk_offset)
{
  ...
  arma_size_t size     = cuarma::traits::size(v_k);
  arma_size_t vk_start = cuarma::traits::start(v_k);
  ...
  for (long i = 0; i < static_cast<long>(size); ++i)
  {
    value_type value_vk = data_v_k[static_cast<arma_size_t>(i) + vk_start] / norm_vk;

    inner_prod_r_dot_vk += data_residual[static_cast<arma_size_t>(i)] * value_vk;

    data_v_k[static_cast<arma_size_t>(i) + vk_start] = value_vk;
  }
                                        ...
                                        arma_size_t buffer_chunk_size)
{
  ...
  // reset buffer:
  for (arma_size_t j = 0; j < k; ++j)
    data_inner_prod[j*buffer_chunk_size] = value_type(0);

  // compute inner products:
  for (arma_size_t i = 0; i < v_k_size; ++i)
  {
    value_type value_vk = data_krylov_basis[static_cast<arma_size_t>(i) + k * v_k_internal_size];

    for (arma_size_t j = 0; j < k; ++j)
      data_inner_prod[j*buffer_chunk_size] += data_krylov_basis[static_cast<arma_size_t>(i) + j * v_k_internal_size] * value_vk;
  }
}
                                        ...
                                        arma_size_t buffer_chunk_size)
{
  ...
  for (std::size_t i=0; i<k; ++i)
    for (arma_size_t j=0; j<buffer_chunk_size; ++j)
  ...
  for (arma_size_t i = 0; i < v_k_size; ++i)
  {
    ...
    for (arma_size_t j = 0; j < k; ++j)
      value_vk -= values_vi_in_vk[j] * data_krylov_basis[static_cast<arma_size_t>(i) + j * v_k_internal_size];

    norm_vk += value_vk * value_vk;
    data_krylov_basis[static_cast<arma_size_t>(i) + k * v_k_internal_size] = value_vk;
  }
