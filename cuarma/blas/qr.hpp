...
namespace cuarma
{
  namespace blas
  {
    namespace detail
    {
      ...
      typename MatrixType::value_type setup_householder_vector_ublas(MatrixType const & A, VectorType & v, MatrixType & matrix_1x1, arma_size_t j)
      {
      ...
      template<typename MatrixType, typename VectorType>
      typename cuarma::result_of::cpu_value_type< typename MatrixType::value_type >::type
      setup_householder_vector_cuarma(MatrixType const & A, VectorType & v, MatrixType & matrix_1x1, arma_size_t j)
      {
        using cuarma::range;
        using cuarma::project;

        typedef typename cuarma::result_of::cpu_value_type< typename MatrixType::value_type >::type   ScalarType;

        //compute norm of column below diagonal:
        matrix_1x1 = cuarma::blas::prod( trans(project(A, range(j+1, A.size1()), range(j, j+1))), project(A, range(j+1, A.size1()), range(j, j+1)) );
        ScalarType sigma = matrix_1x1(0,0);
        ScalarType beta = 0;
        ScalarType A_jj = A(j,j);
      ...
      void householder_reflect(MatrixType & A, VectorType & v, ScalarType beta, arma_size_t j, arma_size_t k)
      {
        ScalarType v_in_col = A(j,k);
        for (arma_size_t i=j+1; i<A.size1(); ++i)
          v_in_col += v[i] * A(i,k);

        //assert(v[j] == 1.0);

        for (arma_size_t i=j; i<A.size1(); ++i)
          A(i,k) -= beta * v_in_col * v[i];
      }

#ifdef CUARMA_WITH_UBLAS
      template<typename MatrixType, typename VectorType, typename ScalarType>
      void householder_reflect_ublas(MatrixType & A, VectorType & v, MatrixType & matrix_1x1, ScalarType beta, arma_size_t j, arma_size_t k)
      {
        using boost::numeric::ublas::range;
        using boost::numeric::ublas::project;
      ...
      template<typename MatrixType, typename VectorType, typename ScalarType>
      void householder_reflect_cuarma(MatrixType & A, VectorType & v, MatrixType & matrix_1x1, ScalarType beta, arma_size_t j, arma_size_t k)
      {
        using cuarma::range;
        using cuarma::project;
        ...
        matrix_1x1 = cuarma::blas::prod(trans(project(v, range(j+1, A.size1()), range(0, 1))),
                                                  project(A, range(j+1, A.size1()), range(k,k+1)));
        v_in_col += matrix_1x1(0,0);
      ...
      template<typename MatrixType, typename VectorType, typename ScalarType>
      void householder_reflect(MatrixType & A, VectorType & v, ScalarType beta, arma_size_t j)
      {
        arma_size_t column_end = A.size2();

        for (arma_size_t k=j; k<column_end; ++k) //over columns
          householder_reflect(A, v, beta, j, k);
      }


      template<typename MatrixType, typename VectorType>
      void write_householder_to_A(MatrixType & A, VectorType const & v, arma_size_t j)
      {
        for (arma_size_t i=j+1; i<A.size1(); ++i)
          A(i,j) = v[i];
      }
...
#ifdef CUARMA_WITH_UBLAS
      template<typename MatrixType, typename VectorType>
      void write_householder_to_A_ublas(MatrixType & A, VectorType const & v, arma_size_t j)
      {
        using boost::numeric::ublas::range;
        using boost::numeric::ublas::project;
      ...
      template<typename MatrixType, typename VectorType>
      void write_householder_to_A_cuarma(MatrixType & A, VectorType const & v, arma_size_t j)
      {
        using cuarma::range;
        using cuarma::project;
      ...
      */
      ...
      std::vector<typename MatrixType::value_type> inplace_qr_ublas(MatrixType & A, arma_size_t block_size = 32)
      {
        typedef typename MatrixType::value_type   ScalarType;
        ...
        for (arma_size_t j = 0; j < std::min(A.size1(), A.size2()); j += block_size)
        {
          arma_size_t effective_block_size = std::min(std::min(A.size1(), A.size2()), j+block_size) - j;

          //determine Householder vectors:
          for (arma_size_t k = 0; k < effective_block_size; ++k)
          {
            betas[j+k] = detail::setup_householder_vector_ublas(A, v, matrix_1x1, j+k);

            for (arma_size_t l = k; l < effective_block_size; ++l)
          ...
          for (arma_size_t k = 1; k < effective_block_size; ++k)
          {
      ...
      *
      * @param A            A dense cuarma matrix to be factored
      * @param block_size   The block size to be used. The number of columns of A must be a multiple of block_size
      */
      template<typename MatrixType>
      std::vector< typename cuarma::result_of::cpu_value_type< typename MatrixType::value_type >::type >
      inplace_qr_cuarma(MatrixType & A, arma_size_t block_size = 16)
      {
        typedef typename cuarma::result_of::cpu_value_type< typename MatrixType::value_type >::type   ScalarType;
        typedef cuarma::matrix_range<MatrixType>  MatrixRange;
        ...
        for (arma_size_t j = 0; j < std::min(A.size1(), A.size2()); j += block_size)
        {
          ...
          for (arma_size_t k = 0; k < effective_block_size; ++k)
          {
            betas[j+k] = detail::setup_householder_vector_cuarma(A, v, matrix_1x1, j+k);
            for (arma_size_t l = k; l < effective_block_size; ++l)
              detail::householder_reflect_cuarma(A, v, matrix_1x1, betas[j+k], j+k, j+l);

            detail::write_householder_to_A_cuarma(A, v, j+k);
          }
...
#ifdef CUARMA_WITH_UBLAS
      //MatrixType is cuarma-matrix
      /** @brief Implementation of a hybrid QR factorization using uBLAS on the CPU and cuarma for GPUs (or multi-core CPU)
      *
      ...
      */
      template<typename MatrixType>
      std::vector< typename cuarma::result_of::cpu_value_type< typename MatrixType::value_type >::type >
      inplace_qr_hybrid(MatrixType & A, arma_size_t block_size = 16)
      {
        typedef typename cuarma::result_of::cpu_value_type< typename MatrixType::value_type >::type   ScalarType;

        typedef cuarma::matrix_range<MatrixType>                      VCLMatrixRange;
        ...
        MatrixType armaW(ublasW.size1(), ublasW.size2());
        MatrixType armaY(ublasY.size1(), ublasY.size2());
        ...
        for (arma_size_t j = 0; j < std::min(A.size1(), A.size2()); j += block_size)
        {
          ...
          cuarma::copy(cuarma::project(A, cuarma::range(0, A.size1()), cuarma::range(j, j+effective_block_size)),  ublasA_part );
          ...
          VCLMatrixRange A_part = cuarma::project(A,
                                                    cuarma::range(0, A.size1()),
                                                    cuarma::range(j, j+effective_block_size));

          cuarma::copy(boost::numeric::ublas::project(ublasA, boost::numeric::ublas::range(0, A.size1()),
                                                        boost::numeric::ublas::range(j, j+effective_block_size)), A_part);

          cuarma::copy(ublasW, armaW);
          cuarma::copy(ublasY, armaY);

          if (A.size2() > j + effective_block_size)
          {

            VCLMatrixRange A_part2(A, cuarma::range(j, A.size1()), cuarma::range(j+effective_block_size, A.size2()));
            VCLMatrixRange W_part(armaW, cuarma::range(j, A.size1()), cuarma::range(0, effective_block_size));
            MatrixType temp = cuarma::blas::prod(trans(W_part), A_part2);

            A_part2 += cuarma::blas::prod(cuarma::project(armaY, cuarma::range(j, A.size1()), cuarma::range(0, effective_block_size)), temp);
          }
        }
    ...
    void recoverQ(MatrixType const & A, VectorType const & betas, MatrixType & Q, MatrixType & R)
    {
      ...
      //
      arma_size_t i_max = std::min(R.size1(), R.size2());
      for (arma_size_t i=0; i<i_max; ++i)
        for (arma_size_t j=i; j<R.size2(); ++j)
          R(i,j) = A(i,j);

      //
      // Recover Q by applying all the Householder reflectors to the identity matrix:
      //
      for (arma_size_t i=0; i<Q.size1(); ++i)
        Q(i,i) = 1.0;

      arma_size_t j_max = std::min(A.size1(), A.size2());
      for (arma_size_t j=0; j<j_max; ++j)
      {
        arma_size_t col_index = j_max - j - 1;
        v[col_index] = 1.0;
        for (arma_size_t i=col_index+1; i<A.size1(); ++i)
          v[i] = A(i, col_index);
    ...
    void inplace_qr_apply_trans_Q(MatrixType const & A, VectorType1 const & betas, VectorType2 & b)
    {
      ...
      //
      for (arma_size_t col_index=0; col_index<std::min(A.size1(), A.size2()); ++col_index)
      {
    ...
    void inplace_qr_apply_trans_Q(cuarma::matrix<T, F, ALIGNMENT> const & A, VectorType1 const & betas, cuarma::vector<T, A2> & b)
    {
      boost::numeric::ublas::matrix<T> ublas_A(A.size1(), A.size2());
      cuarma::copy(A, ublas_A);
    ...
    /** @brief Overload of inplace-QR factorization of a cuarma matrix A
     *
     * @param A            A dense cuarma matrix to be factored
     * @param block_size   The block size to be used.
     */
    template<typename T, typename F, unsigned int ALIGNMENT>
    std::vector<T> inplace_qr(cuarma::matrix<T, F, ALIGNMENT> & A, arma_size_t block_size = 16)
    {
     ...
     */
    template<typename MatrixType>
    std::vector<typename MatrixType::value_type> inplace_qr(MatrixType & A, arma_size_t block_size = 16)
    {
