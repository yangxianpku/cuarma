...
namespace detail
{
  ...
  void gmres_copy_helper(SrcVectorT const & src, DestVectorT & dest, arma_size_t len, arma_size_t start = 0)
  {
    for (arma_size_t i=0; i<len; ++i)
      dest[start+i] = src[start+i];
  }

  template<typename NumericT, typename DestVectorT>
  void gmres_copy_helper(cuarma::vector<NumericT> const & src, DestVectorT & dest, arma_size_t len, arma_size_t start = 0)
  {
    typedef typename cuarma::vector<NumericT>::difference_type   difference_type;
    cuarma::copy( src.begin() + static_cast<difference_type>(start),
  ...
  */
  ...
  void gmres_setup_householder_vector(VectorT const & input_vec, VectorT & hh_vec, NumericT & beta, NumericT & mu, arma_size_t j)
  {
    NumericT input_j = input_vec(j);

    // copy entries from input vector to householder vector:
    detail::gmres_copy_helper(input_vec, hh_vec, cuarma::traits::size(hh_vec) - (j+1), j+1);

    NumericT sigma = cuarma::blas::norm_2(hh_vec);
    sigma *= sigma;
  ...
  {
    NumericT hT_in_x = cuarma::blas::inner_prod(h, x);
    x -= (beta * hT_in_x) * h;
  }
  ...
  */
  ...
  cuarma::vector<ScalarType> pipelined_solve(MatrixType const & A,
                                               cuarma::vector<ScalarType> const & rhs,
                                               gmres_tag const & tag,
                                               cuarma::blas::no_precond,
                                               bool (*monitor)(cuarma::vector<ScalarType> const &, ScalarType, void*) = NULL,
                                               void *monitor_data = NULL)
  {
    cuarma::vector<ScalarType> residual(rhs);
    cuarma::vector<ScalarType> result = cuarma::zero_vector<ScalarType>(rhs.size(), cuarma::traits::context(rhs));

    cuarma::vector<ScalarType> device_krylov_basis(rhs.internal_size() * tag.krylov_dim(), cuarma::traits::context(rhs)); // not using cuarma::matrix here because of spurious padding in column number
    cuarma::vector<ScalarType> device_buffer_R(tag.krylov_dim()*tag.krylov_dim(), cuarma::traits::context(rhs));
    std::vector<ScalarType>      host_buffer_R(device_buffer_R.size());

    arma_size_t buffer_size_per_vector = 128;
    arma_size_t num_buffer_chunks      = 3;
    cuarma::vector<ScalarType> device_inner_prod_buffer = cuarma::zero_vector<ScalarType>(num_buffer_chunks*buffer_size_per_vector, cuarma::traits::context(rhs)); // temporary buffer
    cuarma::vector<ScalarType> device_r_dot_vk_buffer   = cuarma::zero_vector<ScalarType>(buffer_size_per_vector * tag.krylov_dim(), cuarma::traits::context(rhs)); // holds result of first reduction stage for <r, v_k> on device
    cuarma::vector<ScalarType> device_vi_in_vk_buffer   = cuarma::zero_vector<ScalarType>(buffer_size_per_vector * tag.krylov_dim(), cuarma::traits::context(rhs)); // holds <v_i, v_k> for i=0..k-1 on device
    cuarma::vector<ScalarType> device_values_xi_k       = cuarma::zero_vector<ScalarType>(tag.krylov_dim(), cuarma::traits::context(rhs)); // holds values \xi_k = <r, v_k> on device
    ...
    ScalarType norm_rhs = cuarma::blas::norm_2(residual);
    ...
    for (unsigned int restart_count = 0; restart_count <= tag.max_restarts(); ++restart_count)
    {
      ...
      if (restart_count > 0)
      {
        ...
        residual = cuarma::blas::prod(A, result);
        residual = rhs - residual;

        rho_0 = cuarma::blas::norm_2(residual);
      }
      ...
      //
      arma_size_t k = 0;
      for (k = 0; k < static_cast<arma_size_t>(tag.krylov_dim()); ++k)
      {
        if (k == 0)
        {
          // compute v0 = A*r and perform first reduction stage for ||v0||
          cuarma::vector_range<cuarma::vector<ScalarType> > v0(device_krylov_basis, cuarma::range(0, rhs.size()));
          cuarma::blas::pipelined_gmres_prod(A, residual, v0, device_inner_prod_buffer);

          // Normalize v_1 and compute first reduction stage for <r, v_0> in device_r_dot_vk_buffer:
          cuarma::blas::pipelined_gmres_normalize_vk(v0, residual,
        ...
        {
          // compute v0 = A*r and perform first reduction stage for ||v0||
          cuarma::vector_range<cuarma::vector<ScalarType> > vk        (device_krylov_basis, cuarma::range( k   *rhs.internal_size(),  k   *rhs.internal_size() + rhs.size()));
          cuarma::vector_range<cuarma::vector<ScalarType> > vk_minus_1(device_krylov_basis, cuarma::range((k-1)*rhs.internal_size(), (k-1)*rhs.internal_size() + rhs.size()));
          cuarma::blas::pipelined_gmres_prod(A, vk_minus_1, vk, device_inner_prod_buffer);

          //
          // Gram-Schmidt, stage 1: compute first reduction stage of <v_i, v_k>
          //
          cuarma::blas::pipelined_gmres_gram_schmidt_stage1(device_krylov_basis, rhs.size(), rhs.internal_size(), k, device_vi_in_vk_buffer, buffer_size_per_vector);
          ...
          //
          cuarma::blas::pipelined_gmres_gram_schmidt_stage2(device_krylov_basis, rhs.size(), rhs.internal_size(), k,
          ...
          //
          cuarma::blas::pipelined_gmres_normalize_vk(vk, residual,
                                                         device_buffer_R, k*tag.krylov_dim() + k,
                                                         device_inner_prod_buffer, device_r_dot_vk_buffer,
                                                         buffer_size_per_vector, k*buffer_size_per_vector);
        }
      ...
      //
      cuarma::fast_copy(device_r_dot_vk_buffer.begin(), device_r_dot_vk_buffer.end(), host_r_dot_vk_buffer.begin());
      ...
      //
      cuarma::fast_copy(device_buffer_R.begin(), device_buffer_R.end(), host_buffer_R.begin());
      ...
      //
      arma_size_t full_krylov_dim = k; //needed for proper access to R
      for (std::size_t i=0; i<k; ++i)
      {
      ...
      for (int i2=static_cast<int>(k)-1; i2>-1; --i2)
      {
        ...
        for (arma_size_t j=static_cast<arma_size_t>(i)+1; j<k; ++j)
      ...
      //
      for (arma_size_t i=0; i<k; ++i)
        host_update_coefficients[i] = rho_0 * host_values_eta_k_buffer[i];

      cuarma::fast_copy(host_update_coefficients.begin(), host_update_coefficients.end(), device_values_xi_k.begin()); //reuse device_values_xi_k_buffer here for simplicity

      cuarma::blas::pipelined_gmres_update_result(result, residual,
  ...
  */
                     ...
                     void *monitor_data = NULL)
  {
    ...
    unsigned int problem_size = static_cast<unsigned int>(cuarma::traits::size(rhs));
    VectorT result = rhs;
    cuarma::traits::clear(result);

    arma_size_t krylov_dim = static_cast<arma_size_t>(tag.krylov_dim());
    ...
    CPU_NumericType norm_rhs = cuarma::blas::norm_2(rhs);
    ...
    for (unsigned int it = 0; it <= tag.max_restarts(); ++it)
    {
      ...
      //
      res = cuarma::blas::prod(matrix, result);  //initial guess zero
      res = rhs - res;
      precond.apply(res);

      CPU_NumericType rho_0 = cuarma::blas::norm_2(res);
      ...
      for (k = 0; k < krylov_dim; ++k)
      {
        ...
        // prepare storage:
        cuarma::traits::clear(R[k]);
        cuarma::traits::clear(householder_reflectors[k]);

        //compute v_k = A * v_{k-1} via Householder matrices
        if (k == 0)
        {
          v_k_tilde = cuarma::blas::prod(matrix, res);
          precond.apply(v_k_tilde);
        }
        else
        {
          cuarma::traits::clear(v_k_tilde);
          ...
          for (int i = static_cast<int>(k)-1; i > -1; --i)
            detail::gmres_householder_reflect(v_k_tilde, householder_reflectors[arma_size_t(i)], betas[arma_size_t(i)]);

          v_k_tilde_temp = cuarma::blas::prod(matrix, v_k_tilde);
      ...
      for (int i=static_cast<int>(k)-1; i>=0; --i)
        detail::gmres_householder_reflect(res, householder_reflectors[arma_size_t(i)], betas[arma_size_t(i)]);
