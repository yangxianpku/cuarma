...
/** @file cuarma/blas/cuda/bisect_kernel_large_multi.hpp
 *  @encoding:UTF-8 文档编码
                               ...
                               )
{
  const unsigned int tid = threadIdx.x;
    ...
    __shared__  NumericT  s_left[2 * CUARMA_BISECT_MAX_THREADS_BLOCK];
    __shared__  NumericT  s_right[2 * CUARMA_BISECT_MAX_THREADS_BLOCK];

    // number of eigenvalues smaller than interval limits
    __shared__  unsigned int  s_left_count[2 * CUARMA_BISECT_MAX_THREADS_BLOCK];
    __shared__  unsigned int  s_right_count[2 * CUARMA_BISECT_MAX_THREADS_BLOCK];

    // helper array for chunk compaction of second chunk
    __shared__  unsigned int  s_compaction_list[2 * CUARMA_BISECT_MAX_THREADS_BLOCK + 1];
    ...
    if (0 == tid)
    {
     ...
     s_left_count [tid + CUARMA_BISECT_MAX_THREADS_BLOCK] = 0;
     s_right_count[tid + CUARMA_BISECT_MAX_THREADS_BLOCK] = 0;
