...
/** @file cuarma/blas/cuda/ilu_operations.hpp
 *  @encoding:UTF-8 文档编码
               ...
               compressed_matrix<NumericT>       & L)
{
  ...
  //
  extract_L_kernel_1<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(A.handle1()),
                                   cuarma::cuda_arg<unsigned int>(A.handle2()),
                                   static_cast<unsigned int>(A.size1()),
                                   cuarma::cuda_arg<unsigned int>(L.handle1())
                                  );
  CUARMA_CUDA_LAST_ERROR_CHECK("extract_L_kernel_1");

  //
  // Step 2: Exclusive scan on row_buffers:
  //
  cuarma::vector<unsigned int> wrapped_L_row_buffer(cuarma::cuda_arg<unsigned int>(L.handle1().cuda_handle()), cuarma::CUDA_MEMORY, A.size1() + 1);
  cuarma::blas::exclusive_scan(wrapped_L_row_buffer, wrapped_L_row_buffer);
  L.reserve(wrapped_L_row_buffer[L.size1()], false);

  //
  // Step 3: Write entries
  //
  extract_L_kernel_2<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(A.handle1()),
                                   cuarma::cuda_arg<unsigned int>(A.handle2()),
                                   cuarma::cuda_arg<NumericT>(A.handle()),
                                   static_cast<unsigned int>(A.size1()),
                                   cuarma::cuda_arg<unsigned int>(L.handle1()),
                                   cuarma::cuda_arg<unsigned int>(L.handle2()),
                                   cuarma::cuda_arg<NumericT>(L.handle())
                                  );
  CUARMA_CUDA_LAST_ERROR_CHECK("extract_L_kernel_2");
...
void icc_scale(compressed_matrix<NumericT> const & A,
               compressed_matrix<NumericT>       & L)
{
  cuarma::vector<NumericT> D(A.size1(), cuarma::traits::context(A));

  // fill D:
  ilu_scale_kernel_1<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(A.handle1()),
                                   cuarma::cuda_arg<unsigned int>(A.handle2()),
                                   cuarma::cuda_arg<NumericT>(A.handle()),
                                   static_cast<unsigned int>(A.size1()),
                                   cuarma::cuda_arg(D)
                                  );
  CUARMA_CUDA_LAST_ERROR_CHECK("ilu_scale_kernel_1");

  // scale L:
  ilu_scale_kernel_2<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(L.handle1()),
                                   cuarma::cuda_arg<unsigned int>(L.handle2()),
                                   cuarma::cuda_arg<NumericT>(L.handle()),
...
{
  cuarma::backend::mem_handle L_backup;
  cuarma::backend::memory_create(L_backup, L.handle().raw_size(), cuarma::traits::context(L));
  cuarma::backend::memory_copy(L.handle(), L_backup, 0, 0, L.handle().raw_size());

  icc_chow_patel_sweep_kernel<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(L.handle1()),
                                            cuarma::cuda_arg<unsigned int>(L.handle2()),
                                            cuarma::cuda_arg<NumericT>(L.handle()),
                                            cuarma::cuda_arg<NumericT>(L_backup),
                                            static_cast<unsigned int>(L.size1()),

                                            cuarma::cuda_arg<NumericT>(aij_L.handle())
                                           );
  CUARMA_CUDA_LAST_ERROR_CHECK("icc_chow_patel_sweep_kernel");
                ...
                compressed_matrix<NumericT>       & U)
{
  ...
  // Step 1: Count elements in L and U:
  //
  extract_LU_kernel_1<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(A.handle1()),
                                    cuarma::cuda_arg<unsigned int>(A.handle2()),
                                    static_cast<unsigned int>(A.size1()),
                                    cuarma::cuda_arg<unsigned int>(L.handle1()),
                                    cuarma::cuda_arg<unsigned int>(U.handle1())
                                   );
  CUARMA_CUDA_LAST_ERROR_CHECK("extract_LU_kernel_1");

  //
  // Step 2: Exclusive scan on row_buffers:
  //
  cuarma::vector<unsigned int> wrapped_L_row_buffer(cuarma::cuda_arg<unsigned int>(L.handle1()), cuarma::CUDA_MEMORY, A.size1() + 1);
  cuarma::blas::exclusive_scan(wrapped_L_row_buffer, wrapped_L_row_buffer);
  L.reserve(wrapped_L_row_buffer[L.size1()], false);

  cuarma::vector<unsigned int> wrapped_U_row_buffer(cuarma::cuda_arg<unsigned int>(U.handle1()), cuarma::CUDA_MEMORY, A.size1() + 1);
  cuarma::blas::exclusive_scan(wrapped_U_row_buffer, wrapped_U_row_buffer);
  U.reserve(wrapped_U_row_buffer[U.size1()], false);

  //
  // Step 3: Write entries
  //
  extract_LU_kernel_2<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(A.handle1()),
                                    cuarma::cuda_arg<unsigned int>(A.handle2()),
                                    cuarma::cuda_arg<NumericT>(A.handle()),
                                    static_cast<unsigned int>(A.size1()),
                                    cuarma::cuda_arg<unsigned int>(L.handle1()),
                                    ...
                                    cuarma::cuda_arg<NumericT>(L.handle()),
                                    cuarma::cuda_arg<unsigned int>(U.handle1()),
                                    cuarma::cuda_arg<unsigned int>(U.handle2()),
                                    cuarma::cuda_arg<NumericT>(U.handle())
                                   );
  CUARMA_CUDA_LAST_ERROR_CHECK("extract_LU_kernel_2");
               ...
               compressed_matrix<NumericT>       & U)
{
  ...
  ilu_scale_kernel_1<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(A.handle1()),
                                   ...
                                   cuarma::cuda_arg<NumericT>(A.handle()),
                                   static_cast<unsigned int>(A.size1()),
                                   cuarma::cuda_arg<NumericT>(D.handle())
                                  );
  CUARMA_CUDA_LAST_ERROR_CHECK("ilu_scale_kernel_1");
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("ilu_scale_kernel_2");

  // scale U:
  ilu_scale_kernel_2<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(U.handle1()),
                                   cuarma::cuda_arg<unsigned int>(U.handle2()),
                                   cuarma::cuda_arg<NumericT>(U.handle()),
                          ...
                          vector<NumericT>            const & aij_U_trans)
{
  ...
  cuarma::backend::mem_handle U_backup;
  cuarma::backend::memory_create(U_backup, U_trans.handle().raw_size(), cuarma::traits::context(U_trans));
  cuarma::backend::memory_copy(U_trans.handle(), U_backup, 0, 0, U_trans.handle().raw_size());

  ilu_chow_patel_sweep_kernel<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(L.handle1()),
                                            cuarma::cuda_arg<unsigned int>(L.handle2()),
                                            cuarma::cuda_arg<NumericT>(L.handle()),
                                            cuarma::cuda_arg<NumericT>(L_backup),
                                            static_cast<unsigned int>(L.size1()),
                                            ...
                                            cuarma::cuda_arg<unsigned int>(U_trans.handle1()),
                                            cuarma::cuda_arg<unsigned int>(U_trans.handle2()),
                                            cuarma::cuda_arg<NumericT>(U_trans.handle()),
                                            cuarma::cuda_arg<NumericT>(U_backup),

                                            cuarma::cuda_arg<NumericT>(aij_U_trans.handle())
                                           );
  CUARMA_CUDA_LAST_ERROR_CHECK("ilu_chow_patel_sweep_kernel");
...
{
  ilu_form_neumann_matrix_kernel<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(R.handle1()),
                                               cuarma::cuda_arg<unsigned int>(R.handle2()),
                                               cuarma::cuda_arg<NumericT>(R.handle()),
                                               static_cast<unsigned int>(R.size1()),
                                               cuarma::cuda_arg<NumericT>(diag_R.handle())
                                              );
  CUARMA_CUDA_LAST_ERROR_CHECK("ilu_form_neumann_matrix_kernel");
}
