...
/** @file cuarma/blas/cuda/bisect_kernel_large.hpp
 *  @encoding:UTF-8 文档编码
                             ...
                             )
{
    ...
    // CUARMA_BISECT_MAX_THREADS_BLOCK eigenvalues (so that each of these blocks may be
        ...
        if (tid < d)
        {
            ...
            if ((s_cl_helper[ai - 1] != 1) || (s_cl_helper[bi] != 1))
            {
                ...
                else    // both childs are non-terminated
                {
                    ...
                    if (temp > (mat_size > 512 ? CUARMA_BISECT_MAX_THREADS_BLOCK : CUARMA_BISECT_MAX_THREADS_BLOCK / 2))
                    {
                 ...
                 )
{
    ...
    // not dense
    __shared__  NumericT  s_left[2 * CUARMA_BISECT_MAX_THREADS_BLOCK + 1];
    __shared__  NumericT  s_right[2 * CUARMA_BISECT_MAX_THREADS_BLOCK + 1];

    // number of eigenvalues that are smaller than s_left / s_right
    // (correspondence is realized via indices)
    __shared__  unsigned short  s_left_count[2 * CUARMA_BISECT_MAX_THREADS_BLOCK + 1];
    __shared__  unsigned short  s_right_count[2 * CUARMA_BISECT_MAX_THREADS_BLOCK + 1];

    // helper for stream compaction
    __shared__  unsigned short  s_compaction_list[2 * CUARMA_BISECT_MAX_THREADS_BLOCK + 1];
    ...
    {
        s_compaction_list[tid] = 0;
        s_compaction_list[tid + CUARMA_BISECT_MAX_THREADS_BLOCK] = 0;
        s_compaction_list[2 * CUARMA_BISECT_MAX_THREADS_BLOCK] = 0;
    ...
    __shared__ unsigned short  s_cl_helper[2 * CUARMA_BISECT_MAX_THREADS_BLOCK + 1];
    ...
    else if (tid_2 < (2 * (n > 512 ? CUARMA_BISECT_MAX_THREADS_BLOCK : CUARMA_BISECT_MAX_THREADS_BLOCK / 2) + 1))
    {
