...
//#ifdef CUARMA_WITH_SPGEMM_RMERGE
...
namespace detail
{
  ...
  void row_info(compressed_matrix<NumericT, AligmentV> const & mat,
                ...
                cuarma::blas::detail::row_info_types info_selector)
  {
    csr_row_info_extractor_kernel<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                                cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                                cuarma::cuda_arg<NumericT>(mat.handle()),
                                                cuarma::cuda_arg(vec),
                                                static_cast<unsigned int>(mat.size1()),
                                                static_cast<unsigned int>(info_selector)
                                               );
    CUARMA_CUDA_LAST_ERROR_CHECK("csr_row_info_extractor_kernel");
  }
          ...
          NumericT beta)
{
  ...
  for (unsigned int block_id = blockIdx.x; block_id < num_blocks; block_id += gridDim.x)
  {
    ...
    // TODO here: Consider CSR vector for two to four rows 
...
void prod_impl(const cuarma::compressed_matrix<NumericT, AlignmentV> & mat,
               const cuarma::vector_base<NumericT> & vec,
               NumericT alpha,
                     cuarma::vector_base<NumericT> & result,
               NumericT beta)
{
  ...
  if (is_maxwell && double(mat.nnz()) / double(mat.size1()) > 6.4) // less than 10% of threads expected to idle
  {
    if (alpha < NumericT(1) || alpha > NumericT(1) || beta < 0 || beta > 0)
      compressed_matrix_vec_mul_kernel<8, detail::spmv_alpha_beta, NumericT><<<512, 256>>>(   // experience on a GTX 750 Ti suggests that 8 is a substantially better choice here
                                                                    cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                                                    cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                                                    cuarma::cuda_arg<NumericT>(mat.handle()),
                                                                    cuarma::cuda_arg(vec),
     ...
     CUARMA_CUDA_LAST_ERROR_CHECK("compressed_matrix_vec_mul_kernel");
  }
  ...
  {
    if (alpha < NumericT(1) || alpha > NumericT(1) || beta < 0 || beta > 0)
      compressed_matrix_vec_mul_adaptive_kernel<detail::spmv_alpha_beta><<<512, 256>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                                              cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                                              cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                                              cuarma::cuda_arg<NumericT>(mat.handle()),
      ...
      compressed_matrix_vec_mul_adaptive_kernel<detail::spmv_pure><<<512, 256>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                                              cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                                              cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                                              cuarma::cuda_arg<NumericT>(mat.handle()),
                                                              static_cast<unsigned int>(mat.blocks1()),
    ...
    CUARMA_CUDA_LAST_ERROR_CHECK("compressed_matrix_vec_mul_adaptive_kernel");
  }
}
...
/** \cond */
template<>
struct mat_mult_matrix_index<cuarma::column_major>
{
  static __device__ unsigned int apply(unsigned int i, unsigned int j,
                                unsigned int row_start, unsigned int row_inc,
                                unsigned int col_start, unsigned int col_inc,
...
void prod_impl(const cuarma::compressed_matrix<NumericT, AlignmentV> & sp_mat,
               const cuarma::matrix_base<NumericT> & d_mat,
                     cuarma::matrix_base<NumericT> & result)
{
  if (d_mat.row_major() && result.row_major())
  {
    compressed_matrix_d_mat_mul_kernel<mat_mult_matrix_index<row_major>, mat_mult_matrix_index<row_major> ><<<128, 128>>>
                                                  (cuarma::cuda_arg<unsigned int>(sp_mat.handle1()),
                                                   cuarma::cuda_arg<unsigned int>(sp_mat.handle2()),
                                                   cuarma::cuda_arg<NumericT>(sp_mat.handle()),

                                                   cuarma::cuda_arg(d_mat),
                                                   static_cast<unsigned int>(cuarma::traits::start1(d_mat)),         static_cast<unsigned int>(cuarma::traits::start2(d_mat)),
                                                   static_cast<unsigned int>(cuarma::traits::stride1(d_mat)),        static_cast<unsigned int>(cuarma::traits::stride2(d_mat)),
                                                   static_cast<unsigned int>(cuarma::traits::size1(d_mat)),          static_cast<unsigned int>(cuarma::traits::size2(d_mat)),
                                                   static_cast<unsigned int>(cuarma::traits::internal_size1(d_mat)), static_cast<unsigned int>(cuarma::traits::internal_size2(d_mat)),

                                                   cuarma::cuda_arg(result),
                                                   static_cast<unsigned int>(cuarma::traits::start1(result)),         static_cast<unsigned int>(cuarma::traits::start2(result)),
                                                   static_cast<unsigned int>(cuarma::traits::stride1(result)),        static_cast<unsigned int>(cuarma::traits::stride2(result)),
                                                   static_cast<unsigned int>(cuarma::traits::size1(result)),          static_cast<unsigned int>(cuarma::traits::size2(result)),
                                                   static_cast<unsigned int>(cuarma::traits::internal_size1(result)), static_cast<unsigned int>(cuarma::traits::internal_size2(result))
                                                  );
    CUARMA_CUDA_LAST_ERROR_CHECK("compressed_matrix_d_mat_mul_kernel");
  }
...
template<typename NumericT, unsigned int AlignmentV>
void prod_impl(const cuarma::compressed_matrix<NumericT, AlignmentV> & sp_mat,
               const cuarma::matrix_expression< const cuarma::matrix_base<NumericT>,
  ...
  if (d_mat.lhs().row_major() && result.row_major())
  {
    compressed_matrix_d_tr_mat_mul_kernel<mat_mult_matrix_index<row_major>, mat_mult_matrix_index<row_major> ><<<128, 128>>>
                                                (cuarma::cuda_arg<unsigned int>(sp_mat.handle1()),
                                                 ...
                                                 cuarma::cuda_arg(d_mat.lhs()),
                                                 static_cast<unsigned int>(cuarma::traits::start1(d_mat.lhs())),         static_cast<unsigned int>(cuarma::traits::start2(d_mat.lhs())),
                                                 static_cast<unsigned int>(cuarma::traits::stride1(d_mat.lhs())),        static_cast<unsigned int>(cuarma::traits::stride2(d_mat.lhs())),
                                                 static_cast<unsigned int>(cuarma::traits::size1(d_mat.lhs())),          static_cast<unsigned int>(cuarma::traits::size2(d_mat.lhs())),
                                                 static_cast<unsigned int>(cuarma::traits::internal_size1(d_mat.lhs())), static_cast<unsigned int>(cuarma::traits::internal_size2(d_mat.lhs())),
    ...
    CUARMA_CUDA_LAST_ERROR_CHECK("compressed_matrix_d_tr_mat_mul_kernel");
  }
...
typename cuarma::enable_if< cuarma::is_any_sparse_matrix<SparseMatrixT>::value>::type
inplace_solve(const SparseMatrixT & mat,
              cuarma::vector_base<NumericT> & vec,
              cuarma::blas::unit_lower_tag)
{
  csr_unit_lu_forward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                         cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                         cuarma::cuda_arg<NumericT>(mat.handle()),
                                         cuarma::cuda_arg(vec),
                                         static_cast<unsigned int>(mat.size1())
                                        );
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_unit_lu_forward_kernel");
}
...
inplace_solve(const SparseMatrixT & mat,
              cuarma::vector_base<NumericT> & vec,
              cuarma::blas::lower_tag)
{
  csr_lu_forward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                    cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                    cuarma::cuda_arg<NumericT>(mat.handle()),
                                    cuarma::cuda_arg(vec),
                                    static_cast<unsigned int>(mat.size1())
                                   );
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_lu_forward_kernel");
}
...
inplace_solve(const SparseMatrixT & mat,
              cuarma::vector_base<NumericT> & vec,
              cuarma::blas::unit_upper_tag)
{
  csr_unit_lu_backward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                    cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                    cuarma::cuda_arg<NumericT>(mat.handle()),
                                    cuarma::cuda_arg(vec),
                                    static_cast<unsigned int>(mat.size1())
                                   );
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_unit_lu_backward_kernel");
}
...
{
  csr_lu_backward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                    cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                    cuarma::cuda_arg<NumericT>(mat.handle()),
                                    cuarma::cuda_arg(vec),
                                    static_cast<unsigned int>(mat.size1())
                                   );
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_lu_backward_kernel");
}
...
inplace_solve(const matrix_expression<const SparseMatrixT, const SparseMatrixT, op_trans> & mat,
              ...
              cuarma::blas::unit_lower_tag)
{
  csr_trans_unit_lu_forward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.lhs().handle1()),
                                          cuarma::cuda_arg<unsigned int>(mat.lhs().handle2()),
                                          cuarma::cuda_arg<NumericT>(mat.lhs().handle()),
                                          cuarma::cuda_arg(vec),
                                          static_cast<unsigned int>(mat.lhs().size1())
                                         );
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_trans_unit_lu_forward_kernel");
}
...
inplace_solve(const matrix_expression<const SparseMatrixT, const SparseMatrixT, op_trans> & mat,
              ...
              cuarma::blas::lower_tag)
{
  cuarma::vector<NumericT> diagonal(vec.size());

  compressed_matrix_diagonal_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.lhs().handle1()),
                                                cuarma::cuda_arg<unsigned int>(mat.lhs().handle2()),
                                                cuarma::cuda_arg<NumericT>(mat.lhs().handle()),
                                                cuarma::cuda_arg(diagonal),
                                                static_cast<unsigned int>(mat.size1())
                                               );

  csr_trans_lu_forward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.lhs().handle1()),
                                          cuarma::cuda_arg<unsigned int>(mat.lhs().handle2()),
                                          cuarma::cuda_arg<NumericT>(mat.lhs().handle()),
                                          cuarma::cuda_arg(diagonal),
                                          cuarma::cuda_arg(vec),
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_trans_lu_forward_kernel");
}
...
inplace_solve(const matrix_expression<const SparseMatrixT, const SparseMatrixT, op_trans> & mat,
              ...
              cuarma::blas::unit_upper_tag)
{
  csr_trans_unit_lu_backward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.lhs().handle1()),
                                                cuarma::cuda_arg<unsigned int>(mat.lhs().handle2()),
                                                cuarma::cuda_arg<NumericT>(mat.lhs().handle()),
                                                cuarma::cuda_arg(vec),
                                                static_cast<unsigned int>(mat.lhs().size1())
                                              );
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_trans_unit_lu_backward_kernel");
}
              ...
              cuarma::blas::upper_tag)
{
  ...
  csr_trans_lu_backward_kernel<<<1, 128>>>(cuarma::cuda_arg<unsigned int>(mat.lhs().handle1()),
                                           cuarma::cuda_arg<unsigned int>(mat.lhs().handle2()),
                                           cuarma::cuda_arg<NumericT>(mat.lhs().handle()),
                                           cuarma::cuda_arg(diagonal),
                                           cuarma::cuda_arg(vec),
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("csr_trans_lu_backward_kernel");
}
...
namespace detail
{
  ...
  void block_inplace_solve(const matrix_expression<const compressed_matrix<NumericT, AlignmentV>,
                                                   ...
                                                   op_trans> & L,
                           cuarma::backend::mem_handle const & block_indices, arma_size_t num_blocks,
                           vector_base<NumericT> const & /* L_diagonal */,  //ignored
                           vector_base<NumericT> & vec,
                           cuarma::blas::unit_lower_tag)
  {
    csr_block_trans_unit_lu_forward<<<num_blocks, 128>>>(cuarma::cuda_arg<unsigned int>(L.lhs().handle1()),
                                                         cuarma::cuda_arg<unsigned int>(L.lhs().handle2()),
                                                         cuarma::cuda_arg<NumericT>(L.lhs().handle()),
                                                         cuarma::cuda_arg<unsigned int>(block_indices),
                                                         cuarma::cuda_arg(vec),
  ...
  void block_inplace_solve(const matrix_expression<const compressed_matrix<NumericT, AlignmentV>,
                           ...
                           cuarma::blas::upper_tag)
  {
    csr_block_trans_lu_backward<<<num_blocks, 128>>>(cuarma::cuda_arg<unsigned int>(U.lhs().handle1()),
                                                     cuarma::cuda_arg<unsigned int>(U.lhs().handle2()),
                                                     cuarma::cuda_arg<NumericT>(U.lhs().handle()),
                                                     cuarma::cuda_arg(U_diagonal),
                                                     cuarma::cuda_arg<unsigned int>(block_indices),
                                                     cuarma::cuda_arg(vec),
...
*/
template<typename NumericT>
void prod_impl(const cuarma::compressed_compressed_matrix<NumericT> & mat,
               const cuarma::vector_base<NumericT> & vec,
               NumericT alpha,
                     cuarma::vector_base<NumericT> & result,
               NumericT beta)
{
  if (beta < 0 || beta > 0)
    cuarma::blas::cuda::av(result, result, beta, 1, false, false);
  else
    result.clear();

  compressed_compressed_matrix_vec_mul_kernel<<<128, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                                            cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                                            cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                                            cuarma::cuda_arg<NumericT>(mat.handle()),
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("compressed_compressed_matrix_vec_mul_kernel");
}
...
namespace detail
{
  ...
  void row_info(coordinate_matrix<NumericT, AlignmentV> const & mat,
                ...
                cuarma::blas::detail::row_info_types info_selector)
  {
    coo_row_info_extractor<<<64, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle12()),
                                         cuarma::cuda_arg<NumericT>(mat.handle()),
                                         cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                         cuarma::cuda_arg(vec),
                                         static_cast<unsigned int>(info_selector)
                                        );
    CUARMA_CUDA_LAST_ERROR_CHECK("coo_row_info_extractor");
  }
...
*/
template<typename NumericT, unsigned int AlignmentV>
void prod_impl(const cuarma::coordinate_matrix<NumericT, AlignmentV> & mat,
               const cuarma::vector_base<NumericT> & vec,
               NumericT alpha,
                     cuarma::vector_base<NumericT> & result,
               NumericT beta)
{
  ...
  coordinate_matrix_vec_mul_kernel<<<64, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle12()),
                                                cuarma::cuda_arg<NumericT>(mat.handle()),
                                                cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                                cuarma::cuda_arg(vec),
                                                static_cast<unsigned int>(vec.start()),
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("coordinate_matrix_vec_mul_kernel");
}
...
*/
template<typename NumericT, unsigned int AlignmentV>
void prod_impl(const cuarma::coordinate_matrix<NumericT, AlignmentV> & sp_mat,
               const cuarma::matrix_base<NumericT> & d_mat,
                     cuarma::matrix_base<NumericT> & result)
{
  if (d_mat.row_major() && result.row_major())
  {
    coordinate_matrix_d_mat_mul_kernel<mat_mult_matrix_index<row_major>, mat_mult_matrix_index<row_major> ><<<64, 128>>>
                                                  (cuarma::cuda_arg<unsigned int>(sp_mat.handle12()),
                                                   cuarma::cuda_arg<NumericT>(sp_mat.handle()),
                                                   cuarma::cuda_arg<unsigned int>(sp_mat.handle3()),
    ...
    CUARMA_CUDA_LAST_ERROR_CHECK("coordinate_matrix_d_mat_mul_kernel");
  }
  else if (d_mat.row_major() && !result.row_major())
  {
                     ...
                     cuarma::matrix_base<NumericT> & result)
{
  if (d_mat.lhs().row_major() && result.row_major())
  {
    ...
    CUARMA_CUDA_LAST_ERROR_CHECK("coordinate_matrix_d_tr_mat_mul_kernel");
  }
  else if (d_mat.lhs().row_major() && !result.row_major())
  {
...
*/
template<typename NumericT, unsigned int AlignmentV>
void prod_impl(const cuarma::ell_matrix<NumericT, AlignmentV> & mat,
               const cuarma::vector_base<NumericT> & vec,
               NumericT alpha,
                     cuarma::vector_base<NumericT> & result,
               NumericT beta)
{
  if (alpha < NumericT(1) || alpha > NumericT(1) || beta < 0 || beta > 0)
    ell_matrix_vec_mul_kernel<detail::spmv_alpha_beta><<<256, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                            cuarma::cuda_arg<NumericT>(mat.handle()),
                                            cuarma::cuda_arg(vec),
                                            static_cast<unsigned int>(vec.start()),
                                            static_cast<unsigned int>(vec.stride()),
    ...
    ell_matrix_vec_mul_kernel<detail::spmv_pure><<<256, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                            cuarma::cuda_arg<NumericT>(mat.handle()),
                                            cuarma::cuda_arg(vec),
                                            static_cast<unsigned int>(vec.start()),
                                            static_cast<unsigned int>(vec.stride()),
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("ell_matrix_vec_mul_kernel");
}
...
*/
template<typename NumericT, unsigned int AlignmentV>
void prod_impl(const cuarma::ell_matrix<NumericT, AlignmentV> & sp_mat,
               const cuarma::matrix_base<NumericT> & d_mat,
                     cuarma::matrix_base<NumericT> & result)
{
  if (d_mat.row_major() && result.row_major())
  {
    ...
    CUARMA_CUDA_LAST_ERROR_CHECK("ell_matrix_d_mat_mul_kernel");
  }
  else if (d_mat.row_major() && !result.row_major())
  {
                     ...
                     cuarma::matrix_base<NumericT> & result)
{
  if (d_mat.lhs().row_major() && result.row_major())
  {
    ...
    CUARMA_CUDA_LAST_ERROR_CHECK("ell_matrix_d_tr_mat_mul_kernel");
  }
  else if (d_mat.lhs().row_major() && !result.row_major())
  {
...
void prod_impl(const cuarma::sliced_ell_matrix<NumericT, IndexT> & mat,
               const cuarma::vector_base<NumericT> & vec,
               NumericT alpha,
                     cuarma::vector_base<NumericT> & result,
               NumericT beta)
{
  if (alpha < NumericT(1) || alpha > NumericT(1) || beta < 0 || beta > 0)
    sliced_ell_matrix_vec_mul_kernel<detail::spmv_alpha_beta><<<256, 256>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                                   cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                                   cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                                   cuarma::cuda_arg<NumericT>(mat.handle()),
                                                   cuarma::cuda_arg(vec),
    ...
    sliced_ell_matrix_vec_mul_kernel<detail::spmv_pure><<<256, 256>>>(cuarma::cuda_arg<unsigned int>(mat.handle1()),
                                                   cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                                   cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                                   cuarma::cuda_arg<NumericT>(mat.handle()),
                                                   cuarma::cuda_arg(vec),
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("sliced_ell_matrix_vec_mul_kernel");
}
...
*/
template<typename NumericT, unsigned int AlignmentV>
void prod_impl(const cuarma::hyb_matrix<NumericT, AlignmentV> & mat,
               const cuarma::vector_base<NumericT> & vec,
               NumericT alpha,
                     cuarma::vector_base<NumericT> & result,
               NumericT beta)
{
  if (alpha < NumericT(1) || alpha > NumericT(1) || beta < 0 || beta > 0)
    hyb_matrix_vec_mul_kernel<detail::spmv_alpha_beta><<<256, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                            cuarma::cuda_arg<NumericT>(mat.handle()),
                                            cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                            cuarma::cuda_arg<unsigned int>(mat.handle4()),
                                            cuarma::cuda_arg<NumericT>(mat.handle5()),
                                            cuarma::cuda_arg(vec),
                                            static_cast<unsigned int>(vec.start()),
                                            static_cast<unsigned int>(vec.stride()),
                                            alpha,
    ...
    hyb_matrix_vec_mul_kernel<detail::spmv_pure><<<256, 128>>>(cuarma::cuda_arg<unsigned int>(mat.handle2()),
                                            cuarma::cuda_arg<NumericT>(mat.handle()),
                                            cuarma::cuda_arg<unsigned int>(mat.handle3()),
                                            cuarma::cuda_arg<unsigned int>(mat.handle4()),
                                            cuarma::cuda_arg<NumericT>(mat.handle5()),
  ...
  CUARMA_CUDA_LAST_ERROR_CHECK("hyb_matrix_vec_mul_kernel");
}
