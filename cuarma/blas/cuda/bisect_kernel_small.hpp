...
/** @file cuarma/blas/cuda/bisect_kernel_small.hpp
 *  @encoding:UTF-8 文档编码
            ...
            )
{
    ...
    // not dense
    __shared__  NumericT  s_left[CUARMA_BISECT_MAX_THREADS_BLOCK_SMALL_MATRIX];
    __shared__  NumericT  s_right[CUARMA_BISECT_MAX_THREADS_BLOCK_SMALL_MATRIX];

    // number of eigenvalues that are smaller than s_left / s_right
    // (correspondence is realized via indices)
    __shared__  unsigned int  s_left_count[CUARMA_BISECT_MAX_THREADS_BLOCK_SMALL_MATRIX];
    __shared__  unsigned int  s_right_count[CUARMA_BISECT_MAX_THREADS_BLOCK_SMALL_MATRIX];

    // helper for stream compaction
    __shared__  unsigned int
    s_compaction_list[CUARMA_BISECT_MAX_THREADS_BLOCK_SMALL_MATRIX + 1];
