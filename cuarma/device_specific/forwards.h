...
/** @file cuarma/device_specific/forwards.h
*   @encoding:UTF-8 文档编码
...
inline scheduler::statement_node const & lhs_most(scheduler::statement::container_type const & array, arma_size_t root)
{
...
public:
  ...
  generator_not_supported_exception(std::string message) : message_("cuarma: Internal error: The generator cannot handle the statement provided: " + message) {}
...
typedef std::pair<arma_size_t, leaf_t> mapping_key;
...
namespace tree_parsing
{
  ...
  template<class Fun>
  inline void traverse(scheduler::statement const & statement, arma_size_t root_idx, Fun const & fun, bool inspect);
  ...
  inline void process(utils::kernel_generation_stream & stream, leaf_t leaf, std::string const & type_key, std::string const & to_process,
                      scheduler::statement const & statement, arma_size_t root_idx, mapping_type const & mapping, std::set<std::string> & already_processed);
  inline std::string evaluate(leaf_t leaf, std::map<std::string, std::string> const & accessors, scheduler::statement const & statement, arma_size_t root_idx,mapping_type const & mapping);
}
...
public:
  ...
  virtual bool bind(cuarma::backend::mem_handle const * ph) = 0;
  virtual unsigned int get(cuarma::backend::mem_handle const * ph) = 0;
};
...
public:
  ...
  bool bind(cuarma::backend::mem_handle const * ph) {return (ph==NULL)?true:memory.insert(std::make_pair((void*)ph, current_arg_)).second; }
  unsigned int get(cuarma::backend::mem_handle const * ph){ return bind(ph) ? current_arg_++ : at(memory, (void*)ph); }
private:
...
public:
  ...
  bool bind(cuarma::backend::mem_handle const *) {return true; }
  unsigned int get(cuarma::backend::mem_handle const *){ return current_arg_++; }
private:
  unsigned int current_arg_;
  std::map<void*,unsigned int> memory;
};
